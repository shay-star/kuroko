...
#define LIST_WRAP_INDEX()                                                                \
    ...
    if (unlikely(index < 0 || index >= (krk_integer_type)self->values.count))            \
    return krk_runtimeError(vm.exceptions->indexError, "list index out of range: %zd",   \
                            (ssize_t)index)
...
#define LIST_WRAP_SOFT(val)                                                              \
    ...
    if (val < 0)                                                                         \
        val = 0;                                                                         \
    if (val > (krk_integer_type)self->values.count)                                      \
    val = self->values.count
...
KrkValue krk_list_of(int argc, const KrkValue argv[], int hasKw) {
    ...
    if (argc) {
        ...
        AS_LIST(outList)->values =
            KRK_GROW_ARRAY(KrkValue, AS_LIST(outList)->values, 0, argc);
...
KRK_Method(list, __getitem__) {
    ...
    if (IS_INTEGER(argv[1])) {
        ...
        if (vm.globalFlags & KRK_GLOBAL_THREADS)
        ...
        if (step == 1) {
            ...
            /* make into a list */
            KrkValue result = krk_callNativeOnStack(
                len, &krk_currentThread.stackTop[-len], 0, krk_list_of);
...
KRK_Method(list, __eq__) {
    ...
    if (self->values.count != them->values.count)
        return BOOLEAN_VAL(0);
    for (size_t i = 0; i < self->values.count; ++i) {
        if (!krk_valuesSameOrEqual(self->values.values[i], them->values.values[i]))
            return BOOLEAN_VAL(0);
    }
...
KRK_Method(list, insert) {
    ...
    memmove(&self->values.values[index + 1], &self->values.values[index],
...
KRK_Method(list, __repr__) {
    METHOD_TAKES_NONE();
    if (((KrkObj *)self)->flags & KRK_OBJ_FLAGS_IN_REPR)
        return OBJECT_VAL(S("[...]"));
    ...
    pthread_rwlock_rdlock(&self->rwlock);
    for (size_t i = 0; i < self->values.count; ++i) {
        if (!krk_pushStringBuilderFormat(&sb, "%R", self->values.values[i]))
            goto _error;
...
KRK_Method(list, __setitem__) {
    ...
    if (IS_INTEGER(argv[1])) {
        ...
        KRK_SLICER(argv[1], self->values.count) { return NONE_VAL(); }
        ...
        while (len < newLen) {
            FUNC_NAME(list, insert)
            (3,
             (KrkValue[]){argv[0], INTEGER_VAL(start + len),
                          AS_LIST(argv[2])->values[len]},
             0);
            len++;
        }
        ...
        while (newLen < len) {
            FUNC_NAME(list, pop)
            (2, (KrkValue[]){argv[0], INTEGER_VAL(start + len - 1)}, 0);
            len--;
        }
...
KRK_Method(list, index) {
    ...
    if (argc > 2) {
        if (IS_INTEGER(argv[2]))
            ...
            return krk_runtimeError(vm.exceptions->typeError, "%s must be int, not '%T'",
                                    "min", argv[2]);
    }
...
KRK_Method(list, reverse) {
    ...
    if (self->values.count > 1)
        reverse_values(self->values.values, self->values.count);
    pthread_rwlock_unlock(&self->rwlock);
    return NONE_VAL();
}
...
static void copy_slice(struct SortSlice start, struct SortSlice end,
                       struct SortSlice buffer) {
    while (start.keys != end.keys) {
        *buffer.keys = *start.keys;
        if (buffer.values)
            *buffer.values = *start.values;
        slice_advance(&start);
...
static struct SortSlice powersort_strictlyDecreasingPrefix(struct SortSlice begin,
                                                           struct SortSlice end) {
    while (begin.keys + 1 < end.keys && _list_sorter(*(begin.keys + 1), *begin.keys))
        slice_advance(&begin);
    return slice_next(begin);
}

/** @brief While next is greater than or equal to current, advance current */
static struct SortSlice powersort_weaklyIncreasingPrefix(struct SortSlice begin,
                                                         struct SortSlice end) {
    while (begin.keys + 1 < end.keys && !_list_sorter(*(begin.keys + 1), *begin.keys))
        slice_advance(&begin);
    return slice_next(begin);
 ...
 */
static struct SortSlice powersort_extend_and_reverse_right(struct SortSlice begin,
                                                           struct SortSlice end) {
    struct SortSlice j = begin;
    if (j.keys == end.keys)
        return j;
    if (j.keys + 1 == end.keys)
        return slice_next(j);
    if (_list_sorter(*slice_next(j).keys, *j.keys)) {
        ...
        if (begin.values)
            reverse_values(begin.values, j.values - begin.values);
    } else {
 ...
 */
static size_t powersort_power(size_t begin, size_t end, size_t beginA, size_t beginB,
                              size_t endB) {
    size_t n = end - begin;
 ...
 */
static void powersort_merge(struct SortSlice left, struct SortSlice mid,
                            struct SortSlice right, struct SortSlice buffer) {
...
static void powersort(KrkList *list, KrkValue key, int reverse) {
    ...
    for (size_t i = 0; i < bufferSpace->values.capacity; ++i)
        bufferSpace->values.values[bufferSpace->values.count++] = NONE_VAL();
    struct SortSlice buffer = {&bufferSpace->values.values[0],
                               slice.values ? &bufferSpace->values.values[n] : NULL};
    ...
    while (a.end.keys < end.keys) {
        /* Our next run is whatever is after that, assuming the initial run isn't the
         * whole list. */
        ...
        a.power = powersort_power(0, n, a.start.keys - begin.keys,
                                  b.start.keys - begin.keys, b.end.keys - begin.keys);
    ...
    /* While there are things in the stack (excluding the empty 0 slot), merge them into
     * the last a */
    while (top > 0) {
        struct SliceAndPower top_run = stack[top--];
...
_end_sort:
    ...
    if (reverse)
        reverse_values(list->values.values, n);
}

KRK_Method(list, sort) {
    KrkValue key = NONE_VAL();
    int reverse = 0;
    if (!krk_parseArgs(".|$Vp", (const char *[]){"key", "reverse"}, &key, &reverse))
        return NONE_VAL();
...
#define MAKE_LIST_COMPARE(name, op)                                                      \
    KRK_Method(list, __##name##__) {                                                     \
        ...
        if (!IS_list(argv[1]))                                                           \
            return NOTIMPL_VAL();                                                        \
        KrkList *them = AS_list(argv[1]);                                                \
        size_t lesser = self->values.count < them->values.count ? self->values.count     \
                                                                : them->values.count;    \
        for (size_t i = 0; i < lesser; ++i) {                                            \
            KrkValue a = self->values.values[i];                                         \
            KrkValue b = them->values.values[i];                                         \
            if (krk_valuesSameOrEqual(a, b))                                             \
                continue;                                                                \
            if (unlikely(krk_currentThread.flags & KRK_THREAD_HAS_EXCEPTION))            \
                return NONE_VAL();                                                       \
...
#define IS_listiterator(o)                                                               \
    (likely(IS_INSTANCE(o) &&                                                            \
            AS_INSTANCE(o)->_class == vm.baseClasses->listiteratorClass) ||              \
     krk_isInstanceOf(o, vm.baseClasses->listiteratorClass))
...
FUNC_SIG(listiterator, __call__) {
    ...
    if (unlikely((argc != 1)))
        goto _bad;
    if (unlikely(!IS_OBJECT(argv[0])))
        goto _bad;
    if (unlikely(AS_INSTANCE(argv[0])->_class != vm.baseClasses->listiteratorClass))
        goto _bad;
...
_bad:
    ...
    if (!krk_isInstanceOf(argv[0], vm.baseClasses->listiteratorClass))
...
KRK_Function(sorted) {
    if (argc < 1)
        return krk_runtimeError(vm.exceptions->argumentError,
                                "%s() takes %s %d argument%s (%d given)", "sorted",
                                "at least", 1, "", argc);
    ...
    if (!IS_NONE(krk_currentThread.currentException))
        return NONE_VAL();
    FUNC_NAME(list, sort)
    (1, (KrkValue[]){listOut, hasKw ? argv[1] : NONE_VAL(), hasKw ? argv[2] : NONE_VAL()},
     hasKw);
    if (!IS_NONE(krk_currentThread.currentException))
        return NONE_VAL();
    return krk_pop();
...
KRK_Function(reversed) {
    /* FIXME The Python reversed() function produces an iterator and only works for things
     * with indexing or a __reversed__ method; Building a list and reversing it like we do
     * here is not correct! */
    if (argc != 1)
        return krk_runtimeError(vm.exceptions->argumentError,
                                "%s() takes %s %d argument%s (%d given)", "reversed",
                                "exactly", 1, "", argc);
    KrkValue listOut = krk_list_of(0, NULL, 0);
    krk_push(listOut);
    FUNC_NAME(list, extend)(2, (KrkValue[]){listOut, argv[0]}, 0);
...
_noexport void _createAndBind_listClass(void) {
    KrkClass *list =
        ADD_BASE_CLASS(vm.baseClasses->listClass, "list", vm.baseClasses->objectClass);
    ...
    KRK_DOC(BIND_METHOD(list, append),
            ...
            "@arguments item\n\n"
            "Adds an item to the end of a list. Appending items to a list is an "
            "amortized constant-time "
            "operation, but may result in the reallocation of the list if not enough "
            "additional space is "
    ...
    KRK_DOC(BIND_METHOD(list, extend),
            ...
            "@argument iterable\n\n"
            "Adds all of the elements of @p iterable to the end of the list, as if each "
            "were added individually "
            "with @ref _list_append.");
    KRK_DOC(BIND_METHOD(list, pop),
            "@brief Remove and return an element from the list.\n"
            "@arguments [index]\n\n"
            "Removes and returns the entry at the end of the list, or at @p index if "
            "provided. "
            "Popping from the end of the list is constant-time. Popping from the head of "
            "the list "
    ...
    KRK_DOC(BIND_METHOD(list, insert),
            ...
            "@arguments index, val\n\n"
            "Adds @p val to the list at offset @p index, moving all following items "
            "back. Inserting "
            "near the beginning of a list can be costly.");
    KRK_DOC(BIND_METHOD(list, clear), "@brief Empty a list.\n\n"
                                      "Removes all entries from the list.");
    KRK_DOC(BIND_METHOD(list, index), "@brief Locate an item in the list by value.\n"
                                      "@arguments val,[min,[max]]\n\n"
                                      "Searches for @p val in the list and returns its "
                                      "index if found. If @p min is provided, "
                                      "the search will begin at index @p min. If @p max "
                                      "is also provided, the search will end "
                                      "at index @p max.\n"
                                      "Raises @ref ValueError if the item is not found.");
    KRK_DOC(BIND_METHOD(list, count), "@brief Count instances of a value in the list.\n"
                                      "@arguments val\n\n"
                                      "Scans the list for values equal to @p val and "
    ...
    KRK_DOC(BIND_METHOD(list, reverse), "@brief Reverse the contents of a list.\n\n"
                                        "Reverses the elements of the list in-place.");
    KRK_DOC(BIND_METHOD(list, sort), "@brief Sort the contents of a list.\n\n"
                                     "Performs an in-place sort of the elements in the "
                                     "list, returning @c None as a gentle reminder "
                                     "that the sort is in-place. If a sorted copy is "
                                     "desired, use @ref sorted instead.");
    krk_defineNative(&list->methods, "__class_getitem__", krk_GenericAlias)->obj.flags |=
        KRK_OBJ_FLAGS_FUNCTION_IS_CLASS_METHOD;
    ...
    KrkClass *listiterator = ADD_BASE_CLASS(vm.baseClasses->listiteratorClass,
                                            "listiterator", vm.baseClasses->objectClass);
