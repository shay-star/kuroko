 ...
 * not guaranteed.
 */
__attribute__((tls_model("initial-exec"))) __thread KrkThreadState krk_currentThread;
#else
...
void krk_growStack(void) {
    ...
    if (krk_currentThread.flags & KRK_THREAD_DEFER_STACK_FREE) {
        ...
        krk_currentThread.stack =
            KRK_GROW_ARRAY(KrkValue, krk_currentThread.stack, old, newsize);
    }
...
inline void krk_push(KrkValue value) {
    if (unlikely(krk_currentThread.stackTop == krk_currentThread.stackMax))
        krk_growStack();
 ...
 */
KrkClass *krk_makeClass(KrkInstance *module, KrkClass **_class, const char *name,
                        KrkClass *base) {
 ...
 */
_nonnull void krk_finalizeClass(KrkClass *_class) {
    KrkValue tmp;
...
#undef SPECIAL_ATTRS
    ...
    for (struct TypeMap *entry = specials; entry->method; ++entry) {
        *entry->method = NULL;
        ...
        while (_base) {
            if (krk_tableGet_fast(&_base->methods,
                                  AS_STRING(vm.specialMethodNames[entry->index]), &tmp))
                break;
            _base = _base->base;
        }
        if (_base && (IS_CLOSURE(tmp) || IS_NATIVE(tmp)) &&
            (!(AS_OBJECT(tmp)->flags & KRK_OBJ_FLAGS_FUNCTION_IS_STATIC_METHOD) ||
             entry->index == METHOD_NEW)) {
    ...
    if (_class->base && _class->_eq != _class->base->_eq) {
        if (_class->_hash == _class->base->_hash) {
            _class->_hash = NULL;
            KrkValue v;
            if (!krk_tableGet_fast(&_class->methods,
                                   AS_STRING(vm.specialMethodNames[METHOD_HASH]), &v)) {
                krk_tableSet(&_class->methods, vm.specialMethodNames[METHOD_HASH],
                             NONE_VAL());
            }
        }
    }
...
inline KrkClass *krk_getType(KrkValue of) {
    ...
    case KRK_VAL_OBJECT:
        ...
        if (IS_CLASS(of) && AS_CLASS(of)->_class)
            return AS_CLASS(of)->_class;
...
static inline int checkArgumentCount(const KrkClosure *closure, int argCount) {
    ...
    if (unlikely(argCount < minArgs || argCount > maxArgs)) {
        krk_runtimeError(
            vm.exceptions->argumentError, "%s() takes %s %d %sargument%s (%d given)",
            closure->function->name ? closure->function->name->chars : "<unnamed>",
            (minArgs == maxArgs) ? "exactly"
                                 : (argCount < minArgs ? "at least" : "at most"),
            (argCount < minArgs) ? minArgs : maxArgs,
            closure->function->keywordArgs ? "positional " : "",
            ((argCount < minArgs) ? minArgs : maxArgs) == 1 ? "" : "s", argCount);
        return 0;
    }
    return 1;
}
...
static void multipleDefs(const KrkClosure *closure, int destination) {
    krk_runtimeError(
        vm.exceptions->typeError, "%s() got multiple values for argument '%S'",
        closure->function->name ? closure->function->name->chars : "<unnamed>",
        (destination < closure->function->potentialPositionals
             ? AS_STRING(closure->function->positionalArgNames.values[destination])
             : (destination - closure->function->potentialPositionals <
                        closure->function->keywordArgs
                    ? AS_STRING(closure->function->keywordArgNames
                                    .values[destination -
                                            closure->function->potentialPositionals])
                    : S("<unnamed>"))));
}
...
static int _unpack_op(void *context, const KrkValue *values, size_t count) {
    ...
    if (unlikely(output->values.count + count > output->values.capacity)) {
        krk_runtimeError(vm.exceptions->valueError,
...
static int _unpack_args(void *context, const KrkValue *values, size_t count) {
    ...
    if (positionals->count + count > positionals->capacity) {
        ...
        positionals->capacity =
            (count == 1) ? KRK_GROW_CAPACITY(old) : (positionals->count + count);
        positionals->values =
            KRK_GROW_ARRAY(KrkValue, positionals->values, old, positionals->capacity);
    }
...
int krk_processComplexArguments(int argCount, KrkValueArray *positionals,
                                KrkTable *keywords, const char *name) {
#define TOP_ARGS 3
    ...
    for (size_t i = 0; i < existingPositionalArgs; ++i) {
        krk_writeValueArray(positionals,
                            krk_currentThread.stackTop[-argCount + i - TOP_ARGS]);
    }

    size_t startOfExtras = &krk_currentThread.stackTop[-kwargsCount * 2 - TOP_ARGS] -
                           krk_currentThread.stack;
    ...
    for (size_t i = 0; i < kwargsCount; ++i) {
        ...
        if (IS_KWARGS(key)) {
            if (AS_INTEGER(key) == KWARGS_LIST) { /* unpack list */
                if (krk_unpackIterable(value, positionals, _unpack_args))
                    return 0;
            } else if (AS_INTEGER(key) == KWARGS_DICT) { /* unpack dict */
                if (!IS_dict(value)) {
                    krk_runtimeError(vm.exceptions->typeError,
                                     "%s(): **expression value is not a dict.", name);
                    return 0;
                }
                for (size_t i = 0; i < AS_DICT(value)->used; ++i) {
                    ...
                    if (!IS_KWARGS(entry->key)) {
                        if (!IS_STRING(entry->key)) {
                            krk_runtimeError(vm.exceptions->typeError,
                                             "%s(): **expression contains non-string key",
                                             name);
                            return 0;
                        }
            ...
            if (!krk_tableSet(keywords, key, value)) {
                krk_runtimeError(vm.exceptions->typeError,
                                 "%s() got multiple values for argument '%S'", name,
                                 AS_STRING(key));
                return 0;
            }
        }
...
static inline int _callManaged(KrkClosure *closure, int argCount, int returnDepth) {
    ...
    if (argCount && unlikely(IS_KWARGS(krk_currentThread.stackTop[-1]))) {
        ...
        /* This processes the existing argument list into a ValueArray and a Table with
         * the args and keywords */
        if (!krk_processComplexArguments(
                argCount, positionals, keywords,
                closure->function->name ? closure->function->name->chars : "<unnamed>"))
            return 0;
        ...
        /* Do we already know we have too many arguments? Let's bail before doing a bunch
         * of work. */
        if ((positionals->count > potentialPositionalArgs) &&
            !(closure->function->obj.flags & KRK_OBJ_FLAGS_CODEOBJECT_COLLECTS_ARGS)) {
        ...
        /* Did we have way more arguments than we needed? Put the stack where it should
         * be. */
        while ((size_t)argCount > potentialPositionalArgs) {
            krk_pop();
            argCount--;
        ...
        if (closure->function->obj.flags & KRK_OBJ_FLAGS_CODEOBJECT_COLLECTS_ARGS) {
            size_t count = (positionals->count > potentialPositionalArgs)
                               ? (positionals->count - potentialPositionalArgs)
                               : 0;
            KrkValue *offset =
                (count == 0) ? NULL : &positionals->values[potentialPositionalArgs];
        ...
        for (size_t i = 0; i < keywords->used; ++i) {
            ...
            if (!IS_KWARGS(entry->key)) {
                ...
                for (int j = 0; j < (int)closure->function->potentialPositionals; ++j) {
                    if (krk_valuesSame(name,
                                       closure->function->positionalArgNames.values[j])) {
                ...
                for (int j = 0; j < (int)closure->function->keywordArgs; ++j) {
                    if (krk_valuesSame(name,
                                       closure->function->keywordArgNames.values[j])) {
                        if (!IS_KWARGS(
                                krk_currentThread.stackTop
                                    [-argCount + j +
                                     closure->function->potentialPositionals +
                                     !!(closure->function->obj.flags &
                                        KRK_OBJ_FLAGS_CODEOBJECT_COLLECTS_ARGS)])) {
                            multipleDefs(closure,
                                         j + closure->function->potentialPositionals);
                            goto _errorAfterPositionals;
                        }
                        krk_currentThread
                            .stackTop[-argCount + j +
                                      closure->function->potentialPositionals +
                                      !!(closure->function->obj.flags &
                                         KRK_OBJ_FLAGS_CODEOBJECT_COLLECTS_ARGS)] = value;
                        goto _finishKwarg;
                    }
                }
                if (!(closure->function->obj.flags &
                      KRK_OBJ_FLAGS_CODEOBJECT_COLLECTS_KWS)) {
                    krk_runtimeError(vm.exceptions->typeError,
        ...
        argCountX =
            argCount - closure->function->keywordArgs -
            (!!(closure->function->obj.flags & KRK_OBJ_FLAGS_CODEOBJECT_COLLECTS_ARGS) +
             !!(closure->function->obj.flags & KRK_OBJ_FLAGS_CODEOBJECT_COLLECTS_KWS));
    } else if ((size_t)argCount > potentialPositionalArgs &&
               (closure->function->obj.flags & KRK_OBJ_FLAGS_CODEOBJECT_COLLECTS_ARGS)) {
        ...
        while (krk_currentThread.stackTop > startOfPositionals + argCount)
            krk_pop();
    }
    ...
    if (unlikely(closure->function->obj.flags &
                 (KRK_OBJ_FLAGS_CODEOBJECT_IS_GENERATOR |
                  KRK_OBJ_FLAGS_CODEOBJECT_IS_COROUTINE))) {
        KrkInstance *gen =
            krk_buildGenerator(closure, krk_currentThread.stackTop - argCount, argCount);
        krk_currentThread.stackTop = krk_currentThread.stackTop - argCount - returnDepth;
        krk_push(OBJECT_VAL(gen));
        return 2;
    }

    if (unlikely(krk_currentThread.frameCount ==
                 (size_t)krk_currentThread.maximumCallDepth)) {
 ...
 */
inline KrkValue krk_callNativeOnStack(size_t argCount, const KrkValue *stackArgs,
...
static void _rotate(size_t argCount) {
    krk_push(NONE_VAL());
    memmove(&krk_currentThread.stackTop[-argCount],
            &krk_currentThread.stackTop[-argCount - 1], sizeof(KrkValue) * argCount);
}

static inline int _callNative(KrkNative *callee, int argCount, int returnDepth) {
    NativeFn native = (NativeFn)callee->function;
    size_t stackOffsetAfterCall =
        (krk_currentThread.stackTop - krk_currentThread.stack) - argCount - returnDepth;
    KrkValue result;
    if (unlikely(argCount && IS_KWARGS(krk_currentThread.stackTop[-1]))) {
        ...
        /* Parse kwargs stuff into the list+dict; note, this no longer pops anything, and
         * expects our list + dict to be at the top: [kwargs] [list] [dict] */
        if (unlikely(!krk_processComplexArguments(argCount, AS_LIST(myList),
                                                  AS_DICT(myDict), callee->name)))
            return 0;

        /* Write the dict into the list */
        krk_writeValueArray(AS_LIST(myList), myDict);

        /* Also add a list for storing references that get removed from the kwargs dict
         * during mutation. */
        ...
        result = krk_callNativeOnStack(argCount, krk_currentThread.stackTop - argCount, 0,
                                       native);
    }
...
int krk_callValue(KrkValue callee, int argCount, int returnDepth) {
    ...
    _innerObject:
        ...
        case KRK_OBJ_CLOSURE:
            return _callManaged(AS_CLOSURE(callee), argCount, returnDepth);
        case KRK_OBJ_NATIVE:
            return _callNative(AS_NATIVE(callee), argCount, returnDepth);
        ...
        default: {
            ...
            if (likely(_class->_call != NULL)) {
                ...
                argCount++;
                returnDepth = returnDepth ? (returnDepth - 1) : 0;
                return (_class->_call->type == KRK_OBJ_CLOSURE)
                           ? _callManaged((KrkClosure *)_class->_call, argCount,
                                          returnDepth)
                           : _callNative((KrkNative *)_class->_call, argCount,
                                         returnDepth);
            } else {
                krk_runtimeError(vm.exceptions->typeError, "'%T' object is not callable",
                                 callee);
                return 0;
            }
        }
...
KrkValue krk_callDirect(KrkObj *callable, int argCount) {
    ...
    case KRK_OBJ_CLOSURE:
        result = _callManaged((KrkClosure *)callable, argCount, 0);
        break;
    case KRK_OBJ_NATIVE:
        result = _callNative((KrkNative *)callable, argCount, 0);
        break;
    default:
...
#define UPVALUE_LOCATION(upvalue)                                                        \
    (upvalue->location == -1 ? &upvalue->closed                                          \
                             : &upvalue->owner->stack[upvalue->location])
...
static void closeUpvalues(int last) {
    while (krk_currentThread.openUpvalues != NULL &&
           krk_currentThread.openUpvalues->location >= last) {
        KrkUpvalue *upvalue = krk_currentThread.openUpvalues;
...
int krk_isFalsey(KrkValue value) {
    ...
    case KRK_VAL_OBJECT: {
        ...
        case KRK_OBJ_STRING:
            return !AS_STRING(value)->codesLength;
        case KRK_OBJ_TUPLE:
            return !AS_TUPLE(value)->values.count;
        default:
            break;
        }
        break;
    ...
    if (type->_bool) {
        ...
        if (!IS_BOOLEAN(result)) {
            krk_runtimeError(vm.exceptions->typeError,
                             "__bool__ should return bool, not %T", result);
            return 1;
        }
...
void krk_setMaximumRecursionDepth(size_t maxDepth) {
    ...
    krk_currentThread.frames =
        realloc(krk_currentThread.frames, maxDepth * sizeof(KrkCallFrame));
}
...
void krk_initVM(int flags) {
    ...
    krk_currentThread.frames =
        calloc(krk_currentThread.maximumCallDepth, sizeof(KrkCallFrame));
...
#undef SPECIAL_ATTRS
    ...
    for (size_t i = 0; i < METHOD__MAX; ++i) {
        vm.specialMethodNames[i] =
            OBJECT_VAL(krk_copyString(_methods[i].s, _methods[i].len));
    }
 ...
 */
const char *krk_typeName(KrkValue value) { return krk_getType(value)->name->chars; }
...
static KrkValue _bin_op(size_t methodOffset, size_t invOffset, const char *operator,
                        KrkValue a, KrkValue b) {
    KrkValue result;
    if (_try_op(methodOffset, a, b, &result))
        return result;
    if (_try_op(invOffset, b, a, &result))
        return result;
    return krk_runtimeError(vm.exceptions->typeError,
                            "unsupported operand types for %s: '%T' and '%T'", operator,
                            a, b);
}

#define MAKE_COMPARE_OP(name, operator, inv)                                             \
    _protected KrkValue krk_operator_##name(KrkValue a, KrkValue b) {                    \
        return _bin_op(offsetof(KrkClass, _##name),                                      \
                       offsetof(KrkClass, _##inv), operator, a, b);                      \
    }
#define MAKE_BIN_OP(name, operator, inv)                                                 \
    ...
    _protected KrkValue krk_operator_i##name(KrkValue a, KrkValue b) {                   \
        KrkValue result;                                                                 \
        if (_try_op(offsetof(KrkClass, _i##name), a, b, &result))                        \
            return result;                                                               \
...
_protected KrkValue krk_operator_eq(KrkValue a, KrkValue b) {
    return BOOLEAN_VAL(krk_valuesEqual(a, b));
}

_protected KrkValue krk_operator_is(KrkValue a, KrkValue b) {
...
static KrkValue _unary_op(size_t methodOffset, const char *operator, KrkValue value) {
    ...
    if (krk_currentThread.flags & KRK_THREAD_HAS_EXCEPTION)
        return NONE_VAL();
    return krk_runtimeError(vm.exceptions->typeError,
                            "bad operand type for unary %s: '%T'", operator, value);
}
...
static int handleException(void) {
    ...
    int exitSlot = (krk_currentThread.exitOnFrame >= 0)
                       ? krk_currentThread.frames[krk_currentThread.exitOnFrame].outSlots
                       : 0;
    ...
    if (stackOffset < exitSlot) {
        if (exitSlot == 0) {
             ...
             */
            if (!(vm.globalFlags & KRK_GLOBAL_CLEAN_OUTPUT))
                krk_dumpTraceback();
        }
    ...
    for (frameOffset = krk_currentThread.frameCount - 1;
         frameOffset >= 0 &&
         (int)krk_currentThread.frames[frameOffset].slots > stackOffset;
         frameOffset--)
        ;
    if (frameOffset == -1) {
        abort();
...
int krk_loadModule(KrkString *path, KrkValue *moduleOut, KrkString *runAs,
    ...
    if (!vm.system ||
        !krk_tableGet_fast(&vm.system->fields, S("module_paths"), &modulePaths)) {
        *moduleOut = NONE_VAL();
    ...
    if (!IS_list(modulePaths)) {
        *moduleOut = NONE_VAL();
        krk_runtimeError(vm.exceptions->importError,
                         "kuroko.module_paths must be a list, not '%T'", modulePaths);
        return 0;
    }
        ...
        if (stat(fileName, &statbuf) == 0) {
            ...
            if (runAs == S("__main__")) {
                ...
                if (!krk_importModule(AS_STRING(dotted_main), runAs)) {
                    krk_runtimeError(vm.exceptions->importError,
                                     "No module named '%S'; '%S' is a package and cannot "
...
#    endif
    ...
    _normalFile:
        ...
        if (isPackage) {
            krk_attachNamedValue(&krk_currentThread.module->fields, "__ispackage__",
                                 BOOLEAN_VAL(1));
            /* For a module that is a package, __package__ is its own name */
            krk_attachNamedValue(&krk_currentThread.module->fields, "__package__",
                                 OBJECT_VAL(runAs));
        } else {
            KrkValue parentName;
            if (IS_INSTANCE(parent) &&
                krk_tableGet_fast(&AS_INSTANCE(parent)->fields, S("__name__"),
                                  &parentName) &&
                IS_STRING(parentName)) {
                krk_attachNamedValue(&krk_currentThread.module->fields, "__package__",
                                     parentName);
            } else {
                /* If there is no parent, or the parent doesn't have a string __name__
                 * attribute, set the __package__ to None, so it at least exists. */
                krk_attachNamedValue(&krk_currentThread.module->fields, "__package__",
                                     NONE_VAL());
            }
        }
...
#    ifndef KRK_STATIC_ONLY
    _sharedObject:
        ...
        if (!dlRef) {
            *moduleOut = NONE_VAL();
            krk_runtimeError(vm.exceptions->importError,
                             "Failed to load native module '%S' from shared object '%s'",
                             runAs, fileName);
            return 0;
        }
        ...
        *moduleOut = moduleOnLoad(runAs);
        ...
        krk_attachNamedObject(&AS_INSTANCE(*moduleOut)->fields, "__name__",
                              (KrkObj *)runAs);
...
#    endif
    ...
    *moduleOut = NONE_VAL();
    ...
    krk_runtimeError(vm.exceptions->importError, "No module named '%S'",
...
int krk_importModule(KrkString *name, KrkString *runAs) {
    ...
    if (name->chars[0] == '.') {
        /**
         * For relative imports, we canonicalize the import name based on the current
         * package, and then trying importModule again with the fully qualified name.
         */

        KrkValue packageName;
        if (!krk_tableGet_fast(&krk_currentThread.module->fields, S("__package__"),
                               &packageName) ||
            !IS_STRING(packageName)) {
            /* We must have __package__ set to a string for this to make any sense. */
            krk_runtimeError(vm.exceptions->importError,
                             "attempted relative import without a package context");
            return 0;
        }
        ...
        krk_push(OBJECT_VAL(S(".")));
        KrkValue components =
            krk_string_split(2, (KrkValue[]){krk_peek(1), krk_peek(0)}, 0);
        if (!IS_list(components)) {
            krk_runtimeError(vm.exceptions->importError,
                             "internal error while calculating package path");
            return 0;
        }
        krk_push(components);
        krk_swap(2);
    ...
    do {
        KrkValue listOut = krk_string_split(
            3,
            (KrkValue[]){krk_currentThread.stack[argBase + 3],
                         krk_currentThread.stack[argBase + 4], INTEGER_VAL(1)},
            0);
        if (!IS_INSTANCE(listOut))
            return 0;
        ...
        if (IS_NONE(krk_currentThread.stack[argBase + 3])) {
            ...
            krk_pop(); /* remainder */
            KrkValue current;
            if (!krk_loadModule(AS_STRING(krk_currentThread.stack[argBase + 1]), &current,
                                runAs, krk_currentThread.stack[argBase - 1]))
                return 0;
            krk_pop(); /* dot-sepaerated */
            krk_pop(); /* slash-separated */
            krk_push(current);
            /* last must be something if we got here, because single-level import happens
             * elsewhere */
            krk_tableSet(&AS_INSTANCE(krk_currentThread.stack[argBase - 1])->fields,
                         krk_currentThread.stack[argBase + 0], krk_peek(0));
            ...
            KrkValue current;
            if (!krk_loadModule(AS_STRING(krk_currentThread.stack[argBase + 1]), &current,
                                AS_STRING(krk_currentThread.stack[argBase + 2]),
                                NONE_VAL()))
                return 0;
            krk_push(current);
            ...
            /* Is this a package? */
            KrkValue tmp;
            if (!krk_tableGet_fast(&AS_INSTANCE(current)->fields, S("__ispackage__"),
                                   &tmp) ||
                !IS_BOOLEAN(tmp) || AS_BOOLEAN(tmp) != 1) {
                krk_runtimeError(vm.exceptions->importError, "'%S' is not a package",
                                 AS_STRING(krk_currentThread.stack[argBase + 2]));
                return 0;
            }
            krk_currentThread.stack[argBase - 1] = krk_pop();
...
int krk_doRecursiveModuleLoad(KrkString *name) { return krk_importModule(name, name); }
...
int krk_bindMethodSuper(KrkClass *originalClass, KrkString *name, KrkClass *realClass) {
    ...
    if (IS_NATIVE(method) || IS_CLOSURE(method)) {
        if (AS_OBJECT(method)->flags & KRK_OBJ_FLAGS_FUNCTION_IS_CLASS_METHOD) {
            out =
                OBJECT_VAL(krk_newBoundMethod(OBJECT_VAL(realClass), AS_OBJECT(method)));
        } else if (IS_NONE(krk_peek(0)) ||
                   (AS_OBJECT(method)->flags & KRK_OBJ_FLAGS_FUNCTION_IS_STATIC_METHOD)) {
            out = method;
        } else {
...
static int valueGetMethod(KrkString *name) {
    ...
    /* Fields */
    if (IS_INSTANCE(this)) {
        if (krk_tableGet_fast(&AS_INSTANCE(this)->fields, name, &value))
            goto found;
        ...
        do {
            if (krk_tableGet_fast(&type->methods, name, &value)) {
                if ((IS_NATIVE(value) || IS_CLOSURE(value)) &&
                    (AS_OBJECT(value)->flags & KRK_OBJ_FLAGS_FUNCTION_IS_CLASS_METHOD)) {
                    goto found_method;
                }
        ...
        if (krk_tableGet_fast(&AS_CLOSURE(this)->fields, name, &value))
            goto found;
    }
    ...
    if (_class) {
        if (IS_NATIVE(method) || IS_CLOSURE(method)) {
            if (AS_OBJECT(method)->flags & KRK_OBJ_FLAGS_FUNCTION_IS_CLASS_METHOD) {
                ...
                value = method;
                goto found_method;
            } else if (AS_OBJECT(method)->flags &
                       KRK_OBJ_FLAGS_FUNCTION_IS_STATIC_METHOD) {
                value = method;
            } else {
                value = method;
                goto found_method;
...
static int valueGetProperty(KrkString *name) {
    ...
    case 2:
        ...
        return 1;
    case 1: {
        KrkValue o = OBJECT_VAL(krk_newBoundMethod(
            krk_currentThread.stackTop[-2], AS_OBJECT(krk_currentThread.stackTop[-1])));
...
int krk_getAttribute(KrkString *name) { return valueGetProperty(name); }

KrkValue krk_valueGetAttribute(KrkValue value, char *name) {
    krk_push(OBJECT_VAL(krk_copyString(name, strlen(name))));
    krk_push(value);
    if (!valueGetProperty(AS_STRING(krk_peek(1)))) {
        return krk_runtimeError(vm.exceptions->attributeError,
                                "'%T' object has no attribute '%s'", krk_peek(0), name);
    }
    krk_swap(1);
    krk_pop(); /* String */
    return krk_pop();
...
int krk_delAttribute(KrkString *name) { return valueDelProperty(name); }
...
static int _setDescriptor(KrkValue owner, KrkClass *_class, KrkString *name,
                          KrkValue to) {
    KrkValue property;
...
static KrkValue setAttr_wrapper(KrkValue owner, KrkClass *_class, KrkTable *fields,
                                KrkString *name, KrkValue to) {
    if (_setDescriptor(owner, _class, name, to))
        return krk_pop();
    krk_tableSet(fields, OBJECT_VAL(name), to);
    return to;
}

_noexport KrkValue krk_instanceSetAttribute_wrapper(KrkValue owner, KrkString *name,
                                                    KrkValue to) {
    return setAttr_wrapper(owner, AS_INSTANCE(owner)->_class, &AS_INSTANCE(owner)->fields,
                           name, to);
}
...
static int valueSetProperty(KrkString *name) {
    ...
    if (IS_INSTANCE(owner)) {
        KrkValue o =
            setAttr_wrapper(owner, type, &AS_INSTANCE(owner)->fields, name, value);
        ...
        KrkValue o =
            setAttr_wrapper(owner, type, &AS_CLOSURE(owner)->fields, name, value);
        krk_currentThread.stackTop[-1] = o;
    } else {
...
int krk_setAttribute(KrkString *name) { return valueSetProperty(name); }
...
KrkValue krk_valueSetAttribute(KrkValue owner, char *name, KrkValue to) {
    ...
    if (!valueSetProperty(AS_STRING(krk_peek(2)))) {
        return krk_runtimeError(vm.exceptions->attributeError,
                                "'%T' object has no attribute '%s'", krk_peek(1), name);
    }
    krk_swap(1);
    krk_pop(); /* String */
    return krk_pop();
...
#define BINARY_OP(op)                                                                    \
    {                                                                                    \
        KrkValue b = krk_peek(0);                                                        \
        KrkValue a = krk_peek(1);                                                        \
        a = krk_operator_##op(a, b);                                                     \
        krk_currentThread.stackTop[-2] = a;                                              \
        krk_pop();                                                                       \
        break;                                                                           \
    }
#define INPLACE_BINARY_OP(op)                                                            \
    {                                                                                    \
        KrkValue b = krk_peek(0);                                                        \
        KrkValue a = krk_peek(1);                                                        \
...
#define LIKELY_INT_BINARY_OP(op)                                                         \
    {                                                                                    \
        KrkValue b = krk_peek(0);                                                        \
        KrkValue a = krk_peek(1);                                                        \
        if (likely(IS_INTEGER(a) && IS_INTEGER(b)))                                      \
            a = krk_int_op_##op(AS_INTEGER(a), AS_INTEGER(b));                           \
        else                                                                             \
            a = krk_operator_##op(a, b);                                                 \
        krk_currentThread.stackTop[-2] = a;                                              \
        krk_pop();                                                                       \
...
#define LIKELY_INT_COMPARE_OP(op, operator)                                              \
    {                                                                                    \
        KrkValue b = krk_peek(0);                                                        \
        KrkValue a = krk_peek(1);                                                        \
        if (likely(IS_INTEGER(a) && IS_INTEGER(b)))                                      \
            a = BOOLEAN_VAL(AS_INTEGER(a) operator AS_INTEGER(b));                       \
        else                                                                             \
            a = krk_operator_##op(a, b);                                                 \
        krk_currentThread.stackTop[-2] = a;                                              \
        krk_pop();                                                                       \
...
#define LIKELY_INT_UNARY_OP(op, operator)                                                \
    {                                                                                    \
        KrkValue a = krk_peek(0);                                                        \
        if (likely(IS_INTEGER(a)))                                                       \
            a = INTEGER_VAL(operator AS_INTEGER(a));                                     \
        else                                                                             \
            a = krk_operator_##op(a);                                                    \
        krk_currentThread.stackTop[-1] = a;                                              \
        break;                                                                           \
    }
...
static inline void makeCollection(NativeFn func, size_t count) {
    KrkValue collection =
        krk_callNativeOnStack(count, &krk_currentThread.stackTop[-count], 0, func);
    if (count) {
...
static inline int doFormatString(int options) {
    ...
    if (options & FORMAT_OP_STR) {
        ...
        if (likely(type->_tostr != NULL)) {
            ...
            krk_runtimeError(vm.exceptions->typeError, "Can not convert '%T' to str",
                             krk_peek(0));
            return 1;
        }
        ...
        if (likely(type->_reprer != NULL)) {
            ...
            krk_runtimeError(vm.exceptions->typeError, "Can not repr '%T'", krk_peek(0));
            return 1;
        }
    }
    ...
    if (likely(type->_format != NULL)) {
        ...
        krk_runtimeError(vm.exceptions->attributeError,
                         "'%T' object has no attribute '%s'", krk_peek(1), "__format__");
        return 1;
    }
...
static inline void commonMethodInvoke(size_t methodOffset, int args,
...
static KrkValue run(void) {
    ...
    while (1) {
        if (unlikely(krk_currentThread.flags &
                     (KRK_THREAD_ENABLE_TRACING | KRK_THREAD_SINGLE_STEP |
                      KRK_THREAD_SIGNALLED))) {
            ...
            if (krk_currentThread.flags & KRK_THREAD_SIGNALLED) {
                krk_currentThread.flags &=
                    ~(KRK_THREAD_SIGNALLED); /* Clear signal flag */
...
/* Only GCC lets us put these on empty statements; just hope clang doesn't start
 * complaining */
...
#define TWO_BYTE_OPERAND                                                                 \
    {                                                                                    \
        OPERAND = OPERAND | (frame->ip[0] << 8) | frame->ip[1];                          \
        frame->ip += 2;                                                                  \
    }
#define THREE_BYTE_OPERAND                                                               \
    {                                                                                    \
        OPERAND = (frame->ip[0] << 16) | (frame->ip[1] << 8);                            \
        frame->ip += 2;                                                                  \
    }                                                                                    \
    FALLTHROUGH
#define ONE_BYTE_OPERAND                                                                 \
    { OPERAND = (OPERAND & ~0xFF) | READ_BYTE(); }

    _switchEntry:
        (void)0;
        switch (opcode) {
        case OP_CLEANUP_WITH: {
            /* Top of stack is a HANDLER that should have had something loaded into it if
             * it was still valid */
            ...
            if (AS_HANDLER_TYPE(handler) == OP_RAISE) {
                krk_currentThread.stackTop[-2] =
                    HANDLER_VAL(OP_CLEANUP_WITH, AS_HANDLER_TARGET(krk_peek(1)));
                ...
                if (IS_INSTANCE(exceptionObject))
                    krk_tableGet_fast(&AS_INSTANCE(exceptionObject)->fields,
                                      S("traceback"), &tracebackEntries);
                ...
                if (!(krk_currentThread.flags & KRK_THREAD_HAS_EXCEPTION)) {
                    ...
                    krk_currentThread.currentException =
                        krk_pop(); /* Original exception */
                    krk_currentThread.flags |= KRK_THREAD_HAS_EXCEPTION;
                }
                goto _finishException;
            } else {
            ...
            if (AS_HANDLER_TYPE(handler) == OP_EXIT_LOOP) {
                frame->ip =
                    frame->closure->function->chunk.code + AS_HANDLER_TARGET(handler);
                OPERAND = AS_INTEGER(krk_peek(1));
                goto _finishPopBlock;
            }
            if (AS_HANDLER_TYPE(handler) != OP_RETURN)
                break;
            krk_pop(); /* handler */
        }              /* fallthrough */
        case OP_RETURN: {
        _finishReturn:
            ...
            int stackOffset;
            for (stackOffset =
                     (int)(krk_currentThread.stackTop - krk_currentThread.stack - 1);
            ...
            if (stackOffset >= (int)frame->slots) {
                ...
                krk_currentThread.stackTop = &krk_currentThread.stack[stackOffset + 1];
                frame->ip =
                    frame->closure->function->chunk.code + AS_HANDLER_TARGET(krk_peek(0));
                krk_currentThread.stackTop[-1] =
                    HANDLER_VAL(OP_RETURN, AS_HANDLER_TARGET(krk_peek(0)));
            ...
            if (krk_currentThread.frameCount == (size_t)krk_currentThread.exitOnFrame) {
                if (frame->closure->function->obj.flags &
                    (KRK_OBJ_FLAGS_CODEOBJECT_IS_GENERATOR |
                     KRK_OBJ_FLAGS_CODEOBJECT_IS_COROUTINE)) {
                    krk_push(result);
                    return KWARGS_VAL(0);
        ...
        case OP_LESS:
            LIKELY_INT_COMPARE_OP(lt, <)
        case OP_GREATER:
            LIKELY_INT_COMPARE_OP(gt, >)
        case OP_LESS_EQUAL:
            LIKELY_INT_COMPARE_OP(le, <=)
        case OP_GREATER_EQUAL:
            LIKELY_INT_COMPARE_OP(ge, >=)
        case OP_ADD:
            LIKELY_INT_BINARY_OP(add)
        case OP_SUBTRACT:
            LIKELY_INT_BINARY_OP(sub)
        case OP_MULTIPLY:
            BINARY_OP(mul)
        case OP_DIVIDE:
            BINARY_OP(truediv)
        ...
        case OP_IS:
            BINARY_OP(is);
        case OP_BITNEGATE:
            LIKELY_INT_UNARY_OP(invert, ~)
        case OP_NEGATE:
            LIKELY_INT_UNARY_OP(neg, -)
        case OP_POS:
            LIKELY_INT_UNARY_OP(pos, +)
        case OP_NONE:
            krk_push(NONE_VAL());
            break;
        case OP_TRUE:
            krk_push(BOOLEAN_VAL(1));
            break;
        case OP_FALSE:
            krk_push(BOOLEAN_VAL(0));
            break;
        case OP_UNSET:
            krk_push(KWARGS_VAL(0));
            break;
        case OP_NOT:
            krk_push(BOOLEAN_VAL(krk_isFalsey(krk_peek(0)))); /* fallthrough */
        case OP_SWAP_POP:
            krk_swap(1); /* fallthrough */
        case OP_POP:
            krk_pop();
            break;

        case OP_INPLACE_ADD:
            INPLACE_BINARY_OP(add)
        case OP_INPLACE_SUBTRACT:
            INPLACE_BINARY_OP(sub)
        case OP_INPLACE_MULTIPLY:
            INPLACE_BINARY_OP(mul)
        case OP_INPLACE_DIVIDE:
            INPLACE_BINARY_OP(truediv)
        case OP_INPLACE_FLOORDIV:
            INPLACE_BINARY_OP(floordiv)
        case OP_INPLACE_MODULO:
            INPLACE_BINARY_OP(mod)
        case OP_INPLACE_BITOR:
            INPLACE_BINARY_OP(or)
        case OP_INPLACE_BITXOR:
            INPLACE_BINARY_OP(xor)
        case OP_INPLACE_BITAND:
            INPLACE_BINARY_OP(and)
        case OP_INPLACE_SHIFTLEFT:
            INPLACE_BINARY_OP(lshift)
        case OP_INPLACE_SHIFTRIGHT:
            INPLACE_BINARY_OP(rshift)
        case OP_INPLACE_POW:
            INPLACE_BINARY_OP(pow)
        case OP_INPLACE_MATMUL:
            INPLACE_BINARY_OP(matmul)
        ...
        case OP_INVOKE_GETTER: {
            commonMethodInvoke(offsetof(KrkClass, _getter), 2,
                               "'%T' object is not subscriptable");
            break;
        }
        case OP_INVOKE_SETTER: {
            commonMethodInvoke(offsetof(KrkClass, _setter), 3,
                               "'%T' object doesn't support item assignment");
            break;
        }
        case OP_INVOKE_DELETE: {
            commonMethodInvoke(offsetof(KrkClass, _delitem), 2,
                               "'%T' object doesn't support item deletion");
            krk_pop(); /* unused result */
            break;
        }
        case OP_INVOKE_ITER: {
            commonMethodInvoke(offsetof(KrkClass, _iter), 1,
                               "'%T' object is not iterable");
            break;
        }
        case OP_INVOKE_CONTAINS: {
            krk_swap(1); /* operands are backwards */
            commonMethodInvoke(offsetof(KrkClass, _contains), 2,
                               "'%T' object can not be tested for membership");
            break;
        }
        ...
        case OP_TRY_ELSE: {
            if (IS_HANDLER(krk_peek(0))) {
                krk_currentThread.stackTop[-1] =
                    HANDLER_VAL(OP_FILTER_EXCEPT, AS_HANDLER_TARGET(krk_peek(0)));
            }
            break;
        }
        case OP_BEGIN_FINALLY: {
            if (IS_HANDLER(krk_peek(0))) {
                ...
                /* We either entered the @c finally without an exception, or the exception
                 * was handled by an @c except */
                case OP_PUSH_TRY:
                case OP_FILTER_EXCEPT:
                    krk_currentThread.stackTop[-1] =
                        HANDLER_VAL(OP_BEGIN_FINALLY, AS_HANDLER_TARGET(krk_peek(0)));
                    break;
                /* We entered the @c finally without handling an exception. */
                case OP_RAISE:
                    krk_currentThread.stackTop[-1] =
                        HANDLER_VAL(OP_END_FINALLY, AS_HANDLER_TARGET(krk_peek(0)));
                    break;
                }
            }
            break;
        ...
        case OP_END_FINALLY: {
            ...
            if (IS_HANDLER(handler)) {
                if (AS_HANDLER_TYPE(handler) == OP_RAISE ||
                    AS_HANDLER_TYPE(handler) == OP_END_FINALLY) {
                    krk_pop(); /* handler */
        ...
        case OP_ANNOTATE: {
            if (IS_CLOSURE(krk_peek(0))) {
                ...
                krk_runtimeError(vm.exceptions->typeError, "Can not annotate '%T'.",
                                 krk_peek(0));
                goto _finishException;
            }
            break;
        ...
        case OP_DICT_SET_TOP: {
            ...
            FUNC_NAME(dict, __setitem__)
            (3, (KrkValue[]){dict, krk_peek(1), krk_peek(0)}, 0);
            krk_pop();
            krk_pop();
            break;
        }
        ...
        case OP_TUPLE_FROM_LIST: {
            ...
            krk_push(tuple);
            for (size_t i = 0; i < count; ++i) {
                AS_TUPLE(tuple)->values.values[AS_TUPLE(tuple)->values.count++] =
                    AS_LIST(list)->values[i];
            }
            krk_swap(1);
            krk_pop();
            break;
        ...
        case OP_OVERLONG_JUMP: {
            /* Overlong jumps replace 2-byte operand jump instructions with a zero-operand
             * instruction that slowly scans through a dumb table to find the intended
             * jump target and opcode. */
            for (size_t i = 0; i < frame->closure->function->overlongJumpsCount; ++i) {
                if (frame->closure->function->overlongJumps[i].instructionOffset ==
                    (size_t)((char *)frame->ip -
                             (char *)frame->closure->function->chunk.code)) {
                    OPERAND =
                        (int)frame->closure->function->overlongJumps[i].intendedTarget
                        << 16;
        ...
        case OP_PUSH_BUILD_CLASS: {
            ...
            krk_tableGet_fast(&vm.builtins->fields,
                              AS_STRING(vm.specialMethodNames[METHOD_BLDCLS]),
                              &build_class);
        ...
        case OP_JUMP_IF_FALSE_OR_POP: {
            TWO_BYTE_OPERAND;
            if (krk_valuesSame(krk_peek(0), BOOLEAN_VAL(0)) || krk_isFalsey(krk_peek(0)))
                frame->ip += OPERAND;
            else
                krk_pop();
            break;
        ...
        case OP_PUSH_TRY: {
            TWO_BYTE_OPERAND;
            uint16_t tryTarget =
                OPERAND + (frame->ip - frame->closure->function->chunk.code);
            krk_push(NONE_VAL());
        ...
        case OP_PUSH_WITH: {
            TWO_BYTE_OPERAND;
            uint16_t cleanupTarget =
                OPERAND + (frame->ip - frame->closure->function->chunk.code);
            KrkValue contextManager = krk_peek(0);
            KrkClass *type = krk_getType(contextManager);
            if (unlikely(!type->_enter || !type->_exit)) {
                if (!type->_enter)
                    krk_runtimeError(vm.exceptions->attributeError, "__enter__");
                else if (!type->_exit)
                    krk_runtimeError(vm.exceptions->attributeError, "__exit__");
                goto _finishException;
            }
            krk_push(contextManager);
        ...
        case OP_YIELD_FROM: {
            ...
            KrkValue method =
                krk_valueGetAttribute_default(krk_peek(1), "send", NONE_VAL());
            if (!IS_NONE(method)) {
                krk_push(method);
                krk_swap(1);
        ...
        case OP_CALL_ITER: {
            ...
            krk_push(krk_callStack(0));
            /* krk_valuesSame() */
            if (krk_valuesSame(iter, krk_peek(0)))
                frame->ip += OPERAND;
            break;
        }
        case OP_LOOP_ITER: {
        ...
        case OP_TEST_ARG: {
            TWO_BYTE_OPERAND;
            if (!krk_valuesSame(krk_pop(), KWARGS_VAL(0)))
                frame->ip += OPERAND;
            break;
        }
        case OP_FILTER_EXCEPT: {
            ...
            if (IS_CLASS(krk_peek(0)) &&
                krk_isInstanceOf(krk_peek(2), AS_CLASS(krk_peek(0)))) {
                isMatch = 1;
            } else if (IS_TUPLE(krk_peek(0))) {
                for (size_t i = 0; i < AS_TUPLE(krk_peek(0))->values.count; ++i) {
                    if (IS_CLASS(AS_TUPLE(krk_peek(0))->values.values[i]) &&
                        krk_isInstanceOf(
                            krk_peek(2),
                            AS_CLASS(AS_TUPLE(krk_peek(0))->values.values[i]))) {
                        isMatch = 1;
                        break;
                    }
                }
            ...
            if (isMatch) {
                ...
                krk_currentThread.stackTop[-2] =
                    HANDLER_VAL(OP_FILTER_EXCEPT, AS_HANDLER_TARGET(krk_peek(1)));
            } else {
        ...
        case OP_GET_GLOBAL: {
            ...
            if (!krk_tableGet_fast(frame->globals, name, &value)) {
                if (!krk_tableGet_fast(&vm.builtins->fields, name, &value)) {
                    krk_runtimeError(vm.exceptions->nameError, "Undefined variable '%S'.",
                                     name);
                    goto _finishException;
                }
            }
        ...
        case OP_CALL: {
            ONE_BYTE_OPERAND;
            if (unlikely(!krk_callValue(krk_peek(OPERAND), OPERAND, 1)))
                goto _finishException;
            frame = &krk_currentThread.frames[krk_currentThread.frameCount - 1];
            break;
        }
        ...
        case OP_CALL_METHOD: {
            ...
            if (IS_NONE(krk_peek(OPERAND + 1))) {
                if (unlikely(!krk_callValue(krk_peek(OPERAND), OPERAND, 2)))
                    goto _finishException;
            } else {
                if (unlikely(!krk_callValue(krk_peek(OPERAND + 1), OPERAND + 1, 1)))
                    goto _finishException;
            }
            frame = &krk_currentThread.frames[krk_currentThread.frameCount - 1];
            break;
        ...
        case OP_IMPORT_FROM: {
            ...
            if (unlikely(!valueGetProperty(name))) {
                /* Try to import... */
                KrkValue moduleName;
                if (!krk_tableGet(&AS_INSTANCE(krk_peek(0))->fields,
                                  vm.specialMethodNames[METHOD_NAME], &moduleName)) {
                    krk_runtimeError(vm.exceptions->importError,
                                     "Can not import '%S' from non-module '%T' object",
                                     name, krk_peek(0));
                    goto _finishException;
                }
                krk_push(moduleName);
                ...
                if (!krk_doRecursiveModuleLoad(AS_STRING(krk_peek(0)))) {
                    krk_currentThread.flags &= ~KRK_THREAD_HAS_EXCEPTION;
                    krk_runtimeError(vm.exceptions->importError,
                                     "Can not import '%S' from '%S'", name,
                                     AS_STRING(moduleName));
                    goto _finishException;
                }
        ...
        case OP_GET_SUPER: {
            ...
            KrkClass *superclass = AS_CLASS(baseClass)->base
                                       ? AS_CLASS(baseClass)->base
                                       : vm.baseClasses->objectClass;
            if (!krk_bindMethodSuper(superclass, name, obj_type)) {
                krk_runtimeError(vm.exceptions->attributeError,
                                 "'%S' object has no attribute '%S'", superclass->name,
                                 name);
                goto _finishException;
            }
        ...
        case OP_CLOSE_MANY: {
            ONE_BYTE_OPERAND;
            closeUpvalues((krk_currentThread.stackTop - krk_currentThread.stack) -
                          OPERAND);
        ...
        case OP_REVERSE: {
            ...
            for (ssize_t i = 0; i < (ssize_t)OPERAND / 2; ++i) {
                ...
                krk_currentThread.stackTop[-i - 2] =
                    krk_currentThread.stackTop[-(OPERAND - i) - 1];
                krk_currentThread.stackTop[-(OPERAND - i) - 1] =
                    krk_currentThread.stackTop[-1];
            }
            krk_pop();
        ...
        case OP_UNPACK: {
            ...
            if (unlikely(values->values.count != OPERAND)) {
                krk_runtimeError(vm.exceptions->valueError,
                                 "not enough values to unpack (expected %u, got %zu)",
                                 OPERAND, values->values.count);
                goto _finishException;
            }
        ...
        case OP_MAKE_STRING: {
            ...
            for (ssize_t i = 0; i < (ssize_t)OPERAND; ++i) {
                ...
                if (unlikely(!IS_STRING(s))) {
                    discardStringBuilder(&sb);
                    krk_runtimeError(vm.exceptions->valueError, "'%T' is not a string",
                                     s);
                    goto _finishException;
                }
                pushStringBuilderStr(&sb, (char *)AS_STRING(s)->chars,
                                     AS_STRING(s)->length);
            }
        ...
        case OP_MISSING_KW: {
            ONE_BYTE_OPERAND;
            krk_runtimeError(vm.exceptions->typeError,
                             "%s() missing required keyword-only argument: %R",
        ...
        case OP_UNPACK_EX: {
            ...
            if (values->values.count < before) {
                krk_runtimeError(
                    ...
                    "not enough values to unpack (expected at least %u, got %zu)",
            ...
            if (after) {
                size_t more = after;
                while (more) {
                    values->values.values[before + more] =
                        AS_LIST(list)->values[--AS_LIST(list)->count];
                    more--;
                }
            ...
            krk_currentThread.stackTop[-(ssize_t)(before + after + 1)] =
            ...
            if (!handleException()) {
                ...
                case OP_FILTER_EXCEPT:
                    krk_currentThread.stackTop[-1] =
                        HANDLER_VAL(OP_END_FINALLY, AS_HANDLER_TARGET(krk_peek(0)));
                    break;
                /* An exception happened while already in the @c finally block from
                 * handling another exception. Bail. */
                case OP_END_FINALLY:
                    krk_currentThread.stackTop[-1] =
                        HANDLER_VAL(OP_RAISE_FROM, AS_HANDLER_TARGET(krk_peek(0)));
                    break;
                /* First exception in this chain. */
                default:
                    krk_currentThread.stackTop[-1] =
                        HANDLER_VAL(OP_RAISE, AS_HANDLER_TARGET(krk_peek(0)));
                    break;
                }
...
KrkInstance *krk_startModule(const char *name) {
    ...
    krk_attachNamedObject(&module->fields, "__name__",
...
KrkValue krk_interpret(const char *src, const char *fromFile) {
    ...
    krk_attachNamedObject(&krk_currentThread.module->fields, "__file__",
                          (KrkObj *)function->chunk.filename);
