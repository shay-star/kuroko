...
void krk_printValueSafe(FILE *f, KrkValue printable) {
    if (!IS_OBJECT(printable)) {
        ...
        case KRK_VAL_INTEGER:
            fprintf(f, PRIkrk_int, AS_INTEGER(printable));
            break;
        case KRK_VAL_BOOLEAN:
            fprintf(f, "%s", AS_BOOLEAN(printable) ? "True" : "False");
            break;
        case KRK_VAL_NONE:
            fprintf(f, "None");
            break;
        case KRK_VAL_HANDLER:
            ...
            case OP_PUSH_TRY:
                fprintf(f, "{try->%d}", (int)AS_HANDLER_TARGET(printable));
                break;
            case OP_PUSH_WITH:
                fprintf(f, "{with->%d}", (int)AS_HANDLER_TARGET(printable));
                break;
            case OP_RAISE:
                fprintf(f, "{raise<-%d}", (int)AS_HANDLER_TARGET(printable));
                break;
            case OP_FILTER_EXCEPT:
                fprintf(f, "{except<-%d}", (int)AS_HANDLER_TARGET(printable));
                break;
            case OP_BEGIN_FINALLY:
                fprintf(f, "{finally<-%d}", (int)AS_HANDLER_TARGET(printable));
                break;
            case OP_RETURN:
                fprintf(f, "{return<-%d}", (int)AS_HANDLER_TARGET(printable));
                break;
            case OP_END_FINALLY:
                fprintf(f, "{end<-%d}", (int)AS_HANDLER_TARGET(printable));
                break;
            case OP_EXIT_LOOP:
                fprintf(f, "{exit<-%d}", (int)AS_HANDLER_TARGET(printable));
                break;
            case OP_RAISE_FROM:
                fprintf(f, "{reraise<-%d}", (int)AS_HANDLER_TARGET(printable));
                break;
            }
            break;
        case KRK_VAL_KWARGS: {
        ...
        default:
#    ifndef KRK_NO_FLOAT
            if (IS_FLOATING(printable))
                fprintf(f, "%.16g", AS_FLOATING(printable));
#    endif
            break;
        }
        ...
        fprintf(f, "'");
        /*
         * Print at most STRING_DEBUG_TRUNCATE characters, as bytes, escaping anything not
         * ASCII. See also str.__repr__ which does something similar with escape
         * sequences, but this is a dumber, safer, and slightly faster approach.
         */
        for (size_t c = 0; c < AS_STRING(printable)->length && c < STRING_DEBUG_TRUNCATE;
             ++c) {
        ...
        case KRK_OBJ_CODEOBJECT:
            fprintf(f, "<codeobject %s>",
                    AS_codeobject(printable)->name ? AS_codeobject(printable)->name->chars
                                                   : "?");
            break;
        case KRK_OBJ_CLASS:
            fprintf(f, "<class %s>",
                    AS_CLASS(printable)->name ? AS_CLASS(printable)->name->chars : "?");
            break;
        case KRK_OBJ_INSTANCE:
            fprintf(f, "<instance of %s>", AS_INSTANCE(printable)->_class->name->chars);
            break;
        case KRK_OBJ_NATIVE:
            fprintf(f, "<nativefn %s>", ((KrkNative *)AS_OBJECT(printable))->name);
            break;
        case KRK_OBJ_CLOSURE:
            fprintf(f, "<function %s>", AS_CLOSURE(printable)->function->name->chars);
            break;
        case KRK_OBJ_BYTES:
            fprintf(f, "<bytes of len %ld>", (long)AS_BYTES(printable)->length);
            break;
        case KRK_OBJ_TUPLE: {
            fprintf(f, "(");
        ...
        case KRK_OBJ_BOUND_METHOD:
            fprintf(
                ...
                AS_BOUND_METHOD(printable)->method
                    ? (AS_BOUND_METHOD(printable)->method->type == KRK_OBJ_CLOSURE
                           ? ((KrkClosure *)AS_BOUND_METHOD(printable)->method)
                                 ->function->name->chars
                           : (AS_BOUND_METHOD(printable)->method->type == KRK_OBJ_NATIVE
                                  ? ((KrkNative *)AS_BOUND_METHOD(printable)->method)
                                        ->name
                                  : "(unknown)"))
...
void krk_debug_dumpStack(FILE *file, KrkCallFrame *frame) {
    ...
    for (KrkValue *slot = krk_currentThread.stack; slot < krk_currentThread.stackTop;
         slot++) {
        fprintf(file, "[%c", frame->slots == i ? '*' : ' ');

        for (size_t x = krk_currentThread.frameCount; x > 0; x--) {
            if (krk_currentThread.frames[x - 1].slots > i)
                continue;
            ...
            for (size_t j = 0; j < f->closure->function->localNameCount; ++j) {
                if (relative == f->closure->function->localNames[j].id
                    ...
                    && f->closure->function->localNames[j].birthday <=
                           (size_t)(f->ip - f->closure->function->chunk.code) &&
                    f->closure->function->localNames[j].deathday >=
                        (size_t)(f->ip - f->closure->function->chunk.code)) {
...
static inline const char *opcodeClean(const char *opc) { return &opc[3]; }
...
#    define CONSTANT(opc, more)                                                          \
    case opc: {                                                                          \
        size_t constant _unused = chunk->code[offset + 1];                               \
        size = 2;                                                                        \
        more;                                                                            \
        break;                                                                           \
    }                                                                                    \
    case opc##_LONG: {                                                                   \
        size_t constant _unused = (chunk->code[offset + 1] << 16) |                      \
...
#    define JUMP(opc, sign)                                                              \
    case opc: {                                                                          \
        uint16_t jump = (chunk->code[offset + 1] << 8) | (chunk->code[offset + 2]);      \
        krk_tableSet(AS_DICT(func->jumpTargets),                                         \
                     INTEGER_VAL((size_t)(offset + 3 sign jump)), BOOLEAN_VAL(1));       \
        size = 3;                                                                        \
        break;                                                                           \
    }
...
#    undef FORMAT_VALUE_MORE
    ...
    if (!IS_dict(func->jumpTargets))
        return 0;
    KrkValue garbage;
    if (krk_tableGet(AS_DICT(func->jumpTargets), INTEGER_VAL(startPoint), &garbage))
        return 1;
    return 0;
}

#    define OPARGS                                                                       \
        FILE *f, const char *fullName, size_t *size, size_t *offset,                     \
            KrkCodeObject *func, KrkChunk *chunk
#    define OPARG_VALS f, fullName, size, offset, func, chunk

static void _print_opcode(OPARGS) { fprintf(f, "%-16s ", opcodeClean(fullName)); }
...
static void _constant(OPARGS, int isLong, void (*more)(OPARGS, size_t constant)) {
    _print_opcode(OPARG_VALS);
    size_t constant = isLong ? (chunk->code[*offset + 1] << 16) |
                                   (chunk->code[*offset + 2] << 8) |
                                   (chunk->code[*offset + 3])
                             : chunk->code[*offset + 1];
...
#    define JUMP(opc, sign)                                                              \
    case opc:                                                                            \
        fprintf(f, "(%s, to %zu)", opcodeClean(#opc), *offset + 3 sign current_jump);    \
        return;
#    define OPERAND(opc, more)
#    define CONSTANT(opc, more)
#    define COMPLICATED(opc, more)
...
#    define SIMPLE(opc)                                                                  \
    case opc:                                                                            \
        _simple(f, #opc, &size, &offset, func, chunk);                                   \
        break;
#    define CONSTANT(opc, more)                                                          \
    case opc:                                                                            \
        _constant(f, #opc, &size, &offset, func, chunk, 0, more);                        \
        break;                                                                           \
    case opc##_LONG:                                                                     \
        _constant(f, #opc "_LONG", &size, &offset, func, chunk, 1, more);                \
        break;
#    define OPERAND(opc, more)                                                           \
    case opc:                                                                            \
        _operand(f, #opc, &size, &offset, func, chunk, 0, more);                         \
        break;                                                                           \
    case opc##_LONG:                                                                     \
        _operand(f, #opc "_LONG", &size, &offset, func, chunk, 1, more);                 \
        break;
#    define JUMP(opc, sign)                                                              \
    case opc:                                                                            \
        _jump(f, #opc, &size, &offset, func, chunk, sign 1);                             \
        break;
#    define COMPLICATED(opc, more)                                                       \
    case opc:                                                                            \
        _complicated(f, #opc, &size, &offset, func, chunk, more);                        \
        break;
...
static void _closure_more(OPARGS, size_t constant) {
    ...
    for (size_t j = 0; j < function->upvalueCount; ++j) {
        ...
        if (isLocal & 2) {
            index = (index << 16) | (chunk->code[*offset + *size] << 8) |
                    chunk->code[*offset + 1 + *size];
            offset += 2;
        }
        if (isLocal & 1) {
            for (size_t i = 0; i < func->localNameCount; ++i) {
                if (func->localNames[i].id == (size_t)index &&
                    func->localNames[i].birthday <= *offset &&
            ...
            fprintf(f, "upvalue<%d>", index);
        }
        if (j + 1 != function->upvalueCount)
            fprintf(f, ", ");
    }
}
...
static void _format_value_more(OPARGS, size_t operand) {
    if (operand != 0) {
        int hasThing = 0;
        fprintf(f, " (");
        if (operand & FORMAT_OP_EQ) {
            fprintf(f, "eq");
            hasThing = 1;
        }
        if (operand & FORMAT_OP_STR) {
            fprintf(f, "%sstr", hasThing ? ", " : "");
            hasThing = 1;
        }
        if (operand & FORMAT_OP_REPR) {
            fprintf(f, "%srepr", hasThing ? ", " : "");
            hasThing = 1;
        }
        if (operand & FORMAT_OP_FORMAT) {
            fprintf(f, "%swith format", hasThing ? ", " : "");
        }
        fprintf(f, ")");
    }
}
...
static void _local_more(OPARGS, size_t operand) {
    for (size_t i = 0; i < func->localNameCount; ++i) {
        if (func->localNames[i].id == operand &&
            func->localNames[i].birthday <= *offset &&
            func->localNames[i].deathday >= *offset) {
            fprintf(f, " (%s", func->localNames[i].name->chars);
            if ((short int)operand < func->potentialPositionals) {
                fprintf(f, ", arg");
            } else if ((short int)operand <
                       func->potentialPositionals + func->keywordArgs +
                           !!(func->obj.flags & KRK_OBJ_FLAGS_CODEOBJECT_COLLECTS_ARGS)) {
                fprintf(f, ", kwarg");
            }
            fprintf(f, ")");
            break;
...
size_t krk_disassembleInstruction(FILE *f, KrkCodeObject *func, size_t offset) {
    KrkChunk *chunk = &func->chunk;
    if (offset > 0 &&
        krk_lineNumber(chunk, offset) == krk_lineNumber(chunk, offset - 1)) {
        fprintf(f, "     ");
    } else {
        if (offset > 0)
            fprintf(f, "\n");
...
#    include "opcodes.h"
    ...
    for (size_t i = 0; i < func->localNameCount; ++i) {
        if (func->localNames[i].birthday >= offset &&
            func->localNames[i].birthday < offset + size) {
            fprintf(f, " +%s", func->localNames[i].name->chars);
        }
    }
    ...
    for (size_t i = 0; i < func->localNameCount; ++i) {
        if (func->localNames[i].deathday >= offset &&
            func->localNames[i].deathday < offset + size) {
            fprintf(f, " -%s", func->localNames[i].name->chars);
        }
    }
...
int krk_debug_enableBreakpoint(int breakIndex) {
    if (breakIndex < 0 || breakIndex >= vm.dbgState->breakpointsCount ||
        ...
        return 1;
    vm.dbgState->breakpoints[breakIndex]
        .inFunction->chunk.code[vm.dbgState->breakpoints[breakIndex].offset] =
        OP_BREAKPOINT;
    return 0;
}
...
int krk_debug_removeBreakpoint(int breakIndex) {
    ...
    vm.dbgState->breakpoints[breakIndex].inFunction = NULL;
    while (vm.dbgState->breakpointsCount &&
           vm.dbgState->breakpoints[vm.dbgState->breakpointsCount - 1].inFunction ==
               NULL) {
...
int krk_debuggerHook(KrkCallFrame *frame) {
    ...
    if (!vm.dbgState->thisWasForced) {
        ...
        /* If we weren't asked to step to the next breakpoint, we need to disable single
         * stepping. */
        krk_debug_disableSingleStep();
        vm.dbgState->thisWasForced = 0;
    }

    /* If the top of the repeat stack is an index, we need to ensure we re-enable that
     * breakpoint */
    if (vm.dbgState->repeatStack_top != -1 &&
        !(krk_currentThread.flags & KRK_THREAD_SINGLE_STEP)) {
...
int krk_debug_examineBreakpoint(int breakIndex, KrkCodeObject **funcOut,
                                size_t *offsetOut, int *flagsOut, int *enabled) {
    if (breakIndex < 0 || breakIndex >= vm.dbgState->breakpointsCount)
        return -1;
    ...
    if (funcOut)
        *funcOut = vm.dbgState->breakpoints[breakIndex].inFunction;
    if (offsetOut)
        *offsetOut = vm.dbgState->breakpoints[breakIndex].offset;
    if (flagsOut)
        *flagsOut = vm.dbgState->breakpoints[breakIndex].flags;
    if (enabled)
        *enabled =
            (vm.dbgState->breakpoints[breakIndex]
                 .inFunction->chunk.code[vm.dbgState->breakpoints[breakIndex].offset] ==
             OP_BREAKPOINT) ||
            breakIndex == vm.dbgState->repeatStack_top;
...
int krk_debugBreakpointHandler(void) {
    ...
    for (int i = 0; i < vm.dbgState->breakpointsCount; ++i) {
        if (vm.dbgState->breakpoints[i].inFunction == callee &&
...
void krk_debug_addExpression(KrkCodeObject *codeobject, uint8_t start, uint8_t midStart,
                             uint8_t midEnd, uint8_t end) {
    /* Traceback entries point to the last byte of an opcode, due to the way instruction
     * fetch advances the instruction pointer past all of the constituent operands of an
     * opcode; as such, our map is based on these last bytes and we need to look at the
     * byte preceding the current count when adding new entries. */
    size_t offset = codeobject->chunk.count - 1;

    /* We can feasibly support offsets larger than UINT32_MAX on 64-bit platforms, though
     * this should never really happen. Just in case, avoid messing up our table with bad
     * values. */
    ...
    if (codeobject->expressionsCapacity < codeobject->expressionsCount + 1) {
        ...
        codeobject->expressions =
            KRK_GROW_ARRAY(KrkExpressionsMap, codeobject->expressions, old,
                           codeobject->expressionsCapacity);
    }

    codeobject->expressions[codeobject->expressionsCount] =
        (KrkExpressionsMap){offset, start, midStart, midEnd, end};
    codeobject->expressionsCount++;
}

int krk_debug_expressionUnderline(const KrkCodeObject *codeobject, uint8_t *start,
                                  uint8_t *midStart, uint8_t *midEnd, uint8_t *end,
                                  size_t instruction) {
    /* We could do binary search here, but as we only print these when an exception
     * 'escapes', it's not really worth the optimization over a linear search per line in
     * the traceback. */
