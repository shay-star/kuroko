...
#define DO_KEY(key)                                                                      \
    krk_attachNamedObject(AS_DICT(result), #key,                                         \
                          (KrkObj *)krk_copyString(buf.key, strlen(buf.key)))
...
static void _loadEnviron(KrkInstance *module) {
    ...
    KrkClass *Environ =
        krk_makeClass(module, &os_Environ, "_Environ", vm.baseClasses->dictClass);
...
KRK_Function(system) {
    const char *cmd;
    if (!krk_parseArgs("s", (const char *[]){"command"}, &cmd))
        return NONE_VAL();
...
KRK_Function(chdir) {
    const char *path;
    if (!krk_parseArgs("s", (const char *[]){"path"}, &path))
        return NONE_VAL();
    if (chdir(path))
        return krk_runtimeError(KRK_EXC(OSError), "%s", strerror(errno));
    return NONE_VAL();
...
KRK_Function(strerror) {
    int errnum;
    if (!krk_parseArgs("i", (const char *[]){"errnum"}, &errnum))
        return NONE_VAL();
...
KRK_Function(access) {
    const char *path;
    int mask;
    if (!krk_parseArgs("si", (const char *[]){"pathname", "mode"}, &path, &mask))
        return NONE_VAL();
...
KRK_Function(abort) { abort(); }

KRK_Function(exit) {
    int status;
    if (!krk_parseArgs("i", (const char *[]){"status"}, &status))
        return NONE_VAL();
    exit(status);
}
...
KRK_Function(truncate) {
    const char *path;
    size_t length;
    if (!krk_parseArgs("sn", (const char *[]){"path", "length"}, &path, &length))
        return NONE_VAL();
...
KRK_Function(dup) {
    int fd;
    if (!krk_parseArgs("i", (const char *[]){"fd"}, &fd))
        return NONE_VAL();
    int result = dup(fd);
    if (result < 0) {
        return krk_runtimeError(KRK_EXC(OSError), "%s", strerror(errno));
...
KRK_Function(dup2) {
    int fd, fd2;
    if (!krk_parseArgs("ii", (const char *[]){"fd", "fd2"}, &fd, &fd2))
        return NONE_VAL();
...
KRK_Function(lseek) {
    int fd, how;
    ssize_t offset;
    if (!krk_parseArgs("ini", (const char *[]){"fd", "offset", "how"}, &fd, &offset,
                       &how))
        return NONE_VAL();
...
KRK_Function(open) {
    ...
    int flags;
    int mode = 0777;
    if (!krk_parseArgs("si|i", (const char *[]){"path", "flags", "mode"}, &path, &flags,
                       &mode))
        return NONE_VAL();
...
KRK_Function(mkdir) {
    const char *path;
    int mode = 0777;
    if (!krk_parseArgs("s|i", (const char *[]){"path", "mode"}, &path, &mode))
        return NONE_VAL();
...
KRK_Function(read) {
    int fd;
    ssize_t count;
    if (!krk_parseArgs("in", (const char *[]){"fd", "count"}, &fd, &count))
        return NONE_VAL();
...
KRK_Function(write) {
    int fd;
    KrkBytes *buf;
    if (!krk_parseArgs("iO!", (const char *[]){"fd", "buf"}, &fd, KRK_BASE_CLASS(bytes),
                       &buf))
        return NONE_VAL();
...
KRK_Function(kill) {
    ...
    ssize_t pid;
    int sig;
    if (!krk_parseArgs("ni", (const char *[]){"pid", "sig"}, &pid, &sig))
        return NONE_VAL();
...
KRK_Function(symlink) {
    const char *src;
    const char *dst;
    if (!krk_parseArgs("ss", (const char *[]){"target", "linkpath"}, &src, &dst))
        return NONE_VAL();
...
KRK_Function(tcsetpgrp) {
    int fd;
    ssize_t pgrp;
    if (!krk_parseArgs("in", (const char *[]){"fd", "pgrp"}, &fd, &pgrp))
        return NONE_VAL();
...
static int makeArgs(int count, const KrkValue *values, char ***argsOut,
...
KRK_Function(execl) {
    ...
    char **args;
    if (makeArgs(argc - 1, &argv[1], &args, _method_name))
        return NONE_VAL();
...
KRK_Function(execle) {
    ...
    char **args;
    char **env;
    if (makeArgs(argc - 2, &argv[1], &args, _method_name))
        return NONE_VAL();
...
KRK_Function(execv) {
    ...
    char **argp;
    if (makeArgs(args->values.count, args->values.values, &argp, _method_name))
        return NONE_VAL();
...
#define getProp(name)                                                                    \
    ...
    if (!IS_INTEGER(name))                                                               \
    return krk_runtimeError(vm.exceptions->valueError, "stat_result is invalid")
...
KRK_Method(stat_result, __repr__) {
    ...
    size_t len = snprintf(buf, 1024,
                          ...
                          "st_gid=%d,"
                          "st_size=%d)",
                          (int)AS_INTEGER(st_dev), (int)AS_INTEGER(st_ino),
                          (int)AS_INTEGER(st_mode), (int)AS_INTEGER(st_nlink),
                          (int)AS_INTEGER(st_uid), (int)AS_INTEGER(st_gid),
...
#define DO_INT(name) krk_attachNamedValue(&module->fields, #name, INTEGER_VAL(name))
    ...
    KRK_DOC(
        ...
        "On POSIX platforms, the result should match the contents and layout of a "
        "standard @c uname() call. "
        "On Windows, values are synthesized from available information.");
    KRK_DOC(BIND_FUNC(module, system), "@brief Call the system shell.\n"
                                       "@arguments cmd\n\n"
                                       "Runs @p cmd using the system shell and returns "
                                       "the platform-dependent return value.");
    KRK_DOC(BIND_FUNC(module, getcwd),
            "@brief Get the name of the current working directory.");
    KRK_DOC(BIND_FUNC(module, chdir), "@brief Change the current working directory.\n"
                                      "@arguments newcwd\n\n"
                                      "Attempts to change the working directory to @p "
                                      "newcwd. Raises @ref OSError on failure.");
    KRK_DOC(BIND_FUNC(module, getpid), "@brief Obtain the system process identifier.");
    KRK_DOC(
    ...
    KRK_DOC(BIND_FUNC(module, exit),
            ...
            "@bsnote{This will exit the interpreter without calling cleanup routines.}");
    KRK_DOC(BIND_FUNC(module, remove), "@brief Delete a file.\n"
                                       "@arguments path\n\n"
    ...
    KRK_DOC(BIND_FUNC(module, isatty),
            ...
            "@arguments fd\n\n"
            "Returns a @ref bool indicating whether the open file descriptor @p fd "
    ...
    KRK_DOC(BIND_FUNC(module, open), "@brief Open a file.\n"
                                     ...
                                     "Opens the file at @p path with the specified @p "
                                     "flags and @p mode. Returns a file descriptor.\n\n"
                                     "@bsnote{Not to be confused with <a class=\"el\" "
                                     "href=\"mod_fileio.html#open\">fileio.open</a>}");
    KRK_DOC(BIND_FUNC(module, close), "@brief Close an open file descriptor.\n"
                                      "@arguments fd");
    ...
    KRK_DOC(BIND_FUNC(module, mkdir), "@brief Create a directory.\n"
                                      "@arguments path,mode=0o777\n\n"
                                      "Creates a directory at @p path.");

    KRK_DOC(BIND_FUNC(module, execl), "@brief Replace the current process.\n"
                                      "@arguments path,[args...]\n\n"
                                      "The @c exec* family of functions replaces the "
                                      "calling process's image with a new one. "
                                      "@c execl takes a @p path to a binary and an "
                                      "arbitrary number of @ref str arguments to "
                                      "pass to the new executable.");
    KRK_DOC(BIND_FUNC(module, execle), "@brief Replace the current process.\n"
                                       "@arguments path,[args...],env\n\n"
                                       "The @c exec* family of functions replaces the "
                                       "calling process's image with a new one. "
                                       "@c execle takes a @p path to a binary, an "
                                       "arbitrary number of @ref str arguments to "
                                       "pass to the new executable, and @ref list of @c "
                                       "'KEY=VALUE' pairs to set as the new "
                                       "environment.");
    KRK_DOC(BIND_FUNC(module, execlp),
            "@brief Replace the current process.\n"
            "@arguments filename,[args...]\n\n"
            "The @c exec* family of functions replaces the calling process's image with "
            "a new one. "
            "@c execlp takes a @p filename of a binary and an arbitrary number of @ref "
            "str arguments to "
            "pass to the new executable. @p filename will be searched for in @c $PATH.");
    KRK_DOC(BIND_FUNC(module, execv), "@brief Replace the current process.\n"
                                      "@arguments path,args\n\n"
                                      "The @c exec* family of functions replaces the "
                                      "calling process's image with a new one. "
                                      "@c execv takes a @p path to a binary and a @ref "
                                      "list @p args of @ref str arguments to "
                                      "pass to the new executable.");
    KRK_DOC(BIND_FUNC(module, execvp),
            ...
            "The @c exec* family of functions replaces the calling process's image with "
            "a new one. "
            "@c execvp takes a @p filename of a binary and a @ref list @p args of @ref "
            "str arguments to "
            "pass to the new executable. @p filename will be searched for in @c $PATH.");
    ...
    KRK_DOC(BIND_FUNC(module, access),
            ...
            "@arguments path,mask\n\n"
            "Use the values @c F_OK, @c R_OK, @c W_OK, and @c X_OK to construct @p mask "
            "and check if the current "
            "process has sufficient access rights to perform the requested operations on "
            "the file "
            "at @p path.");
    ...
    KRK_DOC(BIND_FUNC(module, pipe),
            "@brief Create a pipe.\n\n"
            "Creates a _pipe_, returning a two-tuple of file descriptors for the read "
            "and write ends respectively.");
    KRK_DOC(BIND_FUNC(module, kill),
            "@brief Send a signal to a process.\n"
            "@arguments pid,signum\n\n"
            "Send the signal @p signum to the process at @p pid.\n");
    KRK_DOC(BIND_FUNC(module, fork), "@brief Fork the current process.\n\n"
                                     "Returns the PID of the new child process in the "
                                     "original process and @c 0 in the child.");
    ...
    KRK_DOC(BIND_FUNC(module, tcgetpgrp),
            ...
            "@arguments fd\n\n"
            "Return the PID representing the foreground process group of the terminal "
            "specified by the file descriptor @p fd.");
    KRK_DOC(BIND_FUNC(module, tcsetpgrp),
            "@brief %Set the terminal foreground process group.\n"
            "@arguments fd,pgrp\n\n"
            "%Set the PID representing the foreground process group of the terminal "
            "specified by the file descriptor @p fd to @p pgrp.");
    KRK_DOC(BIND_FUNC(module, ttyname),
            "@brief Get the path to a terminal device.\n"
            "@arguments fd\n\n"
            "Returns a @ref str representing the path to the terminal device provided by "
            "the file descriptor @p fd.");
    ...
    /* Nothing special */
    KrkClass *stat_result = krk_makeClass(module, &os_stat_result, "stat_result",
