...
KRK_Function(enablebreakpoint) {
    int breakIndex;
    if (!krk_parseArgs("i", (const char *[]){"breakpoint"}, &breakIndex))
        return NONE_VAL();
...
KRK_Function(addbreakpoint) {
    ...
    if (!krk_parseArgs("Vi|i", (const char *[]){"func", "lineno", "flags"}, &func,
                       &lineNo, &flags))
        return NONE_VAL();
    ...
    if (IS_STRING(func)) {
        ...
        } else if (IS_BOUND_METHOD(func) &&
                   IS_CLOSURE(OBJECT_VAL(AS_BOUND_METHOD(func)->method))) {
    ...
    if (result < 0)
        return krk_runtimeError(vm.exceptions->baseException,
...
KRK_Function(dis) {
    KrkValue funcVal;
    if (!krk_parseArgs("V", (const char *[]){"func"}, &funcVal))
        return NONE_VAL();

    if (IS_CLOSURE(funcVal)) {
        KrkCodeObject *func = AS_CLOSURE(funcVal)->function;
        krk_disassembleCodeObject(stdout, func,
                                  func->name ? func->name->chars : "<unnamed>");
    } else if (IS_codeobject(funcVal)) {
        krk_disassembleCodeObject(stdout, AS_codeobject(funcVal),
                                  AS_codeobject(funcVal)->name
                                      ? AS_codeobject(funcVal)->name->chars
                                      : "<unnamed>");
    } else if (IS_BOUND_METHOD(funcVal)) {
        if (AS_BOUND_METHOD(funcVal)->method->type == KRK_OBJ_CLOSURE) {
            KrkCodeObject *func =
                ((KrkClosure *)AS_BOUND_METHOD(funcVal)->method)->function;
            const char *methodName = func->name ? func->name->chars : "<unnamed>";
            const char *typeName =
                IS_CLASS(AS_BOUND_METHOD(funcVal)->receiver)
                    ? AS_CLASS(AS_BOUND_METHOD(funcVal)->receiver)->name->chars
            ...
            krk_runtimeError(vm.exceptions->typeError,
                             "Can not disassemble built-in method of '%T'",
                             AS_BOUND_METHOD(funcVal)->receiver);
        }
    } else if (IS_CLASS(funcVal)) {
        KrkValue code;
        if (krk_tableGet(&AS_CLASS(funcVal)->methods, vm.specialMethodNames[METHOD_FUNC],
                         &code) &&
            IS_CLOSURE(code)) {
        ...
        krk_runtimeError(vm.exceptions->typeError, "Don't know how to disassemble '%T'",
                         funcVal);
    }
...
KRK_Function(build) {
    ...
    if (!krk_parseArgs("s|s", (const char *[]){"code", "filename"}, &code, &fileName))
        return NONE_VAL();
...
#    define CONSTANT(opc, more)                                                          \
    ...
    case opc##_LONG: {                                                                   \
        constant = (chunk->code[offset + 1] << 16) | (chunk->code[offset + 2] << 8) |    \
                   (chunk->code[offset + 3]);                                            \
        size = 4;                                                                        \
        more;                                                                            \
        break;                                                                           \
...
#    define OPERAND(opc, more)                                                           \
    ...
    case opc##_LONG: {                                                                   \
        operand = (chunk->code[offset + 1] << 16) | (chunk->code[offset + 2] << 8) |     \
                  (chunk->code[offset + 3]);                                             \
        size = 4;                                                                        \
        more;                                                                            \
        break;                                                                           \
...
#    define JUMP(opc, sign)                                                              \
    case opc: {                                                                          \
        jump = 0 sign((chunk->code[offset + 1] << 8) | (chunk->code[offset + 2]));       \
        size = 3;                                                                        \
        break;                                                                           \
    }
...
KRK_Function(examine) {
    KrkCodeObject *func;
    if (!krk_parseArgs("O!", (const char *[]){"func"}, KRK_BASE_CLASS(codeobject), &func))
        return NONE_VAL();
...
KRK_Function(ip_to_expression) {
    ...
    if (!krk_parseArgs("VN", (const char *[]){"func", "ip"}, &func, &ip))
        return NONE_VAL();

    KrkCodeObject *actual;

    if (IS_CLOSURE(func))
        actual = AS_CLOSURE(func)->function;
    else if (IS_codeobject(func))
        actual = AS_codeobject(func);
    else if (IS_BOUND_METHOD(func) &&
             IS_CLOSURE(OBJECT_VAL(AS_BOUND_METHOD(func)->method)))
        actual = ((KrkClosure *)AS_BOUND_METHOD(func)->method)->function;
    else
...
KRK_Module(dis) {
    ...
    KRK_DOC(
        ...
        "process is entirely static and bytecode analysis can be performed without "
        "calling "
    ...
    KRK_DOC(BIND_FUNC(module, examine),
            ...
            "@arguments func\n\n"
            "Examines the code object @p func and returns a list representation of its "
            "instructions. "
            "Each instruction entry is a tuple of the opcode, total instruction size in "
            "bytes, and "
            "the operand of the argument, either as an integer for jump offsets, the "
            "actual value for "
    ...
    KRK_DOC(BIND_FUNC(module, addbreakpoint),
            ...
            "@arguments func, line\n\n"
            "@p func may be a filename string, or a function, method, or code object. "
            "Returns "
            "the new breakpoint index, or raises @ref Exception if a breakpoint code not "
            "be added.");
    ...
    KRK_DOC(BIND_FUNC(module, ip_to_expression),
            ...
            "@arguments func,ip\n\n"
            "For various reasons, the instruction pointer @p ip must be the last byte of "
            "an opcode.");

    krk_attachNamedValue(&module->fields, "BREAKPOINT_ONCE",
                         INTEGER_VAL(KRK_BREAKPOINT_ONCE));
    krk_attachNamedValue(&module->fields, "BREAKPOINT_REPEAT",
                         INTEGER_VAL(KRK_BREAKPOINT_REPEAT));
...
#    undef COMPLICATED
    ...
    if (runAs && !strcmp(runAs->chars, "__main__")) {
        ...
        krk_interpret("import dis\n"
                      ...
                      "            if isinstance(operand,codeobject) and operand not in "
                      "seen and operand not in next:\n"
                      "                next.append(operand)\n"
                      "        if next:\n"
                      "            print()\n"
                      "import kuroko\n"
