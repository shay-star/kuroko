...
KRK_Method(socket, __init__) {
    ...
    if (!krk_parseArgs(".|iii:socket", (const char *[]){"family", "type", "proto"},
                       &family, &type, &proto)) {
        return NONE_VAL();
    }
...
KRK_Method(socket, __repr__) {
    char tmp[4096];
    size_t len = snprintf(
        tmp, 4096, "<socket.socket fd=%d, family=%s, type=%s, proto=%d>", self->sockfd,
        _af_name(self->family), _sock_type(self->type), self->proto);
    return OBJECT_VAL(krk_copyString(tmp, len));
}

static int socket_parse_address(struct socket *self, KrkValue address,
                                struct sockaddr_storage *sock_addr,
                                socklen_t *sock_size) {
    if (self->family == AF_INET) {
        /* Should be 2-tuple */
        if (!IS_tuple(address)) {
            krk_runtimeError(vm.exceptions->typeError, "Expected 2-tuple, not '%T'",
                             address);
            return 1;
        }
        ...
        if (!IS_str(addr->values.values[0])) {
            krk_runtimeError(vm.exceptions->typeError, "Address should be str, not '%T'",
                             addr->values.values[0]);
            return 1;
        }
        if (!IS_int(addr->values.values[1])) {
            krk_runtimeError(vm.exceptions->typeError, "Port should be int, not '%T'",
                             addr->values.values[1]);
            return 1;
        }
        ...
        if (!AS_STRING(addr->values.values[0])->length) {
            ...
            *sock_size = sizeof(struct sockaddr_in);
            ...
            struct addrinfo *res;
            int error =
                getaddrinfo(AS_CSTRING(addr->values.values[0]), NULL, NULL, &result);
            if (error != 0) {
...
KRK_Method(socket, accept) {
    ...
    if (self->family == AF_INET) {
        ...
        getnameinfo((struct sockaddr *)&addr, addrlen, hostname, NI_MAXHOST, NULL, 0,
                    NI_NUMERICHOST);

        addrTuple->values.values[0] =
            OBJECT_VAL(krk_copyString(hostname, strlen(hostname)));
        addrTuple->values.count = 1;
        addrTuple->values.values[1] =
            INTEGER_VAL(htons(((struct sockaddr_in *)&addr)->sin_port));
        ...
        addrTuple->values.count = 1;
        addrTuple->values.values[1] =
            INTEGER_VAL(htons(((struct sockaddr_in6 *)&addr)->sin6_port));
        addrTuple->values.count = 2;
#endif
#ifdef AF_UNIX
    } else if (self->family == AF_UNIX) {
...
KRK_Method(socket, sendto) {
    ...
    ssize_t result = sendto(self->sockfd, (void *)buf->bytes, buf->length, flags,
...
KRK_Method(socket, fileno) { return INTEGER_VAL(self->sockfd); }
...
KRK_Method(socket, setsockopt) {
    ...
    if (IS_INTEGER(argv[3])) {
        ...
        result = setsockopt(self->sockfd, level, optname,
...
KRK_Method(socket, family) {
    if (argc > 1)
        return krk_runtimeError(vm.exceptions->attributeError, "readonly attribute");
...
KRK_Module(socket) {
    ...
    KrkClass *socket =
        krk_makeClass(module, &SocketClass, "socket", vm.baseClasses->objectClass);
    ...
    KRK_DOC(BIND_METHOD(socket, bind),
            ...
            "@arguments address\n\n"
            "The format of @p address varies by address family. For @c AF_INET, @p "
            "address should be a "
    ...
    KRK_DOC(BIND_METHOD(socket, shutdown), "@brief Shut down an active socket.\n"
                                           "@arguments how\n\n"
                                           "Gracefully closes an open socket.");
    KRK_DOC(BIND_METHOD(socket, recv), "@brief Receive data from a connected socket.\n"
                                       "@arguments bufsize,[flags]\n\n"
                                       "Receive up to @p bufsize bytes of data, which is "
                                       "returned as a @ref bytes object.");
    KRK_DOC(
    ...
    KRK_DOC(BIND_METHOD(socket, setsockopt),
            ...
            "@p level and @p optname should be integer values defined by @c SOL and @c "
            "SO options. "
