...
KRK_Function(sleep) {
    ...
    unsigned int usecs =
        (IS_INTEGER(argv[0]) ? AS_INTEGER(argv[0])
...
KRK_Method(struct_time, __init__) {
    KrkValue seq;
    if (!krk_parseArgs(".V:struct_time", (const char *[]){"iterable"}, &seq))
        return NONE_VAL();
    if (!IS_TUPLE(seq) || AS_TUPLE(seq)->values.count != 9)
        return krk_runtimeError(vm.exceptions->notImplementedError,
                                "sequence other than 9-tuple unsupported");
    for (int i = 0; i < 9; ++i) {
        if (!IS_INTEGER(AS_TUPLE(seq)->values.values[i]))
            return krk_runtimeError(vm.exceptions->valueError, "expected int, not %T",
                                    AS_TUPLE(seq)->values.values[i]);
    }
...
KRK_Method(struct_time, tm_year) {
    return INTEGER_VAL(self->_value.tm_year + 1900);
} /* struct tm is 1900-indexed, snakes are not */
KRK_Method(struct_time, tm_mon) {
    return INTEGER_VAL(self->_value.tm_mon + 1);
} /* struct tm is 0-indexed, snakes are not */
...
KRK_Method(struct_time, tm_wday) {
    return INTEGER_VAL((self->_value.tm_wday + 1) % 7);
} /* struct tm has Sunday = 0, but snakes use Monday = 0 */
KRK_Method(struct_time, tm_yday) {
    return INTEGER_VAL(self->_value.tm_yday + 1);
} /* struct tm is from 0, but snakes start from 1 */
...
KRK_Method(struct_time, __repr__) {
    return krk_stringFromFormat(
        ...
        self->_value.tm_year + 1900, self->_value.tm_mon + 1, self->_value.tm_mday,
        self->_value.tm_hour, self->_value.tm_min, self->_value.tm_sec,
        (self->_value.tm_wday + 1) % 7, self->_value.tm_yday + 1, self->_value.tm_isdst);
}
...
KRK_Function(localtime) {
    int gave_seconds;
    long long seconds;
    if (!krk_parseArgs("|L?", (const char *[]){"seconds"}, &gave_seconds, &seconds))
        return NONE_VAL();
    ...
    if (!localtime_r(&time, &out->_value))
...
static KrkValue krk_asctime(const struct tm *_time) {
    /* asctime is normally locale-aware, but the snake function is not, so we do this
     * manually */
    ...
    snprintf(buf, 39, "%s %s%3d %.2d:%.2d:%.2d %d", dayNames[_time->tm_wday % 7],
             monNames[_time->tm_mon % 12], _time->tm_mday, _time->tm_hour, _time->tm_min,
             _time->tm_sec, _time->tm_year + 1900);
...
KRK_Function(asctime) {
    ...
    if (!krk_parseArgs("|O!", (const char *[]){"t"}, struct_time, &t))
        return NONE_VAL();
    struct tm _time;
    tm_or_now(t, &_time);
...
KRK_Function(ctime) {
    int has_arg;
    long long secs;
    if (!krk_parseArgs("|L?", (const char *[]){"secs"}, &has_arg, &secs))
        return NONE_VAL();
    time_t time = time_or_now(has_arg, secs);
    struct tm _time;
    if (!localtime_r(&time, &_time))
        return krk_runtimeError(vm.exceptions->valueError, "?");
...
KRK_Function(gmtime) {
    int gave_seconds;
    long long seconds;
    if (!krk_parseArgs("|L?", (const char *[]){"secs"}, &gave_seconds, &seconds))
        return NONE_VAL();
    time_t time = time_or_now(gave_seconds, seconds);
...
KRK_Function(mktime) {
    ...
    if (out == -1 && _time.tm_wday == -1)
        return krk_runtimeError(vm.exceptions->valueError, "invalid argument to mktime");
    return FLOATING_VAL(out);
}
...
KRK_Function(strftime) {
    const char *format;
    struct struct_time_obj *t = NULL;
    if (!krk_parseArgs("s|O!", (const char *[]){"format", "t"}, &format, struct_time, &t))
        return NONE_VAL();
    struct tm _time;
    tm_or_now(t, &_time);
...
KRK_Module(time) {
    ...
    KRK_DOC(BIND_FUNC(module, sleep),
            "@brief Pause execution of the current thread.\n"
            "@arguments secs\n\n"
            "Uses the system @c usleep() function to sleep for @p secs seconds, which "
            "may be a @ref float or @ref int. "
            "The available precision is platform-dependent.");
    KRK_DOC(BIND_FUNC(module, time),
            "@brief Return the elapsed seconds since the system epoch.\n\n"
            "Returns a @ref float representation of the number of seconds since the "
            "platform's epoch date. "
    ...
    KRK_DOC(BIND_PROP(struct_time, tm_sec),
    ...
    KRK_DOC(BIND_FUNC(module, localtime),
