...
KRK_Function(setlocale) {
    ...
    if (!krk_parseArgs("i|z", (const char *[]){"category", "locale"}, &category, &locale))
        return NONE_VAL();
    char *result = setlocale(category, locale);
    if (!result) {
        return krk_runtimeError(vm.exceptions->valueError,
                                "unsupported locale setting or query failed");
    }
...
#define DO_DICT_STR(key)                                                                 \
    krk_attachNamedObject(AS_DICT(result), #key,                                         \
                          (KrkObj *)krk_copyString(lc->key, strlen(lc->key)))
...
KRK_Module(locale) {
    ...
    KRK_DOC(BIND_FUNC(module, setlocale), "@brief Set or query the C locale\n"
