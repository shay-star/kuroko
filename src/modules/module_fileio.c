...
KRK_Function(open) {
    ...
    if (argc == 1) {
        ...
        if (AS_CSTRING(argv[1])[AS_STRING(argv[1])->length - 1] == 'b') {
            KrkValue tmp = OBJECT_VAL(
                krk_copyString(AS_CSTRING(argv[1]), AS_STRING(argv[1])->length - 1));
            krk_push(tmp);
            isBinary = 1;
        } else {
            krk_push(arg);
    ...
    if (!file)
        return krk_runtimeError(vm.exceptions->ioError,
                                "open: failed to open file; system returned: %s",
                                strerror(errno));
...
KRK_Method(File, __repr__) {
    ...
    KrkValue filename;
    KrkValue modestr;
    if (!krk_tableGet(&self->inst.fields, OBJECT_VAL(S("filename")), &filename) ||
        !IS_STRING(filename))
        return krk_runtimeError(vm.exceptions->baseException, "Corrupt File");
    if (!krk_tableGet(&self->inst.fields, OBJECT_VAL(S("modestr")), &modestr) ||
        !IS_STRING(modestr))
        return krk_runtimeError(vm.exceptions->baseException, "Corrupt File");

    return krk_stringFromFormat("<%s file '%S', mode '%S' at %p>",
                                self->filePtr ? "open" : "closed", AS_STRING(filename),
                                AS_STRING(modestr), (void *)self);
}
...
KRK_Method(File, read) {
    ...
    if (argc > 1) {
        ...
        if (sizeFromArg < -1)
            return krk_runtimeError(vm.exceptions->valueError, "size must be >= -1");
...
KRK_Method(File, __init__) {
    return krk_runtimeError(vm.exceptions->typeError,
                            "File objects can not be instantiated; use fileio.open() to "
                            "obtain File objects.");
}

KRK_Method(File, __enter__) { return NONE_VAL(); }
KRK_Method(File, __exit__) { return FUNC_NAME(File, close)(1, argv, 0); }
...
KRK_Method(BinaryFile, write) {
    ...
    return INTEGER_VAL(
        fwrite(AS_BYTES(argv[1])->bytes, 1, AS_BYTES(argv[1])->length, file));
}
...
KRK_Function(opendir) {
    ...
    if (!dir)
        return krk_runtimeError(vm.exceptions->ioError, "opendir: %s", strerror(errno));
...
KRK_Method(Directory, __call__) {
    ...
    krk_attachNamedValue(
        AS_DICT(outDict), "name",
        OBJECT_VAL(krk_copyString(entry->d_name, strlen(entry->d_name))));
...
KRK_Method(Directory, __repr__) {
    METHOD_TAKES_NONE();
    KrkValue path;
    if (!krk_tableGet(&self->inst.fields, OBJECT_VAL(S("path")), &path) ||
        !IS_STRING(path))
        return krk_runtimeError(vm.exceptions->valueError, "corrupt Directory");

    return krk_stringFromFormat("<%s directory '%S' at %p>",
                                self->dirPtr ? "open" : "closed", AS_STRING(path),
                                (void *)self);
}

KRK_Method(Directory, __enter__) { return NONE_VAL(); }
KRK_Method(Directory, __exit__) { return FUNC_NAME(Directory, close)(1, argv, 0); }

KRK_Module(fileio) {
    KRK_DOC(module, "@brief Provides access to C <stdio> buffered file I/O functions.\n\n"
    ...
    KRK_DOC(BIND_METHOD(File, readlines),
    ...
    KRK_DOC(BIND_METHOD(File, close),
    ...
    KRK_DOC(BIND_METHOD(File, __init__),
    ...
    KrkClass *Directory =
        krk_makeClass(module, &fileio_Directory, "Directory", KRK_BASE_CLASS(object));
    KRK_DOC(Directory, "Represents an opened file system directory.");
    Directory->allocSize = sizeof(struct Directory);
    Directory->_ongcsweep = _dir_sweep;
    BIND_METHOD(Directory, __repr__);
    KRK_DOC(BIND_METHOD(Directory, __iter__),
            "@brief Iterates over the contents of the directory.\n\n"
            "Each iteration returns @ref dict with two entries: <i>\"name\"</i> and "
            "<i>\"inode\"</i>.");
    KRK_DOC(BIND_METHOD(Directory, __call__),
            "@brief Yields one iteration through the directory.");
    BIND_METHOD(Directory, __enter__);
    KRK_DOC(BIND_METHOD(Directory, __exit__),
            "@brief Closes the directory upon exit from a @c with block.");
    KRK_DOC(BIND_METHOD(Directory, close),
            "@brief Close the directory.\n\nFurther reads can not be made after the "
    ...
    KRK_DOC(BIND_FUNC(module, open),
            ...
            "Opens @p path using the modestring @p mode. Supported modestring characters "
            "depend on the system implementation. "
            "If the last character of @p mode is @c 'b' a @ref BinaryFile will be "
            "returned. If the file could not be opened, "
            "an @ref IOError will be raised.");
    KRK_DOC(BIND_FUNC(module, opendir),
            "@brief Open a directory for scanning.\n"
            "@arguments path\n\n"
            "Opens the directory at @p path and returns a @ref Directory object. If @p "
            "path could not be opened or is not "
