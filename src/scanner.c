...
static int isAtEnd(const KrkScanner *scanner) { return *scanner->cur == '\0'; }
...
static KrkToken errorToken(const KrkScanner *scanner, const char *errorStr) {
    ssize_t column =
        (scanner->linePtr < scanner->start) ? scanner->start - scanner->linePtr : 0;
...
static char peek(const KrkScanner *scanner) { return *scanner->cur; }
...
static int _checkKeyword(KrkScanner *scanner, size_t start, const char *rest,
                         KrkTokenType type) {
...
#define MORE(i) (scanner->cur - scanner->start > i)
    ...
    case 'a':
        if (MORE(1))
            ...
            case 'n':
                return checkKeyword(2, "d", TOKEN_AND);
            case 'w':
                return checkKeyword(2, "ait", TOKEN_AWAIT);
            case 's':
                if (MORE(2)) {
                    switch (scanner->start[2]) {
                    case 's':
                        return checkKeyword(3, "ert", TOKEN_ASSERT);
                    case 'y':
                        return checkKeyword(3, "nc", TOKEN_ASYNC);
                    }
                    break;
                } else {
    ...
    case 'b':
        if (MORE(1))
            return checkKeyword(1, "reak", TOKEN_BREAK);
    ...
    case 'c':
        if (MORE(1))
            switch (scanner->start[1]) {
            case 'l':
                return checkKeyword(2, "ass", TOKEN_CLASS);
            case 'o':
                return checkKeyword(2, "ntinue", TOKEN_CONTINUE);
            }
        break;
    case 'd':
        if (MORE(1))
            ...
            case 'e':
                if (MORE(2))
                    switch (scanner->start[2]) {
                    case 'f':
                        return checkKeyword(3, "", TOKEN_DEF);
                    case 'l':
                        return checkKeyword(3, "", TOKEN_DEL);
                    }
                break;
            }
        break;
        ...
        if (MORE(1))
            ...
            case 'l':
                if (MORE(2))
                    switch (scanner->start[2]) {
                    case 's':
                        return checkKeyword(3, "e", TOKEN_ELSE);
                    case 'i':
                        return checkKeyword(3, "f", TOKEN_ELIF);
                    }
                break;
            case 'x':
                return checkKeyword(2, "cept", TOKEN_EXCEPT);
            }
        break;
    case 'f':
        if (MORE(1))
            ...
            case 'r':
                return checkKeyword(2, "om", TOKEN_FROM);
            }
        else if (scanner->start[1] == '\'' || scanner->start[1] == '"')
    ...
    case 'i':
        if (MORE(1))
            switch (scanner->start[1]) {
            case 'f':
                return checkKeyword(2, "", TOKEN_IF);
            case 'n':
                return checkKeyword(2, "", TOKEN_IN);
            case 'm':
                return checkKeyword(2, "port", TOKEN_IMPORT);
            case 's':
                return checkKeyword(2, "", TOKEN_IS);
            }
        break;
    case 'l':
        if (MORE(1))
            switch (scanner->start[1]) {
            case 'a':
                return checkKeyword(2, "mbda", TOKEN_LAMBDA);
            case 'e':
                return checkKeyword(2, "t", TOKEN_LET);
            }
        break;
    case 'n':
    ...
    case 'p':
        return checkKeyword(1, "ass", TOKEN_PASS);
    case 'r':
        if (MORE(1))
            switch (scanner->start[1]) {
            case 'e':
                return checkKeyword(2, "turn", TOKEN_RETURN);
            case 'a':
                return checkKeyword(2, "ise", TOKEN_RAISE);
            }
        else if (scanner->start[1] == '\'' || scanner->start[1] == '"')
    ...
    case 't':
        return checkKeyword(1, "ry", TOKEN_TRY);
    case 'T':
        return checkKeyword(1, "rue", TOKEN_TRUE);
    case 'w':
        if (MORE(1))
            switch (scanner->start[1]) {
            case 'h':
                return checkKeyword(2, "ile", TOKEN_WHILE);
            case 'i':
                return checkKeyword(2, "th", TOKEN_WITH);
            }
        break;
    case 'y':
        return checkKeyword(1, "ield", TOKEN_YIELD);
    }
    return TOKEN_IDENTIFIER;
}
...
KrkScanner krk_tellScanner(KrkScanner *scanner) { return *scanner; }

void krk_rewindScanner(KrkScanner *scanner, KrkScanner other) { *scanner = other; }
...
KrkToken krk_scanToken(KrkScanner *scanner) {
    ...
    /* Not indentation, not a linefeed on an empty line, must be not be start of line any
     * more */
    scanner->startOfLine = 0;
    ...
    case '.':
        return makeToken(scanner,
                         peek(scanner) == '.'
                             ? (peekNext(scanner, 1) == '.'
                                    ? (advance(scanner), advance(scanner), TOKEN_ELLIPSIS)
                                    : TOKEN_DOT)
                             : TOKEN_DOT);

    case ':':
        return makeToken(scanner, match(scanner, '=') ? TOKEN_WALRUS : TOKEN_COLON);
    case '!':
    ...
    case '<':
        return makeToken(scanner, match(scanner, '=')
                                      ? TOKEN_LESS_EQUAL
                                      : (match(scanner, '<')
                                             ? (match(scanner, '=') ? TOKEN_LSHIFT_EQUAL
                                                                    : TOKEN_LEFT_SHIFT)
                                             : TOKEN_LESS));
    case '>':
        return makeToken(scanner, match(scanner, '=')
                                      ? TOKEN_GREATER_EQUAL
                                      : (match(scanner, '>')
                                             ? (match(scanner, '=') ? TOKEN_RSHIFT_EQUAL
                                                                    : TOKEN_RIGHT_SHIFT)
                                             : TOKEN_GREATER));
    case '-':
        return makeToken(scanner,
    ...
    case '^':
        return makeToken(scanner, match(scanner, '=') ? TOKEN_CARET_EQUAL : TOKEN_CARET);
    case '|':
        return makeToken(scanner, match(scanner, '=') ? TOKEN_PIPE_EQUAL : TOKEN_PIPE);
    case '&':
        return makeToken(scanner,
                         match(scanner, '=') ? TOKEN_AMP_EQUAL : TOKEN_AMPERSAND);
    case '/':
        return makeToken(scanner,
                         match(scanner, '=')
                             ? TOKEN_SOLIDUS_EQUAL
                             : (match(scanner, '/')
                                    ? (match(scanner, '=') ? TOKEN_DSOLIDUS_EQUAL
    ...
    case '*':
        return makeToken(scanner,
                         match(scanner, '=')
                             ...
                             : (match(scanner, '*')
                                    ? (match(scanner, '=') ? TOKEN_POW_EQUAL : TOKEN_POW)
                                    : TOKEN_ASTERISK));
    case '%':
        return makeToken(scanner,
                         match(scanner, '=') ? TOKEN_MODULO_EQUAL : TOKEN_MODULO);
    case '@':
        return makeToken(scanner, match(scanner, '=') ? TOKEN_AT_EQUAL : TOKEN_AT);
