...
typedef struct {
    ...
    unsigned int
        eatingWhitespace; /**< @brief Depth of whitespace-ignoring parse functions. */
} Parser;
...
typedef enum {
    ...
    EXPR_ASSIGN_TARGET, /**< This expression is definitely an assignment target or chained
                           to one. */
    EXPR_DEL_TARGET,  /**< This expression is in the target list of a 'del' statement. */
    EXPR_METHOD_CALL, /**< This expression is the parameter list of a method call; only
                         used by @ref dot and @ref call */
 ...
 */
typedef struct {
    ParseFn prefix; /**< @brief Parse function to call when this token appears at the
                       start of an expression. */
    ParseFn infix;  /**< @brief Parse function to call when this token appears after an
                       expression. */
    Precedence
        precedence; /**< @brief Precedence ordering for Pratt parsing, @ref Precedence */
} ParseRule;
...
typedef struct {
    ...
    char
        isLocal; /**< @brief Flag indicating if @ref index is a local or upvalue index. */
    KrkToken name; /**< @brief Name for direct lookup. Mainly for
                      non-automatically-populated upvalue cells. */
} Upvalue;
...
struct LoopExit {
    ...
    KrkToken token; /**< @brief Token for this exit statement, so its location can be
...
typedef struct Compiler {
    struct Compiler
        *enclosing; /**< @brief Enclosing function compiler, or NULL for a module. */
    ...
    Upvalue *upvalues; /**< @brief Array of upvalue references. Count is stored in the
                          codeobject. */

    size_t loopLocalCount;   /**< @brief Tracks how many locals to pop off the stack when
                                exiting a loop. */
    size_t breakCount;       /**< @brief Number of break statements. */
    size_t breakSpace;       /**< @brief Space in breaks array. */
    struct LoopExit *breaks; /**< @brief Array of loop exit instruction indices for break
                                statements. */
    size_t continueCount;    /**< @brief Number of continue statements. */
    size_t continueSpace;    /**< @brief Space in continues array. */
    struct LoopExit *continues; /**< @brief Array of loop exit instruction indices for
                                   continue statements. */

    size_t localNameCapacity; /**< @brief How much space is available in the codeobject's
                                 local names table. */

    struct IndexWithNext
        *properties; /**< @brief Linked list of class property constant indices. */
    struct Compiler *enclosed; /**< @brief Subcompiler we are enclosing, need for type
                                  annotation compilation. */
    size_t annotationCount; /**< @brief Number of type annotations found while compiling
                               function signature. */

    int delSatisfied; /**< @brief Flag indicating if a 'del' target has been completed. */

    size_t optionsFlags; /**< @brief Special __options__ imports; similar to __future__ in
                            Python */
    int unnamedArgs;     /**< @brief Number of positional arguments that will not be
                            assignable through keywords */
} Compiler;
...
typedef struct ClassCompiler {
    ...
    int hasAnnotations; /**< @brief Flag indicating if an annotation dictionary has been
...
typedef struct GlobalState {
    ...
    ClassCompiler
        *currentClass; /**< @brief Current in-progress class definition (or NULL) */
} GlobalState;
...
static void _GlobalState_gcscan(KrkInstance *_self) {
    ...
    while (compiler != NULL) {
        if (compiler->enclosed && compiler->enclosed->codeobject)
            krk_markObject((KrkObj *)compiler->enclosed->codeobject);
        krk_markObject((KrkObj *)compiler->codeobject);
        compiler = compiler->enclosing;
    }
}

static void _GlobalState_gcsweep(KrkInstance *_self) { /* nothing to do? */ }
...
#define codeobject_from_chunk_pointer(ptr)                                               \
    ((KrkCodeObject *)((char *)(ptr)-offsetof(KrkCodeObject, chunk)))
static ChunkRecorder recordChunk(KrkChunk *in) {
    return (ChunkRecorder){in->count, in->linesCount, in->constants.count,
                           codeobject_from_chunk_pointer(in)->expressionsCount};
}
...
static void initCompiler(struct GlobalState *state, Compiler *compiler,
                         FunctionType type) {
    ...
    if (type != TYPE_MODULE) {
        state->current->codeobject->name =
            krk_copyString(state->parser.previous.start, state->parser.previous.length);
    ...
    if (isCoroutine(type))
        state->current->codeobject->obj.flags |= KRK_OBJ_FLAGS_CODEOBJECT_IS_COROUTINE;
}
...
static void complexAssignment(struct GlobalState *state, ChunkRecorder before,
                              KrkScanner oldScanner, Parser oldParser, size_t targetCount,
                              int parenthesized, size_t argBefore, size_t argAfter);
static void complexAssignmentTargets(struct GlobalState *state, KrkScanner oldScanner,
                                     Parser oldParser, size_t targetCount,
                                     int parenthesized, size_t argBefore,
                                     size_t argAfter);
static int invalidTarget(struct GlobalState *state, int exprType,
...
static void finishError(struct GlobalState *state, KrkToken *token) {
    if (!token->linePtr)
        token->linePtr = token->start;
    size_t i = 0;
    while (token->linePtr[i] && token->linePtr[i] != '\n')
        i++;

    krk_attachNamedObject(&AS_INSTANCE(krk_currentThread.currentException)->fields,
                          "line", (KrkObj *)krk_copyString(token->linePtr, i));
    krk_attachNamedObject(&AS_INSTANCE(krk_currentThread.currentException)->fields,
                          "file", (KrkObj *)currentChunk()->filename);
    krk_attachNamedValue(&AS_INSTANCE(krk_currentThread.currentException)->fields,
                         "lineno", INTEGER_VAL(token->line));
    krk_attachNamedValue(&AS_INSTANCE(krk_currentThread.currentException)->fields,
                         "colno", INTEGER_VAL(token->col));
    krk_attachNamedValue(&AS_INSTANCE(krk_currentThread.currentException)->fields,
                         "width", INTEGER_VAL(token->literalWidth));

    if (state->current->codeobject->name) {
        krk_attachNamedObject(&AS_INSTANCE(krk_currentThread.currentException)->fields,
                              "func", (KrkObj *)state->current->codeobject->name);
    } else {
        KrkValue name = NONE_VAL();
        krk_tableGet(&krk_currentThread.module->fields,
...
#    define raiseSyntaxError(token, ...)                                                 \
        do {                                                                             \
            if (state->parser.hadError)                                                  \
                break;                                                                   \
            krk_runtimeError(vm.exceptions->syntaxError, "syntax error");                \
            finishError(state, token);                                                   \
        } while (0)
#else
#    define raiseSyntaxError(token, ...)                                                 \
        do {                                                                             \
            if (state->parser.hadError)                                                  \
                break;                                                                   \
            krk_runtimeError(vm.exceptions->syntaxError, __VA_ARGS__);                   \
            finishError(state, token);                                                   \
        } while (0)
#endif
...
static void _advance(struct GlobalState *state) {
    ...
    for (;;) {
        ...
        if (state->parser.eatingWhitespace &&
            (state->parser.current.type == TOKEN_INDENTATION ||
             state->parser.current.type == TOKEN_EOL))
            continue;

        if (state->parser.current.type == TOKEN_RETRY)
            continue;
...
static void _startEatingWhitespace(struct GlobalState *state) {
    ...
    if (state->parser.current.type == TOKEN_INDENTATION ||
        state->parser.current.type == TOKEN_EOL)
        advance();
}
...
static void _stopEatingWhitespace(struct GlobalState *state) {
    if (state->parser.eatingWhitespace == 0) {
        error("Internal scanner error: Invalid nesting of "
              "`startEatingWhitespace`/`stopEatingWhitespace` calls.");
    }
    state->parser.eatingWhitespace--;
}
...
static void _consume(struct GlobalState *state, KrkTokenType type, const char *message) {
    ...
    if (state->parser.current.type == TOKEN_EOL ||
        state->parser.current.type == TOKEN_EOF) {
...
static KrkCodeObject *endCompiler(struct GlobalState *state) {
    ...
    function->localNames = KRK_GROW_ARRAY(
        KrkLocalEntry, function->localNames, state->current->localNameCapacity,
        function->localNameCount); /* Shorten this down for runtime */

    if (state->current->continueCount) {
        state->parser.previous = state->current->continues[0].token;
        error("continue without loop");
    }
    if (state->current->breakCount) {
        state->parser.previous = state->current->breaks[0].token;
    ...
    function->chunk.lines =
        KRK_GROW_ARRAY(KrkLineMap, function->chunk.lines, function->chunk.linesCapacity,
                       function->chunk.linesCount);
    function->chunk.linesCapacity = function->chunk.linesCount;
    function->chunk.code = KRK_GROW_ARRAY(
        uint8_t, function->chunk.code, function->chunk.capacity, function->chunk.count);
    function->chunk.capacity = function->chunk.count;

    function->expressions =
        KRK_GROW_ARRAY(KrkExpressionsMap, function->expressions,
    ...
    for (int i = 0; i < function->potentialPositionals; ++i) {
        ...
        KrkValue value = OBJECT_VAL(krk_copyString(
            state->current->locals[i].name.start, state->current->locals[i].name.length));
        krk_push(value);
    ...
    if (function->obj.flags & KRK_OBJ_FLAGS_CODEOBJECT_COLLECTS_ARGS) {
        KrkValue value =
            OBJECT_VAL(krk_copyString(state->current->locals[args].name.start,
    ...
    for (int i = 0; i < function->keywordArgs; ++i) {
        KrkValue value =
            OBJECT_VAL(krk_copyString(state->current->locals[i + args].name.start,
    ...
    function->totalArguments =
        function->potentialPositionals + function->keywordArgs +
        !!(function->obj.flags & KRK_OBJ_FLAGS_CODEOBJECT_COLLECTS_ARGS) +
        !!(function->obj.flags & KRK_OBJ_FLAGS_CODEOBJECT_COLLECTS_KWS);

#if !defined(KRK_NO_DISASSEMBLY) && !defined(KRK_DISABLE_DEBUG)
    if ((krk_currentThread.flags & KRK_THREAD_ENABLE_DISASSEMBLY) &&
        !state->parser.hadError) {
        krk_disassembleCodeObject(stderr, function,
                                  function->name ? function->name->chars : "(module)");
    }
#endif
...
static int isMangleable(const char *name, size_t len) {
    return (len > 2 && name[0] == '_' && name[1] == '_' && name[len - 1] != '_' &&
            (len < 4 || name[len - 2] != '_'));
}
...
static ssize_t identifierConstant(struct GlobalState *state, KrkToken *name) {
    if (state->currentClass && isMangleable(name->start, name->length)) {
        ...
        krk_pushStringBuilderFormat(&sb, "_%.*s%.*s", (int)classLength, className,
                                    (int)name->length, name->start);

        return krk_addConstant(currentChunk(), krk_finishStringBuilder(&sb));
    }

    return krk_addConstant(currentChunk(),
                           OBJECT_VAL(krk_copyString(name->start, name->length)));
}
...
static ssize_t resolveLocal(struct GlobalState *state, Compiler *compiler,
                            KrkToken *name) {
...
static size_t renameLocal(struct GlobalState *state, size_t ind, KrkToken name) {
    if (state->current->codeobject->localNameCount + 1 >
        state->current->localNameCapacity) {
        size_t old = state->current->localNameCapacity;
        state->current->localNameCapacity = KRK_GROW_CAPACITY(old);
        state->current->codeobject->localNames =
            KRK_GROW_ARRAY(KrkLocalEntry, state->current->codeobject->localNames, old,
                           state->current->localNameCapacity);
    }
    state->current->codeobject->localNames[state->current->codeobject->localNameCount]
        .id = ind;
    state->current->codeobject->localNames[state->current->codeobject->localNameCount]
        .birthday = currentChunk()->count;
    state->current->codeobject->localNames[state->current->codeobject->localNameCount]
        .deathday = 0;
    state->current->codeobject->localNames[state->current->codeobject->localNameCount]
        .name = krk_copyString(name.start, name.length);
...
static size_t addLocal(struct GlobalState *state, KrkToken name) {
    if (state->current->localCount + 1 > state->current->localsSpace) {
        ...
        state->current->locals = KRK_GROW_ARRAY(Local, state->current->locals, old,
                                                state->current->localsSpace);
    }
...
static void declareVariable(struct GlobalState *state) {
    ...
    for (ssize_t i = state->current->localCount - 1; i >= 0; i--) {
        ...
        if (local->depth != -1 && local->depth < (ssize_t)state->current->scopeDepth)
            break;
        if (identifiersEqual(name, &local->name)) {
            error("Duplicate definition for local '%.*s' in this scope.",
...
static ssize_t parseVariable(struct GlobalState *state, const char *errorMessage) {
    ...
    if ((state->current->optionsFlags & OPTIONS_FLAG_COMPILE_TIME_BUILTINS) &&
        *state->parser.previous.start != '_') {
        KrkValue value;
        if (krk_tableGet_fast(&vm.builtins->fields,
                              krk_copyString(state->parser.previous.start,
                                             state->parser.previous.length),
                              &value)) {
            error("Conflicting declaration of global '%.*s' is invalid when "
                  "'compile_time_builtins' is enabled.",
...
static void markInitialized(struct GlobalState *state) {
    if (state->current->scopeDepth == 0)
        return;
    state->current->locals[state->current->localCount - 1].depth =
...
static void _emitOverlongJump(struct GlobalState *state, int offset, int jump) {
    ...
    size_t i = 0;
    while (i < co->overlongJumpsCount &&
           co->overlongJumps[i].instructionOffset != (uint32_t)offset)
        i++;
    if (i == co->overlongJumpsCount) {
        ...
        if (co->overlongJumpsCount + 1 > co->overlongJumpsCapacity) {
            ...
            co->overlongJumps = KRK_GROW_ARRAY(KrkOverlongJump, co->overlongJumps, old,
                                               co->overlongJumpsCapacity);
        }
 ...
 */
static void writeExpressionLocation(KrkToken *before, KrkToken *after, KrkToken *current,
                                    struct GlobalState *state) {
#ifndef KRK_DISABLE_DEBUG
    /* We can only support the display of underlines when the whole expression is on one
     * line. */
    if (after->linePtr != before->linePtr)
        return;
    if (after->line != state->parser.previous.line)
        return;
    ...
    if (before->col > 255 || current->col > 255 ||
        ...
        (after->col + after->literalWidth) > 255)
        return;

    krk_debug_addExpression(state->current->codeobject, before->col, current->col,
...
static void compareChained(struct GlobalState *state, int inner, KrkToken *preceding) {
    ...
    if (operatorType == TOKEN_NOT)
        consume(TOKEN_IN, "'in' must follow infix 'not'");
    ...
    default:
        error("Invalid binary comparison operator?");
        break;
    }
...
static void binary(struct GlobalState *state, int exprType, RewindState *rewind) {
    ...
    ParseRule *rule = getRule(operatorType);
    KrkToken this = state->parser.previous;
    parsePrecedence(state,
                    (Precedence)(rule->precedence + (rule->precedence != PREC_EXPONENT)));
    invalidTarget(state, exprType, "operator");
    ...
    case TOKEN_LEFT_SHIFT:
        emitByte(OP_SHIFTLEFT);
        break;
    case TOKEN_RIGHT_SHIFT:
        emitByte(OP_SHIFTRIGHT);
        break;
    ...
    writeExpressionLocation(&rewind->oldParser.current, &state->parser.previous, &this,
                            state);
}

static int matchAssignment(struct GlobalState *state) {
    return (state->parser.current.type >= TOKEN_EQUAL &&
            state->parser.current.type <= TOKEN_MODULO_EQUAL)
               ? (advance(), 1)
               : 0;
}

static int checkEndOfDel(struct GlobalState *state) {
    if (check(TOKEN_COMMA) || check(TOKEN_EOL) || check(TOKEN_EOF) ||
...
static int matchComplexEnd(struct GlobalState *state) {
    return match(TOKEN_COMMA) || match(TOKEN_EQUAL) || match(TOKEN_RIGHT_PAREN);
}
...
static void assignmentValue(struct GlobalState *state) {
    ...
    case TOKEN_PIPE_EQUAL:
        emitByte(OP_INPLACE_BITOR);
        break;
    case TOKEN_CARET_EQUAL:
        emitByte(OP_INPLACE_BITXOR);
        break;
    case TOKEN_AMP_EQUAL:
        emitByte(OP_INPLACE_BITAND);
        break;
    case TOKEN_LSHIFT_EQUAL:
        emitByte(OP_INPLACE_SHIFTLEFT);
        break;
    case TOKEN_RSHIFT_EQUAL:
        emitByte(OP_INPLACE_SHIFTRIGHT);
        break;

    case TOKEN_PLUS_EQUAL:
        emitByte(OP_INPLACE_ADD);
        break;
    case TOKEN_PLUS_PLUS:
        emitByte(OP_INPLACE_ADD);
        break;
    case TOKEN_MINUS_EQUAL:
        emitByte(OP_INPLACE_SUBTRACT);
        break;
    case TOKEN_MINUS_MINUS:
        emitByte(OP_INPLACE_SUBTRACT);
        break;
    case TOKEN_ASTERISK_EQUAL:
        emitByte(OP_INPLACE_MULTIPLY);
        break;
    case TOKEN_POW_EQUAL:
        emitByte(OP_INPLACE_POW);
        break;
    case TOKEN_SOLIDUS_EQUAL:
        emitByte(OP_INPLACE_DIVIDE);
        break;
    case TOKEN_DSOLIDUS_EQUAL:
        emitByte(OP_INPLACE_FLOORDIV);
        break;
    case TOKEN_MODULO_EQUAL:
        emitByte(OP_INPLACE_MODULO);
        break;
    case TOKEN_AT_EQUAL:
        emitByte(OP_INPLACE_MATMUL);
        break;
...
static void dot(struct GlobalState *state, int exprType, RewindState *rewind) {
    ...
    if (exprType == EXPR_ASSIGN_TARGET) {
        if (matchComplexEnd(state)) {
            ...
            writeExpressionLocation(&rewind->oldParser.current, &state->parser.previous,
                                    &name, state);
            emitByte(OP_POP);
            return;
        }
...
static void letDeclaration(struct GlobalState *state) {
    ...
    if (state->current->scopeDepth == 0) {
        ...
        for (size_t i = 0; i < argCount; i++) {
            state->current->locals[state->current->localCount - 1 - i].depth =
                state->current->scopeDepth;
        }
    }
...
static void beginScope(struct GlobalState *state) { state->current->scopeDepth++; }
...
static void endScope(struct GlobalState *state) {
    ...
    while (state->current->localCount > 0 &&
           state->current->locals[state->current->localCount - 1].depth >
               (ssize_t)state->current->scopeDepth) {
        if (state->current->locals[state->current->localCount - 1].isCaptured) {
            if (popCount) {
                if (popCount == 1)
                    emitByte(OP_POP);
                else {
                    EMIT_OPERAND_OP(OP_POP_MANY, popCount);
                }
                popCount = 0;
            }
            closeCount++;
            ...
            if (closeCount) {
                if (closeCount == 1)
                    ...
                    EMIT_OPERAND_OP(OP_CLOSE_MANY, closeCount);
                }
                closeCount = 0;
            }
            popCount++;
        ...
        for (size_t i = 0; i < state->current->codeobject->localNameCount; i++) {
            if (state->current->codeobject->localNames[i].id ==
                    state->current->localCount - 1 &&
                state->current->codeobject->localNames[i].deathday == 0) {
                state->current->codeobject->localNames[i].deathday =
                    (size_t)currentChunk()->count;
            }
        }
    ...
    if (match(TOKEN_EOL)) {
        if (check(TOKEN_INDENTATION)) {
            ...
            if (currentIndentation <= indentation)
                return;
            advance();
            if (!strcmp(blockName, "def") &&
                (match(TOKEN_STRING) || match(TOKEN_BIG_STRING))) {
                size_t before = currentChunk()->count;
                string(state, EXPR_NORMAL, NULL);
                /* That wrote to the chunk, rewind it; this should only ever go back two
                 * bytes because this should only happen as the first thing in a function
                 * definition, and thus this _should_ be the first constant and thus
                 * opcode + one-byte operand to OP_CONSTANT, but just to be safe we'll
                 * actually use the previous offset... */
                currentChunk()->count = before;
                /* Retreive the docstring from the constant table */
                state->current->codeobject->docstring = AS_STRING(
                    currentChunk()
                        ->constants.values[currentChunk()->constants.count - 1]);
                consume(TOKEN_EOL, "Garbage after docstring defintion");
                if (!check(TOKEN_INDENTATION) ||
                    state->parser.current.length != currentIndentation) {
            ...
            while (check(TOKEN_INDENTATION)) {
                if (state->parser.current.length < currentIndentation)
                    break;
                advance();
                declaration(state);
                if (check(TOKEN_EOL)) {
...
static void doUpvalues(struct GlobalState *state, Compiler *compiler,
    ...
    do {
        if (!(state->current->optionsFlags & OPTIONS_FLAG_NO_IMPLICIT_SELF) &&
            ...
            if (hasCollectors || state->current->codeobject->requiredArgs != 1) {
                errorAtCurrent("Argument name 'self' in a method signature is reserved "
                               "for the implicit first argument.");
                return 1;
            }
            advance();
        ...
        if (match(TOKEN_SOLIDUS)) {
            if (hasCollectors || state->current->unnamedArgs ||
                !state->current->codeobject->potentialPositionals) {
                error("Syntax error.");
                return 1;
            }
            state->current->unnamedArgs =
                state->current->codeobject->potentialPositionals;
            continue;
        }
        if (match(TOKEN_ASTERISK) || check(TOKEN_POW)) {
            if (match(TOKEN_POW)) {
                ...
                hasCollectors = 2;
                state->current->codeobject->obj.flags |=
            ...
            ssize_t paramConstant = parseVariable(
                state, (hasCollectors == 1) ? "Expected parameter name after '*'."
            ...
            KrkToken name = state->parser.previous;
            if (!(state->current->optionsFlags & OPTIONS_FLAG_NO_IMPLICIT_SELF) &&
                isMethod(type) && identifiersEqual(&name, &self)) {
                errorAtCurrent("Argument name 'self' in a method signature is reserved "
                               "for the implicit first argument.");
                return 1;
            }
...
static void classBody(struct GlobalState *state, size_t blockWidth) {
    ...
    if (check(TOKEN_AT)) {
        ...
        static struct CompilerSpecialMethod {
            const char *name;
            int type;
        } compilerSpecialMethods[] = {{"__init__", TYPE_INIT},
                                      {"__class_getitem__", TYPE_CLASSMETHOD},
                                      {"__init_subclass__", TYPE_CLASSMETHOD},
                                      {"__new__", TYPE_STATIC},
                                      {NULL, 0}};

        for (struct CompilerSpecialMethod *method = compilerSpecialMethods; method->name;
             method++) {
            if (state->parser.previous.length == strlen(method->name) &&
                memcmp(state->parser.previous.start, method->name,
                       strlen(method->name)) == 0) {
...
static size_t addUpvalue(struct GlobalState *state, Compiler *compiler, ssize_t index,
                         int isLocal, KrkToken name);
static KrkToken classDeclaration(struct GlobalState *state) {
    size_t blockWidth = (state->parser.previous.type == TOKEN_INDENTATION)
                            ? state->parser.previous.length
                            : 0;
    ...
    subcompiler.codeobject->chunk.filename =
        subcompiler.enclosing->codeobject->chunk.filename;
    ...
    RewindState parameters = {recordChunk(currentChunk()),
                              krk_tellScanner(&state->scanner), state->parser};
...
_pop_class:
    ...
    RewindState afterFunction = {recordChunk(currentChunk()),
                                 krk_tellScanner(&state->scanner), state->parser};
...
static void lambda(struct GlobalState *state, int exprType, RewindState *rewind) {
    ...
    lambdaCompiler.codeobject->chunk.filename =
        lambdaCompiler.enclosing->codeobject->chunk.filename;
    beginScope(state);

    if (!check(TOKEN_COLON)) {
        if (argumentList(state, TYPE_LAMBDA))
            goto _bail;
    }
...
static void asyncDeclaration(struct GlobalState *state, int declarationLevel) {
    ...
    if (match(TOKEN_DEF)) {
        ...
        ssize_t global =
            parseVariable(state, "Expected coroutine name after 'async def'");
        if (state->parser.hadError)
            return;
        markInitialized(state);
...
static KrkToken decorator(struct GlobalState *state, size_t level, FunctionType type) {
    ...
    if (type == TYPE_METHOD) {
        if (identifiersEqual(&at_staticmethod, &state->parser.current))
            type = TYPE_STATIC;
        if (identifiersEqual(&at_classmethod, &state->parser.current))
    ...
    if (blockWidth) {
        consume(TOKEN_INDENTATION,
                "Expected next line after decorator to have same indentation.");
        if (state->parser.previous.length != blockWidth)
            error("Expected next line after decorator to have same indentation.");
    }
    ...
    if (check(TOKEN_DEF)) {
        ...
        if (type == TYPE_METHOD && funcName.length == 8 &&
            !memcmp(funcName.start, "__init__", 8)) {
            type = TYPE_INIT;
        }
        ...
        function(state, type == TYPE_METHOD ? TYPE_COROUTINE_METHOD : TYPE_COROUTINE,
                 blockWidth);
    } else if (check(TOKEN_AT)) {
...
static void emitLoop(struct GlobalState *state, int loopStart, uint8_t loopType) {
    ...
    /* Patch continue statements to point to here, before the loop operation (yes that's
     * silly) */
    while (state->current->continueCount > 0 &&
           state->current->continues[state->current->continueCount - 1].offset >
               loopStart) {
    ...
    int offset =
        currentChunk()->count - loopStart + ((loopType == OP_LOOP_ITER) ? -1 : 2);
    if (offset > 0xFFFF) {
...
static void ifStatement(struct GlobalState *state) {
    ...
    if (blockWidth == 0 ||
        (check(TOKEN_INDENTATION) && (state->parser.current.length == blockWidth))) {
        /* This is complicated */
        KrkToken previous;
        if (blockWidth) {
...
static void patchBreaks(struct GlobalState *state, int loopStart) {
    ...
    while (state->current->breakCount > 0 &&
           state->current->breaks[state->current->breakCount - 1].offset > loopStart) {
...
static void breakStatement(struct GlobalState *state) {
    if (state->current->breakSpace < state->current->breakCount + 1) {
        ...
        state->current->breaks = KRK_GROW_ARRAY(struct LoopExit, state->current->breaks,
                                                old, state->current->breakSpace);
    }
    ...
    state->current->breaks[state->current->breakCount++] =
        (struct LoopExit){emitJump(OP_JUMP), state->parser.previous};
}
...
static void continueStatement(struct GlobalState *state) {
    if (state->current->continueSpace < state->current->continueCount + 1) {
        ...
        state->current->continues =
            KRK_GROW_ARRAY(struct LoopExit, state->current->continues, old,
                           state->current->continueSpace);
    }
    ...
    state->current->continues[state->current->continueCount++] =
        (struct LoopExit){emitJump(OP_JUMP), state->parser.previous};
}
...
static void whileStatement(struct GlobalState *state) {
    ...
    RewindState rewind = {recordChunk(currentChunk()), krk_tellScanner(&state->scanner),
                          state->parser};
    if (!(match(TOKEN_TRUE) && match(TOKEN_COLON)) &&
        !(match(TOKEN_NUMBER) &&
          (state->parser.previous.length == 1 && *state->parser.previous.start == '1') &&
          match(TOKEN_COLON))) {
...
_anotherExcept:
    ...
    if (blockWidth == 0 ||
        ...
        if (exitJumps && !firstJump && match(TOKEN_EXCEPT)) {
            ...
            /* Remove scoped name */
            if (nameInd)
                state->current->codeobject->localNames[nameInd].deathday =
                    (size_t)currentChunk()->count;
...
static size_t importModule(struct GlobalState *state, KrkToken *startOfName,
                           int leadingDots) {
    size_t ind = 0;
    struct StringBuilder sb = {0};
    ...
    if (!(leadingDots && check(TOKEN_IMPORT))) {
        ...
        if (state->parser.hadError)
            goto _freeImportName;
        pushStringBuilderStr(&sb, state->parser.previous.start,
                             state->parser.previous.length);
...
static void fromImportStatement(struct GlobalState *state) {
    ...
    do {
        ...
        emitBytes(OP_DUP,
                  0); /* Duplicate the package object so we can GET_PROPERTY on it? */
        ...
        if (state->current->scopeDepth) {
            /* Swaps the original module and the new possible local so it can be in the
             * right place */
            emitByte(OP_SWAP);
        }
        declareVariable(state);
...
static void errorAfterStatement(struct GlobalState *state) {
    ...
    case TOKEN_RIGHT_SQUARE:
        errorAtCurrent("Unmatched '%.*s'", (int)state->parser.current.length,
                       state->parser.current.start);
        break;
...
static void yield(struct GlobalState *state, int exprType, RewindState *rewind) {
    if (state->current->type == TYPE_MODULE || state->current->type == TYPE_INIT ||
    ...
    if (match(TOKEN_FROM)) {
        ...
        patchJump(exitJump);
    } else if (check(TOKEN_EOL) || check(TOKEN_EOF) || check(TOKEN_RIGHT_PAREN) ||
...
static int _pushHex(struct GlobalState *state, int isBytes, struct StringBuilder *sb,
                    const char *c, const char *end, size_t n, char type) {
    char tmpbuf[10] = {0};
...
#define PUSH_HEX(n, type) _pushHex(state, isBytes, &sb, c, end, n, type)
    ...
    if ((isBytes || isFormat || isRaw) &&
        !(match(TOKEN_STRING) || match(TOKEN_BIG_STRING))) {
        error("Expected string after prefix? (Internal error - scanner should not have "
              "produced this.)");
        return;
    }
    ...
    do {
        if (isRaw) {
            for (size_t i = 0;
                 i < state->parser.previous.length -
                         (state->parser.previous.type == TOKEN_BIG_STRING ? 6 : 2);
                 ++i) {
                PUSH_CHAR(
                    state->parser.previous
                        .start[(state->parser.previous.type == TOKEN_BIG_STRING ? 3 : 1) +
                               i]);
            }
            goto _nextStr;
        }
        ...
        const char *end =
            state->parser.previous.start + state->parser.previous.length - type;
        while (c < end) {
            if (*c == '\\') {
                switch (c[1]) {
                case '\\':
                ...
                Parser parserBefore = state->parser;
                KrkScanner inner = (KrkScanner){.start = c + 1,
                                                .cur = c + 1,
                                                .linePtr = lineBefore,
                                                .line = lineNo,
                                                .startOfLine = 0,
                ...
                advance();
                parsePrecedence(
                    state, PREC_COMMA); /* allow unparen'd tuples, but not assignments, as
                                           expressions in f-strings */
                if (state->parser.hadError)
                    goto _cleanupError;
                inner = krk_tellScanner(
                    &state->scanner); /* To figure out how far to advance c */
                krk_rewindScanner(&state->scanner,
                                  beforeExpression); /* To get us back to where we were
                                                        with a string token */
                state->parser = parserBefore;
                c = inner.start;
                ...
                if (*c == '!') {
                    ...
                    if (*c == 'r') {
                        ...
                        error("Unsupported conversion flag '%c' for f-string expression.",
                              *c);
                        goto _cleanupError;
                    }
                    c++;
                ...
                if (*c == ':') {
                    ...
                    while (c < end && *c != '}')
                        c++;
                    emitConstant(
                        OBJECT_VAL(krk_copyString(formatStart, c - formatStart)));
                    formatType |= FORMAT_OP_FORMAT;
                }

                /* Default to !r if '=' was present but neither was specified. */
                if (!(formatType & (FORMAT_OP_FORMAT | FORMAT_OP_STR)) &&
                    (formatType & FORMAT_OP_EQ)) {
                    formatType |= FORMAT_OP_REPR;
                }
    ...
    } while ((!isBytes || match(TOKEN_PREFIX_B)) &&
             (match(TOKEN_STRING) || match(TOKEN_BIG_STRING)));
...
static size_t addUpvalue(struct GlobalState *state, Compiler *compiler, ssize_t index,
    ...
    if (upvalueCount + 1 > compiler->upvaluesSpace) {
        ...
        compiler->upvalues =
            KRK_GROW_ARRAY(Upvalue, compiler->upvalues, old, compiler->upvaluesSpace);
    }
...
static ssize_t resolveUpvalue(struct GlobalState *state, Compiler *compiler,
                              KrkToken *name) {
    size_t upvalueCount = compiler->codeobject->upvalueCount;
    for (size_t i = 0; i < upvalueCount; ++i) {
...
static void namedVariable(struct GlobalState *state, KrkToken name, int exprType) {
    if (state->current->type == TYPE_CLASS) {
        ...
        while (properties) {
            KrkString *constant =
                AS_STRING(currentChunk()->constants.values[properties->ind]);
            if (constant->length == name.length &&
                !memcmp(constant->chars, name.start, name.length)) {
    ...
    if (arg != -1) {
        ...
        if ((state->current->optionsFlags & OPTIONS_FLAG_COMPILE_TIME_BUILTINS) &&
            *name.start != '_') {
            KrkValue value;
            if (krk_tableGet_fast(&vm.builtins->fields,
                                  krk_copyString(name.start, name.length), &value)) {
                if ((exprType == EXPR_ASSIGN_TARGET && matchComplexEnd(state)) ||
                    ...
                    error("Can not assign to '%.*s' when 'compile_time_builtins' is "
                          "enabled.",
                          (int)name.length, name.start);
                } else if (exprType == EXPR_DEL_TARGET && checkEndOfDel(state)) {
                    error(
...
static void super_(struct GlobalState *state, int exprType, RewindState *rewind) {
    ...
    if (match(TOKEN_RIGHT_PAREN)) {
        if (!isMethod(state->current->type) &&
            !isClassOrStaticMethod(state->current->type)) {
...
static void comprehensionInner(struct GlobalState *state, KrkScanner scannerBefore,
                               Parser parserBefore,
                               void (*body)(struct GlobalState *, size_t), size_t arg) {
    ssize_t loopInd = state->current->localCount;
    ssize_t varCount = 0;
    int sawComma = 0;
    if (!check(TOKEN_IDENTIFIER)) {
    ...
    do {
        if (!check(TOKEN_IDENTIFIER))
            break;
        defineVariable(state,
                       parseVariable(state, "Expected name for iteration variable."));
        if (state->parser.hadError)
            return;
        emitByte(OP_NONE);
 ...
 */
static void generatorExpression(struct GlobalState *state, KrkScanner scannerBefore,
                                Parser parserBefore,
                                void (*body)(struct GlobalState *, size_t)) {
 ...
 */
static void comprehensionExpression(struct GlobalState *state, KrkScanner scannerBefore,
                                    Parser parserBefore,
                                    void (*body)(struct GlobalState *, size_t),
                                    int type) {
    Compiler subcompiler;
...
static size_t finishStarComma(struct GlobalState *state, size_t arg, size_t *argBefore,
                              size_t *argAfter) {
    *argBefore = arg;
    *argAfter = 1;
    ...
    while (match(TOKEN_COMMA)) {
        if (!getRule(state->parser.current.type)->prefix)
            break;
        ...
        if (match(TOKEN_FOR)) {
            ...
            if (!check(TOKEN_RIGHT_PAREN)) {
                ...
                do {
                    if (match(TOKEN_ASTERISK)) {
                        argCount =
                            finishStarComma(state, argCount, &argBefore, &argAfter);
                        goto _done;
                    }
                    expression(state);
                    argCount++;
...
_done:
    ...
    if (!match(TOKEN_RIGHT_PAREN)) {
        switch (state->parser.current.type) {
        case TOKEN_EQUAL:
            error("Assignment value expression must be enclosed in parentheses.");
            break;
        default:
    ...
    } else if (exprType == EXPR_ASSIGN_TARGET &&
               (check(TOKEN_EQUAL) || check(TOKEN_COMMA) || check(TOKEN_RIGHT_PAREN))) {
        if (!argCount) {
            error("Can not assign to empty target list.");
        } else if (!maybeValidAssignment) {
            error("Can not assign to generator expression.");
...
static void list(struct GlobalState *state, int exprType, RewindState *rewind) {
    ...
    if (!check(TOKEN_RIGHT_SQUARE)) {
        ...
        if (match(TOKEN_FOR)) {
            ...
            comprehensionExpression(state, scannerBefore, parserBefore, listInner,
                                    OP_MAKE_LIST);
        } else {
            size_t argCount = 1;
            while (match(TOKEN_COMMA) && !check(TOKEN_RIGHT_SQUARE)) {
...
static void dict(struct GlobalState *state, int exprType, RewindState *rewind) {
    ...
    if (!check(TOKEN_RIGHT_BRACE)) {
        ...
        if (check(TOKEN_COMMA) || check(TOKEN_RIGHT_BRACE)) {
            ...
            comprehensionExpression(state, scannerBefore, parserBefore, setInner,
                                    OP_MAKE_SET);
        } else {
            ...
            if (match(TOKEN_FOR)) {
                ...
                comprehensionExpression(state, scannerBefore, parserBefore, dictInner,
                                        OP_MAKE_DICT);
            } else {
                /*
...
static void complexAssignmentTargets(struct GlobalState *state, KrkScanner oldScanner,
    ...
    do {
        ...
        if (checkTargetCount == targetCount &&
            state->parser.previous.type == TOKEN_COMMA) {
...
static void comma(struct GlobalState *state, int exprType, RewindState *rewind) {
    ...
    do {
        ...
        if (match(TOKEN_ASTERISK)) {
            expressionCount =
                finishStarComma(state, expressionCount, &argBefore, &argAfter);
            goto _maybeassign;
            return;
        }
        expressionCount++;
...
_maybeassign:
    (void)0;
    if (exprType == EXPR_CAN_ASSIGN && match(TOKEN_EQUAL)) {
        complexAssignment(state, rewind->before, rewind->oldScanner, rewind->oldParser,
                          expressionCount, 0, argBefore, argAfter);
    }
}
...
static void pstar(struct GlobalState *state, int exprType, RewindState *rewind) {
    ...
    if (exprType == EXPR_CAN_ASSIGN && match(TOKEN_EQUAL)) {
        complexAssignment(state, rewind->before, rewind->oldScanner, rewind->oldParser,
                          totalArgs, 0, argBefore, argAfter);
    }
}
        ...
        do {
            ...
            if (match(TOKEN_IDENTIFIER)) {
                ...
                if (check(TOKEN_EQUAL)) {
                    ...
                    /*
                     * This is a regular argument that happened to start with an
                     * identifier, roll it back so we can process it that way.
                     */
                    krk_ungetToken(&state->scanner, state->parser.current);
...
static void parsePrecedence(struct GlobalState *state, Precedence precedence) {
    ...
    if (prefixRule == pstar && precedence > PREC_COMMA)
        prefixRule = NULL;
    ...
    if (prefixRule == NULL) {
        ...
        case TOKEN_RIGHT_PAREN:
        case TOKEN_RIGHT_SQUARE:
            error("Unmatched '%.*s'", (int)state->parser.previous.length,
    ...
    int exprType = 0;
    if (precedence <= PREC_ASSIGNMENT || precedence == PREC_CAN_ASSIGN)
        exprType = EXPR_CAN_ASSIGN;
    if (precedence == PREC_MUST_ASSIGN)
        exprType = EXPR_ASSIGN_TARGET;
    if (precedence == PREC_DEL_TARGET)
        exprType = EXPR_DEL_TARGET;
    ...
    while (precedence <= getRule(state->parser.current.type)->precedence) {
        ...
        if (exprType == EXPR_ASSIGN_TARGET &&
            (state->parser.previous.type == TOKEN_COMMA ||
             state->parser.previous.type == TOKEN_EQUAL))
            break;
        advance();
...
_noexport void _createAndBind_compilerClass(void) {
    KrkClass *CompilerState = ADD_BASE_CLASS(KRK_BASE_CLASS(CompilerState),
                                             "CompilerState", KRK_BASE_CLASS(object));
...
static ParseRule *getRule(KrkTokenType type) { return &krk_parseRules[type]; }
