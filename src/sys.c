...
KRK_Function(set_tracing) {
    ...
    if (!krk_parseArgs("|$pp", (const char *[]){"tracing", "disassembly"}, &tracing,
                       &disassembly)) {
        return NONE_VAL();
    }

#    define SET_THREAD(arg, flag)                                                        \
        do {                                                                             \
            if (arg != -1) {                                                             \
                if (arg)                                                                 \
                    krk_currentThread.flags |= KRK_THREAD_ENABLE_##flag;                 \
                else                                                                     \
                    krk_currentThread.flags &= ~KRK_THREAD_ENABLE_##flag;                \
            }                                                                            \
        } while (0)
...
KRK_Function(getsizeof) {
    ...
    case KRK_OBJ_STRING: {
        ...
        if (self->codes && self->chars != self->codes) {
            if ((self->obj.flags & KRK_OBJ_FLAGS_STRING_MASK) <=
                ...
                mySize += self->codesLength;
            else if ((self->obj.flags & KRK_OBJ_FLAGS_STRING_MASK) ==
                     KRK_OBJ_FLAGS_STRING_UCS2)
                mySize += 2 * self->codesLength;
            else if ((self->obj.flags & KRK_OBJ_FLAGS_STRING_MASK) ==
                     KRK_OBJ_FLAGS_STRING_UCS4)
                mySize += 4 * self->codesLength;
        }
        break;
    }
    ...
    case KRK_OBJ_CLOSURE: {
        ...
        mySize +=
            sizeof(KrkClosure) + sizeof(KrkUpvalue *) * self->function->upvalueCount;
        break;
    }
    ...
    case KRK_OBJ_INSTANCE: {
        ...
        if (krk_isInstanceOf(argv[0], vm.baseClasses->listClass)) {
            ...
            mySize +=
                (sizeof(KrkTableEntry) + sizeof(ssize_t)) * AS_DICT(argv[0])->capacity;
        }
        break;
    }
...
KRK_Function(importmodule) {
    ...
    if (!krk_doRecursiveModuleLoad(AS_STRING(argv[0])))
        return NONE_VAL(); /* ImportError already raised */
    return krk_pop();
}
...
KRK_Function(members) {
    KrkValue val;
    if (!krk_parseArgs("V", (const char *[]){"obj"}, &val))
        return NONE_VAL();
...
KRK_Function(set_recursion_depth) {
    ...
    int quiet = 0;
    if (!krk_parseArgs("I|p", (const char *[]){"maxdepth", "quiet"}, &maxdepth, &quiet))
        return NONE_VAL();
...
#define STR(x) STR_(x)
    krk_attachNamedObject(
        &vm.system->fields, "version",
        (KrkObj *)S(STR(KRK_VERSION_MAJOR) "." STR(KRK_VERSION_MINOR) "." STR(
            KRK_VERSION_PATCH) KRK_VERSION_EXTRA));
    krk_attachNamedObject(&vm.system->fields, "buildenv",
                          (KrkObj *)S(KRK_BUILD_COMPILER));
    krk_attachNamedObject(&vm.system->fields, "builddate", (KrkObj *)S(KRK_BUILD_DATE));
    krk_attachNamedValue(&vm.system->fields, "hexversion",
                         INTEGER_VAL((KRK_VERSION_MAJOR << 24) |
                                     (KRK_VERSION_MINOR << 16) |
                                     (KRK_VERSION_PATCH << 8) | (KRK_VERSION_LEVEL << 4) |
                                     (KRK_VERSION_SERIAL)));
    ...
    KRK_DOC(BIND_FUNC(vm.system, importmodule),
            ...
            "@arguments module\n\n"
            "Imports the dot-separated module @p module as if it were imported by the @c "
            "import statement and returns the resulting module object.\n\n"
    ...
    KRK_DOC(BIND_FUNC(vm.system, unload),
            "Removes a module from the module table. It is not necessarily garbage "
            "collected if other references to it exist.");
    ...
    krk_attachNamedObject(&vm.system->fields, "module",
                          (KrkObj *)vm.baseClasses->moduleClass);
    ...
    if (vm.binpath) {
        krk_attachNamedObject(&vm.system->fields, "executable_path",
                              (KrkObj *)krk_copyString(vm.binpath, strlen(vm.binpath)));
...
#    ifndef _WIN32
        ...
        if (strstr(dir, "/bin") == (dir + strlen(dir) - 4)) {
            ...
            krk_writeValueArray(AS_LIST(module_paths),
                                krk_stringFromFormat("%s/lib/kuroko/", dir));
        } else {
            krk_writeValueArray(AS_LIST(module_paths),
