...
static KrkTuple *functionArgs(KrkCodeObject *_self) {
    ...
    if (_self->obj.flags & KRK_OBJ_FLAGS_CODEOBJECT_COLLECTS_ARGS) {
        tuple->values.values[tuple->values.count++] = krk_stringFromFormat(
            "*%S",
            AS_STRING(_self->positionalArgNames.values[_self->potentialPositionals]));
    }

    for (short i = 0; i < _self->keywordArgs; ++i) {
        tuple->values.values[tuple->values.count++] =
            krk_stringFromFormat("%S=", AS_STRING(_self->keywordArgNames.values[i]));
    }

    if (_self->obj.flags & KRK_OBJ_FLAGS_CODEOBJECT_COLLECTS_KWS) {
        tuple->values.values[tuple->values.count++] = krk_stringFromFormat(
            "**%S", AS_STRING(_self->keywordArgNames.values[_self->keywordArgs]));
    }
...
KRK_StaticMethod(function, __new__) {
    ...
    if (IS_CLOSURE(argv[2]) && AS_CLOSURE(argv[2])->upvalueCount == code->upvalueCount) {
        ...
        return krk_pop();
    } else if (IS_TUPLE(argv[2]) &&
               AS_TUPLE(argv[2])->values.count == code->upvalueCount) {
         ...
         *           exist independently for this closure instance, and are not shared
         * with any other closures.
         */
        krk_push(OBJECT_VAL(krk_newClosure(code, argv[3])));
        for (size_t i = 0; i < code->upvalueCount; ++i) {
            AS_CLOSURE(krk_peek(0))->upvalues[i] = krk_newUpvalue(-1);
            AS_CLOSURE(krk_peek(0))->upvalues[i]->closed =
                AS_TUPLE(argv[2])->values.values[i];
        }
        return krk_pop();
    }
...
KRK_Method(function, __doc__) {
    ...
    if (IS_NATIVE(self) && AS_NATIVE(self)->doc) {
        return OBJECT_VAL(
            krk_copyString(AS_NATIVE(self)->doc, strlen(AS_NATIVE(self)->doc)));
...
KRK_Method(function, __repr__) {
    ...
    if (!IS_STRING(name))
        name = OBJECT_VAL(S("<unnamed>"));

    krk_push(name);

    struct StringBuilder sb = {0};
    krk_pushStringBuilderFormat(&sb, "<function %S at %p>", AS_STRING(name),
                                (void *)AS_OBJECT(self));
...
KRK_Method(function, __closure__) {
    ...
    for (size_t i = 0; i < cnt; ++i) {
        out->values.values[out->values.count++] =
            OBJECT_VAL(AS_CLOSURE(self)->upvalues[i]);
    }
...
KRK_Method(codeobject, __constants__) {
    ...
    memcpy(AS_TUPLE(krk_peek(0))->values.values, self->chunk.constants.values,
...
KRK_Method(codeobject, co_argcount) { return INTEGER_VAL(self->potentialPositionals); }

KRK_Method(codeobject, co_kwonlyargcount) { return INTEGER_VAL(self->keywordArgs); }
...
KRK_Method(codeobject, co_posonlyargcount) {
    ...
    for (size_t i = 0; i < self->potentialPositionals; ++i) {
        if (!IS_NONE(self->positionalArgNames.values[i]))
...
KRK_Method(codeobject, __locals__) {
    ...
    for (size_t i = 0; i < self->localNameCount; ++i) {
        ...
        AS_TUPLE(krk_peek(0))->values.values[AS_TUPLE(krk_peek(0))->values.count++] =
            INTEGER_VAL(self->localNames[i].id);
        AS_TUPLE(krk_peek(0))->values.values[AS_TUPLE(krk_peek(0))->values.count++] =
            INTEGER_VAL(self->localNames[i].birthday);
        AS_TUPLE(krk_peek(0))->values.values[AS_TUPLE(krk_peek(0))->values.count++] =
            INTEGER_VAL(self->localNames[i].deathday);
        AS_TUPLE(krk_peek(0))->values.values[AS_TUPLE(krk_peek(0))->values.count++] =
            OBJECT_VAL(self->localNames[i].name);
        AS_TUPLE(krk_peek(1))->values.values[AS_TUPLE(krk_peek(1))->values.count++] =
            krk_peek(0);
        krk_pop();
    }
    return krk_pop();
...
KRK_Method(codeobject, co_flags) {
    ...
    if (self->obj.flags & KRK_OBJ_FLAGS_CODEOBJECT_COLLECTS_ARGS)
        out |= 0x04;
    if (self->obj.flags & KRK_OBJ_FLAGS_CODEOBJECT_COLLECTS_KWS)
        out |= 0x08;
    if (self->obj.flags & KRK_OBJ_FLAGS_CODEOBJECT_IS_GENERATOR)
        out |= 0x20;
    if (self->obj.flags & KRK_OBJ_FLAGS_CODEOBJECT_IS_COROUTINE)
        out |= 0x80;
...
KRK_Method(method, __name__) {
    ATTRIBUTE_NOT_ASSIGNABLE();
    return IS_function(OBJECT_VAL(self->method))
               ? FUNC_NAME(function, __name__)(1, (KrkValue[]){OBJECT_VAL(self->method)},
                                               0)
               : OBJECT_VAL(S("?"));
}
...
KRK_Method(method, __qualname__) {
    ...
    return IS_function(OBJECT_VAL(self->method))
               ? FUNC_NAME(function,
                           __qualname__)(1, (KrkValue[]){OBJECT_VAL(self->method)}, 0)
               : OBJECT_VAL(S("?"));
}
...
KRK_Method(method, _ip_to_line) {
    METHOD_TAKES_EXACTLY(1);
    return IS_function(OBJECT_VAL(self->method))
               ? FUNC_NAME(function, _ip_to_line)(
                     2, (KrkValue[]){OBJECT_VAL(self->method), argv[1]}, 0)
               : OBJECT_VAL(S("?"));
}
...
KRK_Method(method, __repr__) {
    ...
    if (!IS_STRING(s))
        s = FUNC_NAME(method, __name__)(1, argv, 0);
    if (!IS_STRING(s))
        return NONE_VAL();
    krk_push(s);

    struct StringBuilder sb = {0};
    krk_pushStringBuilderFormat(&sb, "<bound method '%S' of %T object", AS_STRING(s),
                                self->receiver);
    if (IS_OBJECT(self->receiver))
        krk_pushStringBuilderFormat(&sb, " at %p", (void *)AS_OBJECT(self->receiver));
...
KRK_Method(method, __file__) {
    ATTRIBUTE_NOT_ASSIGNABLE();
    return IS_function(OBJECT_VAL(self->method))
               ? FUNC_NAME(function, __file__)(1, (KrkValue[]){OBJECT_VAL(self->method)},
                                               0)
               : OBJECT_VAL(S("?"));
}
...
KRK_Method(method, __args__) {
    ATTRIBUTE_NOT_ASSIGNABLE();
    return IS_function(OBJECT_VAL(self->method))
               ? FUNC_NAME(function, __args__)(1, (KrkValue[]){OBJECT_VAL(self->method)},
                                               0)
               : OBJECT_VAL(S("?"));
}
...
KRK_Method(method, __doc__) {
    ATTRIBUTE_NOT_ASSIGNABLE();
    return IS_function(OBJECT_VAL(self->method))
               ? FUNC_NAME(function, __doc__)(1, (KrkValue[]){OBJECT_VAL(self->method)},
                                              0)
               : OBJECT_VAL(S("?"));
}
...
KRK_Method(method, __annotations__) {
    ...
    return IS_function(OBJECT_VAL(self->method))
               ? FUNC_NAME(function,
                           __annotations__)(1, (KrkValue[]){OBJECT_VAL(self->method)}, 0)
               : OBJECT_VAL(S("?"));
}
...
KRK_Method(method, __code__) {
    ATTRIBUTE_NOT_ASSIGNABLE();
    return IS_function(OBJECT_VAL(self->method))
               ? FUNC_NAME(function, __code__)(1, (KrkValue[]){OBJECT_VAL(self->method)},
                                               0)
               : OBJECT_VAL(S("?"));
}
...
KRK_Function(staticmethod) {
    KrkObj *method;
    if (!krk_parseArgs("O!", (const char *[]){"method"}, KRK_BASE_CLASS(function),
                       &method))
        return NONE_VAL();
...
_noexport void _createAndBind_functionClass(void) {
    KrkClass *codeobject = ADD_BASE_CLASS(vm.baseClasses->codeobjectClass, "codeobject",
    ...
    KrkClass *function = ADD_BASE_CLASS(vm.baseClasses->functionClass, "function",
                                        vm.baseClasses->objectClass);
    ...
    krk_defineNative(&function->methods, "__class_getitem__", krk_GenericAlias)
        ->obj.flags |= KRK_OBJ_FLAGS_FUNCTION_IS_CLASS_METHOD;
    krk_finalizeClass(function);

    KrkClass *method = ADD_BASE_CLASS(vm.baseClasses->methodClass, "method",
                                      vm.baseClasses->objectClass);
    ...
    BUILTIN_FUNCTION("staticmethod", FUNC_NAME(krk, staticmethod),
                     "A static method does not take an implicit self or cls argument.");
    BUILTIN_FUNCTION("classmethod", FUNC_NAME(krk, classmethod),
