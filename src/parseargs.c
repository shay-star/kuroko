 ...
 * @param expected     Description of expected type; generally a type name, but maybe
 * something like "str of length 1".
 * @param arg          The value passed that failed the type check.
 * @param argName      Name of the argument. If NULL or zero-length, argument name is not
 * included in the description.
 */
_cold static void raise_TypeError(const char *method_name, const char *expected,
                                  KrkValue arg, const char *argName) {
    krk_runtimeError(vm.exceptions->typeError, "%s()%s%s expects %s, not '%T'",
                     method_name, (argName && *argName) ? " argument " : "",
                     (argName && *argName) ? argName : "", expected, arg);
}
 ...
 */
_cold static const char *methodName(const char *method_name, const char *fmt) {
 ...
 */
static int extractKwArg(KrkTable *kwargs, KrkString *argName, KrkValue *out,
                        KrkValueArray *refList) {
    if (!krk_tableGet_fast(kwargs, argName, out))
        return 1;
 ...
 */
int krk_parseVArgs(const char *orig_method_name, int argc, const KrkValue argv[],
    ...
    int acceptextrakws =
        0; /**< Whether extra keyword args should produce an error (0) or not (1) */

    if (*fmt == '.') {
        /**
         * If the format string starts with `.` then argument processing skips the first
         * argument on the assumption that this is a method and the first argument has
         * already been handled by the method wrapper macros or directly by the function.
         * This makes error messages a bit nicer, as argument counts will exclude the
         * implicit self.
         */
        argv++;
        argc--;
    ...
    while (*fmt) {
        ...
        if (*fmt == '|') {
            /**
             * @c | begins optional arguments - eg. default args. Every format option
             * after this point should be preset to usable default value, as it will not
             * be touched if the argument is not found.
             */
            if (!required) {
                krk_runtimeError(vm.exceptions->typeError,
                                 "format string has multiple |s");
                return 1;
            }
            required = 0;
            fmt++;
        ...
        if (*fmt == '*') {
             ...
             * the count of remaining arguments (int) and the pointer to their start in
             * the original argument list (KrkValue*).
             *
        ...
        if (*fmt == '$') {
            /**
             * @c $ indicates the end of positional arguments. Everything after this point
             * is only accepted as a keyword argument. @c $ must appear after one of @c |
             * or @c *.
             *
             * If any positional arguments remain when @c $ is encountred, a too-many
             * arguments exception will be raised.
             */
            if (required) {
                krk_runtimeError(vm.exceptions->typeError,
        ...
        if (*fmt == '~') {
            /**
             * If @c ~ is encountered anywhere in the format string, then extraneous
             * keyword arguments are left as-is and no exception is raised when they are
             * found. As keyword arguments are deleted from the kwargs dict while
             * processing other arguments, this means if @c hasKw is set then @c
             * argv[argc] will be left with only the unhandled keyword arguments, same as
             * for a @c **kwargs argument in a Kuroko function signature.
             */
            acceptextrakws = 1;
            fmt++;
            continue;
        ...
        if (iarg < argc) {
            ...
            arg = argv[iarg];
            iarg++;
        } else if ((required && !hasKw) ||
                   (hasKw &&
                    extractKwArg(AS_DICT(argv[argc]),
                                 krk_copyString(names[oarg], strlen(names[oarg])), &arg,
                                 AS_LIST(argv[argc + 1])) &&
                    required)) {
            /* If keyword argument lookup failed and this is not an optional argument,
             * raise an exception. */
            krk_runtimeError(vm.exceptions->typeError,
        ...
        if (*fmt == '?') {
            /* "is present", useful for things where relying on a default isn't useful but
             * you still want to have all the type checking and automatic parsing. */
            fmt++;
        ...
        if (*fmt == '!') {
            ...
            if (!krk_valuesSame(arg, KWARGS_VAL(0)) && !krk_isInstanceOf(arg, type)) {
                raise_TypeError(_method_name, type ? type->name->chars : "unknown type",
                                arg, names[oarg]);
                goto _error;
            }
        }
         ...
         */
#define NUMERIC(c, type)                                                                 \
    case c: {                                                                            \
        type *out = va_arg(args, type *);                                                \
        if (!krk_valuesSame(arg, KWARGS_VAL(0))) {                                       \
            if (!krk_long_to_int(arg, sizeof(type), out))                                \
                goto _error;                                                             \
        }                                                                                \
        break;                                                                           \
    }
    ...
    if (iarg < argc) {
        ...
        krk_runtimeError(vm.exceptions->argumentError,
                         "%s() takes %s %d argument%s (%d given)", _method_name,
                         required ? "exactly" : "at most", oarg, oarg == 1 ? "" : "s",
                         argc);
        return 0;
    }
    ...
    if (!acceptextrakws && hasKw && AS_DICT(argv[argc])->count) {
        ...
        for (size_t i = 0; i < AS_DICT(argv[argc])->capacity; ++i) {
            ...
            if (IS_STRING(entry->key)) {
                /* See if this was the name of an argument, which means it was already
                 * provided as a positional argument. */
                for (int j = 0; j < oarg; ++j) {
                    if (*names[j] && strlen(names[j]) == AS_STRING(entry->key)->length &&
                        !strcmp(names[j], AS_CSTRING(entry->key))) {
                        krk_runtimeError(vm.exceptions->typeError,
 ...
 */
int krk_parseArgs_impl(const char *method_name, int argc, const KrkValue argv[],
