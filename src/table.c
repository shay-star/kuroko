...
static inline ssize_t krk_tableIndexKeyC(const KrkTableEntry *entries,
                                         const ssize_t *indexes, size_t capacity,
                                         KrkValue key,
                                         int (*comparator)(KrkValue, KrkValue)) {
    uint32_t index;
...
static ssize_t krk_tableIndexKey(const KrkTableEntry *entries, const ssize_t *indexes,
                                 size_t capacity, KrkValue key) {
    return krk_tableIndexKeyC(entries, indexes, capacity, key, krk_valuesSameOrEqual);
}

static ssize_t krk_tableIndexKeyExact(const KrkTableEntry *entries,
                                      const ssize_t *indexes, size_t capacity,
                                      KrkValue key) {
...
int krk_tableSet(KrkTable *table, KrkValue key, KrkValue value) {
    ...
    ssize_t index =
        krk_tableIndexKey(table->entries, table->indexes, table->capacity, key);
    if (index < 0)
        return 0;
    KrkTableEntry *entry;
...
int krk_tableSetExact(KrkTable *table, KrkValue key, KrkValue value) {
    ...
    ssize_t index =
        krk_tableIndexKeyExact(table->entries, table->indexes, table->capacity, key);
    if (index < 0)
        return 0;
    KrkTableEntry *entry;
    int isNew = table->indexes[index] < 0;
...
int krk_tableSetIfExists(KrkTable *table, KrkValue key, KrkValue value) {
    ...
    ssize_t index =
        krk_tableIndexKey(table->entries, table->indexes, table->capacity, key);
    if (index < 0 || table->indexes[index] < 0)
        return 0;
...
int krk_tableGet_fast(KrkTable *table, KrkString *str, KrkValue *value) {
    ...
    for (;;) {
        if (table->indexes[index] == -1) {
            ...
            if (tombstone == -1)
                tombstone = index;
        } else if (krk_valuesSame(table->entries[table->indexes[index]].key,
                                  OBJECT_VAL(str))) {
            *value = table->entries[table->indexes[index]].value;
            return 1;
        }
...
KrkString *krk_tableFindString(KrkTable *table, const char *chars, size_t length,
                               uint32_t hash) {
    if (table->count == 0)
        return NULL;
    ...
    for (;;) {
        if (table->indexes[index] == -1) {
            ...
            if (tombstone == -1)
                tombstone = index;
        } else if (AS_STRING(table->entries[table->indexes[index]].key)->length ==
                       length &&
                   AS_OBJECT(table->entries[table->indexes[index]].key)->hash == hash &&
                   memcmp(AS_STRING(table->entries[table->indexes[index]].key)->chars,
                          chars, length) == 0) {
