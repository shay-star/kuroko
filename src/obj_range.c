...
KRK_Method(range, __repr__) {
    ...
    krk_pushStringBuilderFormat(&sb, "range(%zd,%zd", (ssize_t)self->min,
                                (ssize_t)self->max);
    if (self->step != 1)
        krk_pushStringBuilderFormat(&sb, ",%zd", (ssize_t)self->step);
...
KRK_Method(range, __iter__) {
    ...
    FUNC_NAME(rangeiterator, __init__)
    (4, (KrkValue[]){krk_peek(0), INTEGER_VAL(min), INTEGER_VAL(max), INTEGER_VAL(step)},
     0);
    krk_pop();
...
KRK_Method(range, __contains__) {
    int i;
    if (!krk_parseArgs(".i", (const char *[]){"i"}, &i))
        return NONE_VAL();
    if (self->step == 1)
...
_noexport void _createAndBind_rangeClass(void) {
    KrkClass *range =
    ...
    KRK_DOC(BIND_METHOD(range, __init__),
            ...
            "With one argument, iteration will start at @c 0 and continue to @p max, "
            "exclusive. "
            "With two arguments, iteration starts at @p min and continues to @p max, "
            "exclusive. "
    ...
    KrkClass *rangeiterator = ADD_BASE_CLASS(
        vm.baseClasses->rangeiteratorClass, "rangeiterator", vm.baseClasses->objectClass);
