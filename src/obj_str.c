...
KRK_StaticMethod(str, __new__) {
    ...
    if (IS_STRING(argv[1]))
        return argv[1]; /* strings are immutable, so we can just return the arg */
    /* Find the type of arg */
    krk_push(argv[1]);
    if (!krk_getType(argv[1])->_tostr)
        return krk_runtimeError(vm.exceptions->typeError, "Can not convert '%T' to str",
                                argv[1]);
...
KRK_Method(str, __hash__) { return INTEGER_VAL(self->obj.hash); }

KRK_Method(str, __len__) { return INTEGER_VAL(self->codesLength); }
...
KRK_Method(str, __getitem__) {
    ...
    if (IS_INTEGER(argv[1])) {
        ...
        if (asInt < 0)
            asInt += (int)AS_STRING(argv[0])->codesLength;
        ...
        if ((self->obj.flags & KRK_OBJ_FLAGS_STRING_MASK) == KRK_OBJ_FLAGS_STRING_ASCII) {
            ...
            size_t length = krk_codepointToBytes(KRK_STRING_FAST(self, asInt),
                                                 (unsigned char *)&asbytes);
            return OBJECT_VAL(krk_copyString((char *)&asbytes, length));
        }
    } else if (IS_slice(argv[1])) {
        KRK_SLICER(argv[1], self->codesLength) { return NONE_VAL(); }

        if (step == 1) {
            long len = end - start;
            if ((self->obj.flags & KRK_OBJ_FLAGS_STRING_MASK) ==
                KRK_OBJ_FLAGS_STRING_ASCII) {
            ...
            while ((step < 0) ? (i > end) : (i < end)) {
                size_t length = krk_codepointToBytes(KRK_STRING_FAST(self, i),
                                                     (unsigned char *)&asbytes);
            ...
            if (!AT_END() && c[1] == '{') {
                ...
                *fieldStop = '\0';
                ...
                if (isDigits) {
                    ...
                    KrkValue fieldAsString =
                        OBJECT_VAL(krk_copyString(fieldStart, fieldLength));
                ...
                if (IS_STRING(value)) {
                    ...
                    if (type->_tostr) {
                        ...
                        if (!krk_bindMethod(
                                type, AS_STRING(vm.specialMethodNames[METHOD_STR]))) {
                ...
                krk_push(asString);
                pushStringBuilderStr(&sb, AS_CSTRING(asString),
                                     AS_STRING(asString)->length);
                krk_pop();
            }
        } else if (*c == '}') {
            if (!AT_END() && c[1] == '}') {
...
_formatError:
    krk_runtimeError(vm.exceptions->typeError, "Error parsing format string: %s",
                     errorStr);
    goto _freeAndDone;
...
_formatOutOfRange:
    krk_runtimeError(vm.exceptions->indexError, "Positional index out of range: %d",
                     erroneousIndex);
    goto _freeAndDone;
...
KRK_Method(str, __mul__) {
    ...
    *c = '\0';
    return OBJECT_VAL(krk_takeStringVetted(out, totalLength, self->codesLength * howMany,
...
static int _str_join_callback(void *context, const KrkValue *values, size_t count) {
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        if (_context->isFirst) {
            ...
            pushStringBuilderStr(_context->sb, (char *)_context->self->chars,
                                 _context->self->length);
        }
        pushStringBuilderStr(_context->sb, (char *)AS_STRING(values[i])->chars,
                             AS_STRING(values[i])->length);
    }
...
static int substringMatch(const char *haystack, size_t haystackLen, const char *needle,
                          size_t needleLength) {
...
KRK_Method(str, __contains__) {
    ...
    for (size_t i = 0; i < self->length; ++i) {
        if (substringMatch(self->chars + i, self->length - i, needle->chars,
                           needle->length)) {
...
static KrkValue _string_strip_shared(int argc, const KrkValue argv[], int which) {
    ...
    if (which < 2)
        while (start < end && charIn((c = KRK_STRING_FAST(self, j)), subset)) {
            j++;
            start += CODEPOINT_BYTES(c);
        }
    if (which != 1)
        while (end > start && charIn((c = KRK_STRING_FAST(self, k)), subset)) {
            k--;
            end -= CODEPOINT_BYTES(c);
        }
...
strCompare(__gt__, >, <, >) strCompare(__lt__, <, >, <) strCompare(__ge__, >, <, >=)
    strCompare(__le__, <, >, <=)
...
KRK_Method(str, split) {
    ...
    if (!krk_parseArgs(".|z#i", (const char *[]){"sep", "maxsplit"}, &sep, &sepLen,
                       &maxsplit)) {
        return NONE_VAL();
    }
    ...
    if (!sep) {
        ...
        while (i != self->length) {
            struct StringBuilder sb = {0};
            while (i != self->length &&
                   !substringMatch(c, self->length - i, sep, sepLen)) {
                pushStringBuilder(&sb, *c);
                i++;
                c++;
            }
...
KRK_Method(str, replace) {
    ...
    char *c = self->chars;
    while (i < self->length) {
        if (substringMatch(c, self->length - i, oldStr->chars, oldStr->length) &&
            (IS_NONE(count) || replacements < AS_INTEGER(count))) {
...
#define WRAP_INDEX(index)                                                                \
    if (index < 0)                                                                       \
        index += self->codesLength;                                                      \
    if (index < 0)                                                                       \
        index = 0;                                                                       \
    if (index >= (krk_integer_type)self->codesLength)                                    \
    index = self->codesLength
...
KRK_Method(str, find) {
    ...
    for (krk_integer_type i = start; i < end; ++i) {
        ...
        for (j = 0; j < (krk_integer_type)substr->codesLength && (i + j < end); ++j) {
            if (KRK_STRING_FAST(self, i + j) != KRK_STRING_FAST(substr, j))
                break;
        }
        if (j == (krk_integer_type)substr->codesLength)
...
KRK_Method(str, startswith) {
    ...
    if (!krk_parseArgs(".O!|ii", (const char *[]){"prefix", "start", "end"},
                       KRK_BASE_CLASS(str), &substr, &start, &end)) {
        return NONE_VAL();
    }
    ...
    if (end < start || (size_t)(end - start) < substr->codesLength)
        return BOOLEAN_VAL(0);

    for (size_t i = 0; i < substr->codesLength; ++i) {
        if (KRK_STRING_FAST(self, start + i) != KRK_STRING_FAST(substr, i))
            return BOOLEAN_VAL(0);
    }
    return BOOLEAN_VAL(1);
}
...
KRK_Method(str, endswith) {
    ...
    int start = 0;
    int end = self->codesLength;
    if (!krk_parseArgs(".O!|ii", (const char *[]){"suffix", "start", "end"},
                       KRK_BASE_CLASS(str), &substr, &start, &end)) {
        return NONE_VAL();
    }
    ...
    for (size_t i = 0; i < substr->codesLength; ++i) {
        if (KRK_STRING_FAST(self, (end - i - 1)) !=
            KRK_STRING_FAST(substr, (substr->codesLength - i - 1)))
            return BOOLEAN_VAL(0);
    }
    return BOOLEAN_VAL(1);
}
...
KRK_Method(str, isalpha) { CHECK_ALL((c >= 'A' && c <= 'Z') || (c >= 'a' && c <= 'z')); }

KRK_Method(str, isdigit) { CHECK_ALL((c >= '0' && c <= '9')); }
...
KRK_Method(str, islower) { CHECK_ALL((c >= 'a' && c <= 'z')); }

KRK_Method(str, isupper) { CHECK_ALL((c >= 'A' && c <= 'Z')); }
...
KRK_Method(str, title) {
    ...
    for (size_t i = 0; i < self->length; ++i) {
        if (lastWasWhitespace && self->chars[i] >= 'a' && self->chars[i] <= 'z') {
            ...
            pushStringBuilder(&sb, self->chars[i]);
            lastWasWhitespace = !((self->chars[i] >= 'A' && self->chars[i] <= 'Z') ||
                                  (self->chars[i] >= 'a' && self->chars[i] <= 'z'));
        }
    }
...
_corrupt:
    return krk_runtimeError(vm.exceptions->typeError, "Corrupt str iterator: %s",
                            errorStr);
}
...
int krk_pushStringBuilderFormatV(struct StringBuilder *sb, const char *fmt,
                                 va_list args) {
...
KrkValue krk_stringFromFormat(const char *fmt, ...) {
    ...
    va_end(args);
    if (!result)
        return krk_discardStringBuilder(&sb);
    return krk_finishStringBuilder(&sb);
}

_noexport void _createAndBind_strClass(void) {
    KrkClass *str =
        ADD_BASE_CLASS(vm.baseClasses->strClass, "str", vm.baseClasses->objectClass);
    ...
    KrkClass *striterator = ADD_BASE_CLASS(vm.baseClasses->striteratorClass,
                                           "striterator", vm.baseClasses->objectClass);
