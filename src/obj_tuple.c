...
#define TUPLE_WRAP_INDEX()                                                               \
    ...
    if (index < 0 || index >= (krk_integer_type)self->values.count)                      \
    return krk_runtimeError(vm.exceptions->indexError, "tuple index out of range: %zd",  \
                            (ssize_t)index)
    ...
    if (IS_INTEGER(argv[1])) {
        ...
        if (step == 1) {
            ...
            KrkValue result = krk_callNativeOnStack(
                len, &krk_currentThread.stackTop[-len], 0, krk_tuple_of);
...
KRK_Method(tuple, __add__) {
    ...
    if (!IS_tuple(argv[1]))
        return krk_runtimeError(vm.exceptions->typeError,
                                "can only concatenate tuple (not '%T') to tuple",
                                argv[1]);
...
KRK_Method(tuple, __hash__) {
    ...
    for (size_t i = 0; i < (size_t)self->values.count; ++i) {
        uint32_t step = 0;
        if (krk_hashValue(self->values.values[i], &step))
            goto _unhashable;
        t = (t ^ step) * m;
...
_noexport void _createAndBind_tupleClass(void) {
    KrkClass *tuple =
    ...
    ADD_BASE_CLASS(vm.baseClasses->tupleiteratorClass, "tupleiterator",
    ...
    krk_defineNative(&vm.baseClasses->tupleiteratorClass->methods, "__init__",
                     _tuple_iter_init);
    krk_defineNative(&vm.baseClasses->tupleiteratorClass->methods, "__call__",
                     _tuple_iter_call);
