...
static int _bytes_callback(void *context, const KrkValue *values, size_t count) {
    ...
    for (size_t i = 0; i < count; ++i) {
        if (!IS_INTEGER(values[i])) {
            krk_runtimeError(vm.exceptions->typeError, "'%T' is not an integer",
                             values[i]);
            return 1;
        }
        if (AS_INTEGER(values[i]) < 0 || AS_INTEGER(values[i]) > 255) {
            krk_runtimeError(vm.exceptions->typeError,
                             "bytes object must be in range(0, 256)");
            return 1;
        }
...
KRK_StaticMethod(bytes, __new__) {
    ...
    if (IS_bytearray(argv[1])) {
        return OBJECT_VAL(krk_newBytes(AS_BYTES(AS_bytearray(argv[1])->actual)->length,
        ...
        if (AS_INTEGER(argv[1]) < 0)
            return krk_runtimeError(vm.exceptions->valueError, "negative count");
        return OBJECT_VAL(krk_newBytes(AS_INTEGER(argv[1]), NULL));
    } else {
        struct StringBuilder sb = {0};
        if (krk_unpackIterable(argv[1], &sb, _bytes_callback))
            return NONE_VAL();
...
KRK_Method(bytes, __eq__) {
    ...
    if (self->length != them->length)
        return BOOLEAN_VAL(0);
    if (self->obj.hash != them->obj.hash)
        return BOOLEAN_VAL(0);
    for (size_t i = 0; i < self->length; ++i) {
...
KRK_Method(bytes, __repr__) {
    ...
    for (size_t i = 0; i < AS_BYTES(argv[0])->length; ++i) {
        ...
        case '\n':
            pushStringBuilder(&sb, '\\');
            pushStringBuilder(&sb, 'n');
            break;
        case '\r':
            pushStringBuilder(&sb, '\\');
            pushStringBuilder(&sb, 'r');
            break;
        case '\t':
            pushStringBuilder(&sb, '\\');
            pushStringBuilder(&sb, 't');
            break;
        case '\v':
            pushStringBuilder(&sb, '\\');
            pushStringBuilder(&sb, 'v');
            break;
        default: {
            if (ch < ' ' || ch >= 0x7F) {
                pushStringBuilder(&sb, '\\');
...
KRK_Method(bytes, __len__) { return INTEGER_VAL(AS_BYTES(argv[0])->length); }
...
KRK_Method(bytes, decode) {
    METHOD_TAKES_NONE();
    return OBJECT_VAL(
        krk_copyString((char *)AS_BYTES(argv[0])->bytes, AS_BYTES(argv[0])->length));
}
...
static int _bytes_join_callback(void *context, const KrkValue *values, size_t count) {
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        if (_context->isFirst) {
            ...
            pushStringBuilderStr(_context->sb, (char *)_context->self->bytes,
                                 _context->self->length);
        }
        pushStringBuilderStr(_context->sb, (char *)AS_BYTES(values[i])->bytes,
                             AS_BYTES(values[i])->length);
    }
...
KRK_Method(bytearray, __init__) {
    ...
    if (argc < 2) {
        ...
        self->actual =
            OBJECT_VAL(krk_newBytes(AS_BYTES(argv[1])->length, AS_BYTES(argv[1])->bytes));
    } else if (IS_INTEGER(argv[1])) {
...
#undef IS_bytearray
#define IS_bytearray(o)                                                                  \
    (krk_isInstanceOf(o, vm.baseClasses->bytearrayClass) &&                              \
     IS_BYTES(AS_bytearray(o)->actual))
...
KRK_Method(bytearray, __getitem__) {
    ...
    if (IS_INTEGER(argv[1])) {
        ...
        if (asInt < 0)
            asInt += (long)AS_BYTES(self->actual)->length;
        if (asInt < 0 || asInt >= (long)AS_BYTES(self->actual)->length) {
            return krk_runtimeError(vm.exceptions->indexError,
                                    "bytearray index out of range: %d", (int)asInt);
        }

        return INTEGER_VAL(AS_BYTES(self->actual)->bytes[asInt]);
    } else if (IS_slice(argv[1])) {
        KRK_SLICER(argv[1], AS_BYTES(self->actual)->length) { return NONE_VAL(); }
...
KRK_Method(bytearray, decode) {
    METHOD_TAKES_NONE();
    return OBJECT_VAL(krk_copyString((char *)AS_BYTES(self->actual)->bytes,
                                     AS_BYTES(self->actual)->length));
}
...
_noexport void _createAndBind_bytesClass(void) {
    KrkClass *bytes =
    ...
    KRK_DOC(BIND_STATICMETHOD(bytes, __new__),
            ...
            "@arguments iter=None\n\n"
            "Creates a new @ref bytes object. If @p iter is provided, it should be a "
            "@ref tuple or @ref list "
    ...
    KrkClass *bytesiterator = ADD_BASE_CLASS(
        vm.baseClasses->bytesiteratorClass, "bytesiterator", vm.baseClasses->objectClass);
    ...
    KrkClass *bytearray = ADD_BASE_CLASS(vm.baseClasses->bytearrayClass, "bytearray",
                                         vm.baseClasses->objectClass);
    bytearray->allocSize = sizeof(struct ByteArray);
    bytearray->_ongcscan = _bytearray_gcscan;
    KRK_DOC(BIND_METHOD(bytearray, __init__), "@brief A mutable array of bytes.\n"
