...
KRK_StaticMethod(type, __new__) {
    ...
    if (!krk_parseArgs(
            "O!O!O!O!~:type", (const char *[]){"cls", "name", "base", "namespace"},
            vm.baseClasses->typeClass, &metaclass, vm.baseClasses->strClass, &name,
            vm.baseClasses->typeClass, &base, vm.baseClasses->dictClass, &nspace)) {
        return NONE_VAL();
    }

    if (base->obj.flags & KRK_OBJ_FLAGS_NO_INHERIT) {
        return krk_runtimeError(vm.exceptions->typeError, "'%S' can not be subclassed",
                                base->name);
    }
    ...
    if (krk_tableGet_fast(&_class->methods, S("__class_getitem__"), &tmp) &&
        IS_CLOSURE(tmp)) {
        AS_CLOSURE(tmp)->obj.flags |= KRK_OBJ_FLAGS_FUNCTION_IS_CLASS_METHOD;
    }

    if (krk_tableGet_fast(&_class->methods, S("__init_subclass__"), &tmp) &&
        IS_CLOSURE(tmp)) {
        AS_CLOSURE(tmp)->obj.flags |= KRK_OBJ_FLAGS_FUNCTION_IS_CLASS_METHOD;
    }
...
KRK_Method(type, __getitem__) {
    ...
    return krk_runtimeError(vm.exceptions->attributeError,
                            "'%s' object is not subscriptable", "type");
}
...
KRK_Method(type, __call__) {
    ...
    if (!self->_new) {
        return krk_runtimeError(vm.exceptions->typeError, "%S() can not be built",
                                self->name);
    }
    ...
    /* If an exception happened in __new__, don't try to call __init__ even if the
     * conditions would be right. */
    if (unlikely(krk_currentThread.flags & KRK_THREAD_HAS_EXCEPTION))
        return NONE_VAL();

    if (self->_init != KRK_BASE_CLASS(object)->_init && self->_init != NULL &&
        krk_isInstanceOf(result, self)) {
        /* Because we have to swap the class for the instance here, we can't do a fast
         * call even if __init__ is a native function, so we're stuck with the slow
         * approach... */
...
_noexport void _createAndBind_type(void) {
    KrkClass *type =
