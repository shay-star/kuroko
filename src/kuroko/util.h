...
#define ADD_BASE_CLASS(obj, name, baseClass)                                             \
    krk_makeClass(vm.builtins, &obj, name, baseClass)
    ...
    do {                                                                                 \
        if (unlikely(argc != 1))                                                         \
            return krk_runtimeError(vm.exceptions->argumentError,                        \
                                    "%s() takes no arguments (%d given)", _method_name,  \
                                    (argc - 1));                                         \
    } while (0)

#define METHOD_TAKES_EXACTLY(n)                                                          \
    do {                                                                                 \
        if (unlikely(argc != (n + 1)))                                                   \
            return krk_runtimeError(                                                     \
                vm.exceptions->argumentError, "%s() takes %s %d argument%s (%d given)",  \
                _method_name, "exactly", n, (n != 1) ? "s" : "", (argc - 1));            \
    } while (0)
...
#define METHOD_TAKES_AT_LEAST(n)                                                         \
    do {                                                                                 \
        if (unlikely(argc < (n + 1)))                                                    \
            return krk_runtimeError(                                                     \
                vm.exceptions->argumentError, "%s() takes %s %d argument%s (%d given)",  \
                _method_name, "at least", n, (n != 1) ? "s" : "", (argc - 1));           \
    } while (0)
...
#define METHOD_TAKES_AT_MOST(n)                                                          \
    do {                                                                                 \
        if (unlikely(argc > (n + 1)))                                                    \
            return krk_runtimeError(                                                     \
                vm.exceptions->argumentError, "%s() takes %s %d argument%s (%d given)",  \
                _method_name, "at most", n, (n != 1) ? "s" : "", (argc - 1));            \
    } while (0)
...
#define FUNCTION_TAKES_EXACTLY(n)                                                        \
    do {                                                                                 \
        if (unlikely(argc != n))                                                         \
            return krk_runtimeError(                                                     \
                vm.exceptions->argumentError, "%s() takes %s %d argument%s (%d given)",  \
                _method_name, "exactly", n, (n != 1) ? "s" : "", (argc));                \
    } while (0)
...
#define FUNCTION_TAKES_AT_LEAST(n)                                                       \
    do {                                                                                 \
        if (unlikely(argc < n))                                                          \
            return krk_runtimeError(                                                     \
                vm.exceptions->argumentError, "%s() takes %s %d argument%s (%d given)",  \
                _method_name, "at least", n, (n != 1) ? "s" : "", (argc));               \
    } while (0)
...
#define FUNCTION_TAKES_AT_MOST(n)                                                        \
    do {                                                                                 \
        if (unlikely(argc > n))                                                          \
            return krk_runtimeError(                                                     \
                vm.exceptions->argumentError, "%s() takes %s %d argument%s (%d given)",  \
                _method_name, "at most", n, (n != 1) ? "s" : "", (argc));                \
    } while (0)

#define TYPE_ERROR(expected, value)                                                      \
    krk_runtimeError(vm.exceptions->typeError, "%s() expects %s, not '%T'",              \
                     /* Function name */ _method_name, /* expected type */ #expected,    \
                     value)

#define NOT_ENOUGH_ARGS(name)                                                            \
    krk_runtimeError(vm.exceptions->argumentError, "Expected more args.")

#define CHECK_ARG(i, type, ctype, name)                                                  \
    if (unlikely(argc < (i + 1)))                                                        \
        return NOT_ENOUGH_ARGS(name);                                                    \
    if (unlikely(!IS_##type(argv[i])))                                                   \
        return TYPE_ERROR(type, argv[i]);                                                \
    ctype name _unused = AS_##type(argv[i])

#define FUNC_NAME(klass, name) _##klass##_##name
#define FUNC_SIG(klass, name)                                                            \
    _noexport KrkValue FUNC_NAME(klass, name)(int argc, const KrkValue argv[], int hasKw)

/* This assumes you have a KrkInstance called `module` in the current scope. */
#define MAKE_CLASS(klass)                                                                \
    do {                                                                                 \
        krk_makeClass(module, &klass, #klass, vm.baseClasses->objectClass);              \
        klass->allocSize = sizeof(struct klass);                                         \
    } while (0)
#define BIND_METHOD(klass, method)                                                       \
    krk_defineNative(&klass->methods, #method, _##klass##_##method)
#define BIND_PROP(klass, method)                                                         \
    krk_defineNativeProperty(&klass->methods, #method, _##klass##_##method)
#define BIND_FUNC(module, func) krk_defineNative(&module->fields, #func, _krk_##func)

static inline KrkNative *krk_defineNativeStaticMethod(KrkTable *table, const char *name,
                                                      NativeFn function) {
...
#define BIND_STATICMETHOD(klass, method)                                                 \
    krk_defineNativeStaticMethod(&klass->methods, #method, _##klass##_##method)

static inline KrkNative *krk_defineNativeClassMethod(KrkTable *table, const char *name,
                                                     NativeFn function) {
    KrkNative *out = krk_defineNative(table, name, function);
    out->obj.flags |= KRK_OBJ_FLAGS_FUNCTION_IS_CLASS_METHOD;
    return out;
}
#define BIND_CLASSMETHOD(klass, method)                                                  \
    krk_defineNativeClassMethod(&klass->methods, #method, _##klass##_##method)

#define KRK_Method_internal_name(klass, name) _krk_method_##klass##_##name
#define KRK_Method_internal_sig(klass, name)                                             \
    static inline KrkValue KRK_Method_internal_name(klass, name)(                        \
        const char *_method_name, CURRENT_CTYPE CURRENT_NAME, int argc,                  \
        const KrkValue argv[], int hasKw)
...
#define KRK_Method(klass, name)                                                          \
    ...
    FUNC_SIG(klass, name) {                                                              \
        ...
        return KRK_Method_internal_name(klass, name)(_method_name, CURRENT_NAME, argc,   \
                                                     argv, hasKw);                       \
    }                                                                                    \
    KRK_Method_internal_sig(klass, name)
...
#define KRK_Function_internal_sig(name)                                                  \
    static inline KrkValue KRK_Function_internal_name(name)(                             \
        const char *_method_name, int argc, const KrkValue argv[], int hasKw)
...
#define IS_list(o)                                                                       \
    ((IS_INSTANCE(o) && AS_INSTANCE(o)->_class == vm.baseClasses->listClass) ||          \
     krk_isInstanceOf(o, vm.baseClasses->listClass))
...
#define IS_dict(o)                                                                       \
    ((IS_INSTANCE(o) && AS_INSTANCE(o)->_class == vm.baseClasses->dictClass) ||          \
...
extern KrkValue krk_dict_nth_key_fast(size_t capacity, KrkTableEntry *entries,
                                      size_t index);
...
static inline void _setDoc_class(KrkClass *thing, const char *text, size_t size) {
    krk_attachNamedObject(&thing->methods, "__doc__",
                          (KrkObj *)krk_copyString(text, size));
}
static inline void _setDoc_instance(KrkInstance *thing, const char *text, size_t size) {
    krk_attachNamedObject(&thing->fields, "__doc__",
                          (KrkObj *)krk_copyString(text, size));
}
...
#    define KRK_DOC(thing, text)                                                         \
        _Generic(&((thing)[0]),                                                          \
            ...
            KrkNative *: _setDoc_native)(thing, text, sizeof(text) - 1)
#endif

#define BUILTIN_FUNCTION(name, func, docStr)                                             \
    KRK_DOC(krk_defineNative(&vm.builtins->fields, name, func), docStr)

extern int krk_extractSlicer(const char *_method_name, KrkValue slicerVal,
                             krk_integer_type count, krk_integer_type *start,
                             krk_integer_type *end, krk_integer_type *step);
 ...
 *
 * Unpacks an iterable value, passing a series of arrays of values to a callback, @p
 * callback.
 *
 * If @p iterable is a list or tuple, @p callback will be called once with the total size
 * of the container. Otherwise, @p callback will be called many times with a count of 1,
 * until the iterable is exhausted.
 *
 * If @p iterable is not iterable, an exception is set and 1 is returned.
 * If @p callback returns non-zero, unpacking stops and 1 is returned, with no additional
 * exception.
 */
extern int krk_unpackIterable(KrkValue iterable, void *context,
...
extern int krk_parseVArgs(const char *_method_name, int argc, const KrkValue argv[],
                          int hasKw, const char *fmt, const char **names, va_list args);

extern int krk_parseArgs_impl(const char *_method_name, int argc, const KrkValue argv[],
                              int hasKw, const char *format, const char **names, ...);
...
extern int krk_pushStringBuilderFormatV(struct StringBuilder *sb, const char *fmt,
                                        va_list args);
...
#define KRK_Module_internal_sig(name)                                                    \
    static inline void KRK_Module_internal_name(name)(KrkInstance * module,              \
                                                      KrkString * runAs)
