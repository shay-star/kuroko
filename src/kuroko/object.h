...
typedef struct KrkObj {
    ...
    uint16_t
        flags; /**< @brief General object flags, mostly related to garbage collection. */
...
typedef enum {
    /* For compatibility */
    KRK_STRING_ASCII = KRK_OBJ_FLAGS_STRING_ASCII, /**< Codepoints can be extracted
                                                      directly from UTF8 data. */
...
typedef struct KrkUpvalue {
    ...
    struct KrkThreadState
        *owner; /**< @brief The thread that owns the stack this upvalue belongs in */
} KrkUpvalue;
...
typedef struct {
    size_t
        id; /**< @brief Local ID as used by opcodes; offset from the frame's stack base */
    size_t
        birthday; /**< @brief Instruction offset that this local name became valid on */
    size_t deathday; /**< @brief Instruction offset that this local name becomes invalid
                        on */
...
typedef struct {
    uint32_t instructionOffset; /**< @brief Instruction (operand offset) this jump target
                                   applies to */
...
typedef struct {
    ...
    unsigned short potentialPositionals; /**< @brief Precalculated positional arguments
                                            for complex argument processing */
    unsigned short totalArguments; /**< @brief Total argument cells we can fill in complex
                                      argument processing */
    size_t upvalueCount;  /**< @brief Number of upvalues this function collects as a
                             closure */
    ...
    KrkValueArray positionalArgNames; /**< @brief Array of names for positional arguments
                                         (and *args) */
    KrkValueArray keywordArgNames; /**< @brief Array of names for keyword-only arguments
                                      (and **kwargs) */
    size_t localNameCapacity;      /**< @brief Capacity of @ref localNames */
    size_t localNameCount;         /**< @brief Number of entries in @ref localNames */
    KrkLocalEntry *localNames; /**< @brief Stores the names of local variables used in the
    ...
    KrkExpressionsMap *expressions; /**< @brief Mapping of bytecode offsets to expression
                                       spans for debugging */
    ...
    size_t overlongJumpsCapacity; /**< @brief Number of possible entries in pessimal jump
                                     table */
...
typedef struct {
    KrkObj obj;              /**< @protected @brief Base */
    KrkCodeObject *function; /**< @brief The codeobject containing the bytecode run when
                                this function is called */
    KrkUpvalue **upvalues;   /**< @brief Array of upvalues collected from the surrounding
                                context when the closure was created */
...
typedef struct KrkClass {
    ...
    KrkString *filename;     /**< @brief Filename of the original source that defined the
                                codeobject for the class */
    struct KrkClass *base;   /**< @brief Pointer to base class implementation */
    size_t
        allocSize; /**< @brief Size to allocate when creating instances of this class */
    KrkCleanupCallback
        _ongcscan; /**< @brief C function to call when the garbage collector visits an
                      instance of this class in the scan phase */
    KrkCleanupCallback _ongcsweep; /**< @brief C function to call when the garbage
                                      collector is discarding an instance of this class */
    KrkTable subclasses;           /**< @brief Set of classes that subclass this class */

    KrkObj
        *_getter; /**< @brief @c %__getitem__  Called when an instance is subscripted */
    KrkObj *_setter; /**< @brief @c %__setitem__  Called when a subscripted instance is
                        assigned to */
    KrkObj *_reprer; /**< @brief @c %__repr__     Called to create a reproducible string
                        representation of an instance */
    KrkObj *_tostr;  /**< @brief @c %__str__      Called to produce a string from an
                        instance */
    KrkObj *_call;   /**< @brief @c %__call__     Called when an instance is called like a
                        function */
    KrkObj *_init;   /**< @brief @c %__init__     Implicitly called when an instance is
                        created */
    KrkObj *_eq;    /**< @brief @c %__eq__       Implementation for equality check (==) */
    KrkObj *_len;   /**< @brief @c %__len__      Generally called by len() but may be used
    ...
    KrkObj *_delitem; /**< @brief @c %__delitem__  Called when `del` is used with a
                         subscript */
    KrkObj *_iter;    /**< @brief @c %__iter__     Called by `for ... in ...`, etc. */
    KrkObj *_getattr; /**< @brief @c %__getattr__  Overrides normal behavior for attribute
                         access */
    KrkObj *_dir; /**< @brief @c %__dir__      Overrides normal behavior for @c dir() */
    KrkObj *_contains; /**< @brief @c %__contains__ Called to resolve `in` (as a binary
                          operator) */
    KrkObj *_descget;  /**< @brief @c %__get__      Called when a descriptor object is
                          bound as a property */
    KrkObj *_descset;  /**< @brief @c %__set__      Called when a descriptor object is
                          assigned to as a property */
    KrkObj *_classgetitem; /**< @brief @c %__class_getitem__ Class method called when a
                              type object is subscripted; used for type hints */
    KrkObj *_hash; /**< @brief @c %__hash__     Called when an instance is a key in a dict
                      or an entry in a set */
...
typedef struct {
    KrkObj obj; /**< @protected @brief Base */
    KrkValueArray
        values; /**< @brief Stores the length, capacity, and actual values of the tuple */
} KrkTuple;
...
typedef struct {
    ...
    KrkValueArray
        values; /**< @brief Stores the length, capacity, and actual values of the list */
#ifndef KRK_DISABLE_THREADS
 ...
 */
extern KrkString *krk_takeStringVetted(char *chars, size_t length, size_t codesLength,
                                       KrkStringType type, uint32_t hash);
 ...
 */
extern KrkInstance *krk_buildGenerator(KrkClosure *function, KrkValue *arguments,
                                       size_t argCount);
