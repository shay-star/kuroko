...
static int krk_long_compare(const KrkLong *a, const KrkLong *b) {
    ...
    for (size_t i = 0; i < abs_width; ++i) {
        if (a->digits[abs_width - i - 1] > b->digits[abs_width - i - 1])
            return sign; /* left is bigger */
        if (a->digits[abs_width - i - 1] < b->digits[abs_width - i - 1])
            return -sign; /* right is bigger */
    }
...
static int krk_long_add_ignore_sign(KrkLong *res, const KrkLong *a, const KrkLong *b) {
    ...
    for (size_t i = 0; i < owidth - 1; ++i) {
        uint32_t out =
            (i < awidth ? a->digits[i] : 0) + (i < bwidth ? b->digits[i] : 0) + carry;
...
#define PREP_OUTPUT(res, a, b)                                                           \
    KrkLong _tmp_out_##res, *_swap_out_##res = NULL;                                     \
    do {                                                                                 \
        if (res == a || res == b) {                                                      \
            krk_long_init_si(&_tmp_out_##res, 0);                                        \
            _swap_out_##res = res;                                                       \
            res = &_tmp_out_##res;                                                       \
        }                                                                                \
    } while (0)
#define PREP_OUTPUT1(res, a)                                                             \
    KrkLong _tmp_out_##res, *_swap_out_##res = NULL;                                     \
    do {                                                                                 \
        if (res == a) {                                                                  \
            krk_long_init_si(&_tmp_out_##res, 0);                                        \
...
#define FINISH_OUTPUT(res)                                                               \
    do {                                                                                 \
        if (_swap_out_##res) {                                                           \
            _swap(_swap_out_##res, res);                                                 \
            krk_long_clear(&_tmp_out_##res);                                             \
        }                                                                                \
    } while (0)
 ...
 */
static int krk_long_div_rem(KrkLong *quot, KrkLong *rem, const KrkLong *a,
                            const KrkLong *b) {
 ...
 */
static char *krk_long_to_str(const KrkLong *n, int _base, const char *prefix,
    ...
    size_t len =
        (sign == -1 ? 1 : 0) + krk_long_digits_in_base(&abs, _base) + strlen(prefix) + 1;
    char *tmp = malloc(len);
    char *writer = tmp;
    ...
    if (sign == 0) {
        *writer++ = '0';
        ...
        switch (_base) {
        case 2:
            writer = _fast_conversion(&abs, 1, writer);
            break;
        case 4:
            writer = _fast_conversion(&abs, 2, writer);
            break;
        case 8:
            writer = _fast_conversion(&abs, 3, writer);
            break;
        case 16:
            writer = _fast_conversion(&abs, 4, writer);
            break;
        default:
...
static int is_valid(int base, uint8_t c) { return _convert_table[c] < base; }

static int convert_digit(uint8_t c) { return _convert_table[c]; }
 ...
 */
static int krk_long_parse_string(const char *str, KrkLong *num, unsigned int base,
                                 size_t len) {
...
KRK_StaticMethod(long, __new__) {
    ...
    if (argc < 2) {
        ...
        /* XXX This should probably work like int(...) does and default to base 10... and
         * take a base at all... */
        struct BigInt *self = (struct BigInt *)krk_newInstance(KRK_BASE_CLASS(long));
        krk_push(OBJECT_VAL(self));
        if (krk_long_parse_string(AS_CSTRING(argv[1]), self->value, 0,
                                  AS_STRING(argv[1])->length)) {
            return krk_runtimeError(vm.exceptions->valueError,
                                    "invalid literal for long() with base 0: %R",
                                    argv[1]);
        }
        return krk_pop();
...
KRK_Method(long, __float__) { return FLOATING_VAL(krk_long_get_double(self->value)); }

static KrkValue _krk_long_truediv(KrkLong *_top, KrkLong *_bottom) {
    if (_bottom->width == 0)
        return krk_runtimeError(vm.exceptions->valueError, "float division by zero");
    if (_top->width == 0)
...
KRK_Method(long, __truediv__) {
    krk_long tmp;
    if (IS_long(argv[1]))
        krk_long_init_copy(tmp, AS_long(argv[1])->value);
    else if (IS_INTEGER(argv[1]))
        krk_long_init_si(tmp, AS_INTEGER(argv[1]));
    else if (IS_FLOATING(argv[1]))
...
#define PRINTER(name, base, prefix)                                                      \
    KRK_Method(long, __##name##__) {                                                     \
        ...
        return OBJECT_VAL(                                                               \
            krk_takeStringVetted(rev, size, size, KRK_OBJ_FLAGS_STRING_ASCII, hash));    \
    }
...
KRK_Method(long, __int__) { return INTEGER_VAL(krk_long_medium(self->value)); }
...
#define BASIC_BIN_OP_FLOATS(name, long_func, MAYBE_FLOAT, MAYBE_FLOAT_INV)               \
    ...
    _noexport KrkValue krk_long_coerced_##name(krk_integer_type a, krk_integer_type b) { \
...
#ifndef KRK_NO_FLOAT
#    define FLOAT_A(op)                                                                  \
        else if (IS_FLOATING(argv[1])) return FLOATING_VAL(                              \
            krk_long_get_double(self->value) op AS_FLOATING(argv[1]));
#    define FLOAT_B(op)                                                                  \
        else if (IS_FLOATING(argv[1])) return FLOATING_VAL(                              \
            AS_FLOATING(argv[1]) op krk_long_get_double(self->value));
#else
#    define FLOAT_A(op)                                                                  \
        else if (IS_FLOATING(argv[1])) return krk_runtimeError(                          \
            vm.exceptions->valueError, "no float support");
#    define FLOAT_B(op)                                                                  \
        else if (IS_FLOATING(argv[1])) return krk_runtimeError(                          \
            vm.exceptions->valueError, "no float support");
#endif
...
static void _krk_long_lshift(krk_long out, krk_long val, krk_long shift) {
    if (krk_long_sign(shift) < 0) {
        krk_runtimeError(vm.exceptions->valueError, "negative shift count");
        return;
    }
    ...
    if (amount < (size_t)count) {
        ...
        if (!offset) {
            ...
            for (size_t i = 1; i < (size_t)out->width; ++i) {
                out->digits[i - 1] |=
                    (val->digits[i + cycles] << (31 - offset)) & DIGIT_MAX;
                out->digits[i] = (val->digits[i + cycles] >> offset) & DIGIT_MAX;
            }
            if (out->width + cycles < (size_t)w) {
                out->digits[out->width - 1] |=
                    (val->digits[out->width + cycles] << (31 - offset)) & DIGIT_MAX;
            }
        }
    }
...
static void _krk_long_mod(krk_long out, krk_long a, krk_long b) {
    if (krk_long_sign(b) == 0) {
        krk_runtimeError(vm.exceptions->valueError, "integer division or modulo by zero");
        return;
    }
    krk_long garbage;
...
#    define KRK_FLOAT_COMPARE(comp)                                                      \
        else if (IS_FLOATING(argv[1])) return BOOLEAN_VAL(                               \
            krk_long_get_double(self->value) comp AS_FLOATING(argv[1]));
#else
#    define KRK_FLOAT_COMPARE(comp)
#endif
...
KRK_Method(long, __len__) { return INTEGER_VAL(krk_long_sign(self->value)); }
...
KRK_Method(long, __pos__) { return argv[0]; }

typedef int (*fmtCallback)(void *, int, int *);
KrkValue krk_doFormatString(const char *typeName, KrkString *format_spec, int positive,
                            void *abs, fmtCallback callback,
                            fmtCallback (*prepCallback)(void *, int));
...
KRK_Method(long, __format__) {
    ...
    KrkValue result =
        krk_doFormatString("long", format_spec, krk_long_sign(self->value) >= 0, &tmp,
                           NULL, prepLongCallback);
...
KRK_Method(long, bit_count) { return long_bit_count(self->value); }
...
KRK_Method(long, bit_length) { return long_bit_length(self->value); }

static KrkValue long_to_bytes(KrkLong *val, size_t argc, const KrkValue argv[],
                              int hasKw) {
    static const char _method_name[] = "to_bytes";
    int length;
    const char *byteorder;
    int _signed = 0;
    if (!krk_parseArgs(".is|p", (const char *[]){"length", "byteorder", "signed"},
                       &length, &byteorder, &_signed))
        return NONE_VAL();
    if (length < 0)
    ...
    if (_signed && val->width != 0)
        bitCount++;

    if ((size_t)length * 8 < bitCount) {
        krk_long_clear(&tmp);
        /* Should be OverflowError, but we don't have that and I don't care to add it
         * right now */
 ...
 */
static digit_t *dec_add(const digit_t *a, size_t awidth, const digit_t *b, size_t bwidth,
                        size_t *outwidth) {
 ...
 */
static digit_t *dec_shift(const digit_t *a, size_t awidth, size_t amount,
                          size_t *outwidth) {
    if (awidth == 1 && a[0] == 0) {
        *outwidth = 1;
 ...
 */
static digit_t *dec_mul(const digit_t *a, size_t a_width, const digit_t *b,
                        size_t b_width, size_t *outwidth) {
    ...
    *outwidth = a_width + b_width;
    ...
    if (b_width < 50) {
        ...
        while (*outwidth > 1 && out[(*outwidth) - 1] == 0)
            (*outwidth)--;
        return out;
    } else {
        size_t m2 = a_width / 2;
        ...
        while (low1_width > 1 && low1[low1_width - 1] == 0)
            low1_width--;
        digit_t a_zero = 0;
        ...
        while (low2_width > 1 && low2[low2_width - 1] == 0)
            low2_width--;
        digit_t b_zero = 0;
...
KRK_Method(long, __repr__) {
    ...
    if (self->value->width > -10 && self->value->width < 10) {
        ...
        return OBJECT_VAL(
            krk_takeStringVetted(rev, len, len, KRK_OBJ_FLAGS_STRING_ASCII, hash));
    }
...
KrkValue krk_int_from_float(double a) {
    ...
    if (e == 1024)
        return krk_runtimeError(vm.exceptions->valueError,
                                "can not convert float %s to int",
                                m ? "Nan" : "infinity");
    if (e < 47)
 ...
 */
KrkValue krk_double_to_string(double a, unsigned int digits, char formatter, int plus,
                              int forcedigits) {
    union {
        double d;
        uint64_t u;
    ...
    if (e == 1024) {
        ...
        if (sign && !m)
            ...
            krk_pushStringBuilder(&sb, '+');
        if (m)
            krk_pushStringBuilderStr(&sb, caps ? "NAN" : "nan", 3);
        else
            krk_pushStringBuilderStr(&sb, caps ? "INF" : "inf", 3);
    ...
    if (e == -1023 && m == 0) {
        ...
        if (digits && (forcedigits || formatter == ' ')) {
            ...
            for (unsigned int i = 0;
                 i <
                 ((formatter == ' ') ? 1 : (digits - ((!noexp && !alwaysexp) ? 1 : 0)));
                 ++i) {
                krk_pushStringBuilder(&sb, '0');
            }
        }
    ...
    /* We need to cache the decimal versions of each necessary division of 10⁵², if we've
     * not seen them before. */
    KrkValue float_decimal_parts = NONE_VAL();
    if (!krk_tableGet_fast(&vm.baseClasses->floatClass->methods, S("__decimals__"),
                           &float_decimal_parts)) {
        krk_push(OBJECT_VAL(krk_newTuple(54)));
        float_decimal_parts = krk_peek(0);

        KrkLong d;
        krk_long_parse_string("10000000000000000000000000000000000000000000000000000", &d,
                              10, 53);

        for (int i = 0; i < 53; ++i) {
            AS_TUPLE(float_decimal_parts)
                ->values.values[AS_TUPLE(float_decimal_parts)->values.count++] =
                make_long_obj(&d);
            if (i != 52) {
                KrkLong o;
        ...
        krk_attachNamedValue(&vm.baseClasses->floatClass->methods, "__decimals__",
                             float_decimal_parts);
        krk_pop();
    }
    ...
    if (e == -1023) {
         ...
         * is -1022, so instead of initializing our counter to have the leading 1, we
         * start with just 0. */
        ...
        krk_long_init_copy(
            &c, AS_long(AS_TUPLE(float_decimal_parts)->values.values[0])->value);
    }
    ...
    for (int i = 0; i < 52; ++i) {
        if (m & (1ULL << (51 - i))) {
            krk_long_add(
                &c, &c,
                AS_long(AS_TUPLE(float_decimal_parts)->values.values[i + 1])->value);
        }
    }

    /* At this point, we know that we have 52 decimal digits to the right of the radix
     * point; this represents the base-10 exponent of our denominator. We want to maintain
     * an exact value for m after turning the base-2 exponent into a base-10 exponent, so
     * if our original base-2 exponent is negative, we might need to add more 0s to the
     * end of both the top and bottom of the fraction - we'll add to b to account for
     * that. */
    int b = 52;

    if (e < 0) {
        /* Repeatedly multiply to increase number of decimal digits by 31, until the
         * resulting binary representation has enough trailing 0 bits we can shift away
         * the negative exponent and still have an exact decimal representation. */
        while (1) {
            ssize_t i = 0;
            while (!_bit_is_set(&c, i))
                i++;
            if (i >= -e)
                break;
            krk_long_mul(
                &c, &c, AS_long(AS_TUPLE(float_decimal_parts)->values.values[53])->value);
            b += 31;
        }
    }
    ...
    int whole_digits =
        ((int)len >= b) ? ten_exponent + 1 : 0; /* digits before radix point */
    int missing_digits =
        (b >= (int)len) ? b - (int)len : 0; /* digits after radix point not in actual */
    ...
    if (!alwaysexp && !noexp) {
        ...
        if (digits == 0)
            digits = 1; /* treat precision of 0 as 1 */
        if (actual > digits) {
        ...
        if (ten_exponent < -4 || ten_exponent >= (int)digits) {
            ...
            if (formatter == ' ' && actual <= (size_t)whole_digits)
                trailing_zeros = 1;
            else
                trailing_zeros = 0;
        }
        ...
        if (missing_digits > (int)digits) {
            ...
            if (round_to(str, len, actual, digits - missing_digits))
                missing_digits--;
            ...
            if (round_to(str, len, actual, digits + whole_digits))
                whole_digits++;
        ...
        if (actual > digits) {
            if (round_to(str, len, actual, digits + 1))
                ten_exponent += 1;
            actual = digits + 1;
        } else {
    ...
    if (!whole_digits)
        ...
        krk_pushStringBuilderStr(&sb, str, whole_digits);
    if (forcedigits || actual > (size_t)whole_digits || trailing_zeros)
        krk_pushStringBuilder(&sb, '.');
    if (missing_digits)
        for (int i = 0; i < missing_digits; ++i)
            krk_pushStringBuilder(&sb, '0');
    if (actual > (size_t)whole_digits)
        krk_pushStringBuilderStr(&sb, str + whole_digits, actual - whole_digits);
    ...
    if (print_exponent) {
        ...
        krk_pushStringBuilderFormat(&sb, "%c%c%s%d", expch, expsign,
                                    abs_ten_exponent < 10 ? "0" : "", abs_ten_exponent);
    }
...
KrkValue krk_parse_float(const char *s, size_t l) {
    ...
    if (c + 3 == l) {
        if (((s[c + 0] | 0x20) == 'n') && ((s[c + 1] | 0x20) == 'a') &&
            ((s[c + 2] | 0x20) == 'n')) {
            return FLOATING_VAL(((union Float){.i = 0x7ff0000000000001ULL}).d); /* nan */
        }
        if (((s[c + 0] | 0x20) == 'i') && ((s[c + 1] | 0x20) == 'n') &&
            ((s[c + 2] | 0x20) == 'f')) {
            return FLOATING_VAL(((union Float){.i = 0x7ff0000000000000ULL}).d *
                                sign); /* inf */
        }
    }

    /* Collect digits or separators before a radix point. */
    while (c < l && ((s[c] >= '0' && s[c] <= '9') || s[c] == '_'))
        c++;
    pe = c;

    /* If we are now at a radix point, collect it and then collect digits after the radix
     * point. */
    if (c < l && s[c] == '.') {
        c++;
        ss = c;
        while (c < l && s[c] >= '0' && s[c] <= '9')
            c++;
        se = c;
    }
    ...
    if (c != l)
        return krk_runtimeError(vm.exceptions->valueError, "invalid literal for float");
    ...
    /* Pack up all the digits from whole and fractional parts into a string so we can
     * parse it with our faster decimal string parsing tools. */
    struct StringBuilder sb = {0};
    ...
    /* If that results in an empty string (because we stripped all of the zeros, and it
     * was only zeros), then replace it with "0" or the parser will be unhappy. */
...
KrkValue krk_float_to_fraction(double d) {
    ...
    /* NaN or Inf */
    if (e == 0x7FF)
        return krk_runtimeError(vm.exceptions->valueError, "unrepresentable");
    if (e == 0) {
...
#define BIND_METHOD(klass, method)                                                       \
    do {                                                                                 \
        krk_defineNative(&_##klass->methods, #method, _##klass##_##method);              \
    } while (0)
#define BIND_STATICMETHOD(klass, method)                                                 \
    do {                                                                                 \
        krk_defineNativeStaticMethod(&_##klass->methods, #method, _##klass##_##method);  \
    } while (0)
...
_noexport void _createAndBind_longClass(void) {
    KrkClass *_long =
        ADD_BASE_CLASS(vm.baseClasses->longClass, "long", vm.baseClasses->intClass);
