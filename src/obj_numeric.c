...
KRK_StaticMethod(int, __new__) {
    ...
    if (!krk_parseArgs("O|V?i?:int", (const char *[]){"", "", "base"}, &cls, &has_x, &x,
                       &has_base, &base))
        return NONE_VAL();
...
_just_x:
    ...
    if (IS_STRING(x)) {
        ...
        if (IS_NONE(result)) {
            return krk_runtimeError(vm.exceptions->valueError,
                                    "invalid literal for int() with base %zd: %R",
                                    (ssize_t)base, x);
        }
        return result;
    }

    if (krk_isInstanceOf(x, KRK_BASE_CLASS(long)))
        return x;
    return krk_runtimeError(vm.exceptions->typeError,
                            "%s() argument must be a string or a number, not '%T'", "int",
                            x);
}
...
KRK_Method(int, __eq__) {
    ...
    if (likely(IS_INTEGER(argv[1])))
        return BOOLEAN_VAL(self == AS_INTEGER(argv[1]));
#ifndef KRK_NO_FLOAT
    else if (IS_FLOATING(argv[1]))
        return BOOLEAN_VAL(self == AS_FLOATING(argv[1]));
#endif
    return NOTIMPL_VAL();
}

KRK_Method(int, __hash__) { return INTEGER_VAL((uint32_t)AS_INTEGER(argv[0])); }
...
const char *krk_parseCommonFormatSpec(struct ParsedFormatSpec *result, const char *spec,
                                      size_t length) {
    result->fill = " ";
    result->fillSize = 1;
    ...
    if (length > 1) {
        ...
        if ((spec[0] & 0xC0) == 0xC0) { /* wider than one byte */
            while ((spec[i] & 0xc0) == 0x80)
                i++; /* count continuation bytes */
        }
...
KrkValue krk_doFormatString(const char *typeName, KrkString *format_spec, int positive,
    ...
    const char *spec =
        krk_parseCommonFormatSpec(&opts, format_spec->chars, format_spec->length);
    if (!spec)
        return NONE_VAL();
    ...
    default:
        return krk_runtimeError(vm.exceptions->valueError,
                                "Unknown format code '%c' for object of type '%s'", *spec,
                                typeName);
    }
    ...
    if (opts.alt && altPrefix && width > 2)
        width -= 2;
    if ((!positive || opts.sign == '+') && width > 1)
        width--;
    ...
    if (prepCallback)
        callback = prepCallback(abs, base);
    ...
    do {
        ...
        if (opts.sep && !(digits % sepcount) &&
            (more || (opts.align == '=' && l < width))) {
...
KRK_Method(int, __format__) {
    ...
    return krk_doFormatString(krk_typeName(argv[0]), format_spec, self >= 0, &abs,
...
#define OVERFLOW_CHECKED_INT_OPERATION(name, operator)                                   \
    ...
    _noexport KrkValue krk_int_op_##name(krk_integer_type a, krk_integer_type b) {       \
        if (likely((int32_t)a == a && (int32_t)b == b)) {                                \
            int32_t result_one = a operator b;                                           \
            int64_t result_two = a operator b;                                           \
            if (likely(result_one == result_two))                                        \
                return INTEGER_VAL(result_two);                                          \
        }                                                                                \
...
#define BASIC_BIN_OP(name, operator)                                                     \
    KRK_Method(int, __##name##__) {                                                      \
        if (likely(IS_INTEGER(argv[1])))                                                 \
            return krk_int_op_##name(self, AS_INTEGER(argv[1]));                         \
        else if (likely(IS_FLOATING(argv[1])))                                           \
            return MAYBE_FLOAT(                                                          \
                FLOATING_VAL((double)self operator AS_FLOATING(argv[1])));               \
        return NOTIMPL_VAL();                                                            \
    }                                                                                    \
    KRK_Method(int, __r##name##__) {                                                     \
        if (likely(IS_INTEGER(argv[1])))                                                 \
            return krk_int_op_##name(AS_INTEGER(argv[1]), self);                         \
        else if (likely(IS_FLOATING(argv[1])))                                           \
            return MAYBE_FLOAT(                                                          \
                FLOATING_VAL(AS_FLOATING(argv[1]) operator(double) self));               \
        return NOTIMPL_VAL();                                                            \
    }
...
#define INT_ONLY_BIN_OP(name, operator)                                                  \
    KRK_Method(int, __##name##__) {                                                      \
        if (likely(IS_INTEGER(argv[1])))                                                 \
            return INTEGER_VAL(self operator AS_INTEGER(argv[1]));                       \
        return NOTIMPL_VAL();                                                            \
    }                                                                                    \
    KRK_Method(int, __r##name##__) {                                                     \
        if (likely(IS_INTEGER(argv[1])))                                                 \
            return INTEGER_VAL(AS_INTEGER(argv[1]) operator self);                       \
        return NOTIMPL_VAL();                                                            \
    }
...
#define COMPARE_OP(name, operator)                                                       \
    KRK_Method(int, __##name##__) {                                                      \
        if (likely(IS_INTEGER(argv[1])))                                                 \
            return BOOLEAN_VAL(self operator AS_INTEGER(argv[1]));                       \
        else if (likely(IS_FLOATING(argv[1])))                                           \
            return MAYBE_FLOAT(BOOLEAN_VAL((double)self operator AS_FLOATING(argv[1]))); \
        return NOTIMPL_VAL();                                                            \
    }
...
#define DEFER_TO_LONG(name)                                                              \
    ...
    KRK_Method(int, __##name##__) {                                                      \
        if (likely(IS_INTEGER(argv[1])))                                                 \
            return krk_long_coerced_##name(self, AS_INTEGER(argv[1]));                   \
        return NOTIMPL_VAL();                                                            \
    }                                                                                    \
    KRK_Method(int, __r##name##__) {                                                     \
        if (likely(IS_INTEGER(argv[1])))                                                 \
            return krk_long_coerced_##name(AS_INTEGER(argv[1]), self);                   \
        return NOTIMPL_VAL();                                                            \
    }
...
KRK_Method(int, __truediv__) {
    ...
    if (likely(IS_INTEGER(argv[1]))) {
        ...
        if (unlikely(b == 0))
            return krk_runtimeError(vm.exceptions->zeroDivisionError,
...
KRK_Method(int, __rtruediv__) {
    ...
    if (unlikely(self == 0))
        ...
        return FLOATING_VAL((double)AS_INTEGER(argv[1]) / (double)self);
    else if (likely(IS_FLOATING(argv[1])))
        return FLOATING_VAL(AS_FLOATING(argv[1]) / (double)self);
    return NOTIMPL_VAL();
}
#endif
...
static KrkValue _krk_int_div(krk_integer_type a, krk_integer_type b) {
    ...
    if ((a < 0) != (b < 0)) {
        /* If signs don't match, the result is negative, and rounding down means away from
         * 0... */
...
KRK_Method(int, __mod__) {
    METHOD_TAKES_EXACTLY(1);
    if (likely(IS_INTEGER(argv[1])))
        return _krk_int_mod(self, AS_INTEGER(argv[1]));
    return NOTIMPL_VAL();
}
...
KRK_Method(int, __rmod__) {
    METHOD_TAKES_EXACTLY(1);
    if (likely(IS_INTEGER(argv[1])))
        return _krk_int_mod(AS_INTEGER(argv[1]), self);
    return NOTIMPL_VAL();
}
...
KRK_Method(int, __rfloordiv__) {
    ...
    if (unlikely(self == 0))
        ...
        return _krk_int_div(AS_INTEGER(argv[1]), self);
    else if (likely(IS_FLOATING(argv[1])))
        return MAYBE_FLOAT(
            FLOATING_VAL(__builtin_floor(AS_FLOATING(argv[1]) / (double)self)));
    return NOTIMPL_VAL();
}
...
KRK_Method(int, __invert__) { return INTEGER_VAL(~self); }

KRK_Method(int, __neg__) { return INTEGER_VAL(-self); }

KRK_Method(int, __abs__) { return self < 0 ? INTEGER_VAL(-self) : INTEGER_VAL(self); }

KRK_Method(int, __pos__) { return argv[0]; }
...
KRK_StaticMethod(float, __new__) {
    ...
    if (IS_FLOATING(argv[1]))
        return argv[1];
    if (IS_INTEGER(argv[1]))
        return FLOATING_VAL(AS_INTEGER(argv[1]));
    if (IS_BOOLEAN(argv[1]))
...
KRK_Method(float, __repr__) { return krk_double_to_string(self, 16, ' ', 0, 0); }

KRK_Method(float, __format__) {
    char *format_spec;
    size_t format_spec_length;
    if (!krk_parseArgs(".s#", (const char *[]){"format_spec"}, &format_spec,
                       &format_spec_length))
        return NONE_VAL();
    ...
    KrkValue result =
        krk_double_to_string(self, digits, formatter, opts.sign == '+', forcedigits);
    if (!IS_STRING(result) || !opts.width)
        return result;

    krk_push(result);

    /* Calculate how much padding we need to add. */
    size_t avail = (size_t)opts.width > AS_STRING(result)->length
                       ? (size_t)opts.width - AS_STRING(result)->length
                       : 0;
    ...
    if (opts.align == '<') {
        ...
        before = avail;
        if (avail && AS_STRING(result)->length &&
            (AS_CSTRING(result)[0] == '-' || AS_CSTRING(result)[0] == '+')) {
    ...
    for (size_t i = 0; i < before; ++i)
        krk_pushStringBuilderStr(&sb, opts.fill, opts.fillSize);
    krk_pushStringBuilderStr(&sb, AS_CSTRING(result) + hassign,
                             AS_STRING(result)->length - hassign);
...
KRK_Method(float, __eq__) {
    METHOD_TAKES_EXACTLY(1);
    if (IS_INTEGER(argv[1]))
        return BOOLEAN_VAL(self == (double)AS_INTEGER(argv[1]));
    else if (IS_FLOATING(argv[1]))
        return BOOLEAN_VAL(self == AS_FLOATING(argv[1]));
    return NOTIMPL_VAL();
}

KRK_Method(float, __hash__) { return INTEGER_VAL((uint32_t)self); }

KRK_Method(float, __neg__) { return FLOATING_VAL(-self); }
...
#    define BASIC_BIN_OP(name, operator)                                                 \
        KRK_Method(float, __##name##__) {                                                \
            ...
            if (likely(IS_FLOATING(argv[1])))                                            \
                return FLOATING_VAL(self operator AS_FLOATING(argv[1]));                 \
            else if (likely(IS_INTEGER(argv[1])))                                        \
                return FLOATING_VAL(self operator(double) AS_INTEGER(argv[1]));          \
            return NOTIMPL_VAL();                                                        \
        }                                                                                \
        KRK_Method(float, __r##name##__) {                                               \
            METHOD_TAKES_EXACTLY(1);                                                     \
            if (likely(IS_FLOATING(argv[1])))                                            \
                return FLOATING_VAL(AS_FLOATING(argv[1]) operator self);                 \
            else if (likely(IS_INTEGER(argv[1])))                                        \
                return FLOATING_VAL((double)AS_INTEGER(argv[1]) operator self);          \
            return NOTIMPL_VAL();                                                        \
        }
...
#    define COMPARE_OP(name, operator)                                                   \
        KRK_Method(float, __##name##__) {                                                \
            METHOD_TAKES_EXACTLY(1);                                                     \
            if (likely(IS_FLOATING(argv[1])))                                            \
                return BOOLEAN_VAL(self operator AS_FLOATING(argv[1]));                  \
            else if (likely(IS_INTEGER(argv[1])))                                        \
                return BOOLEAN_VAL(self operator(double) AS_INTEGER(argv[1]));           \
            return NOTIMPL_VAL();                                                        \
        }
...
KRK_Method(float, __rtruediv__) {
    ...
    if (unlikely(self == 0.0))
        ...
        return FLOATING_VAL(AS_FLOATING(argv[1]) / self);
    else if (likely(IS_INTEGER(argv[1])))
        return FLOATING_VAL((double)AS_INTEGER(argv[1]) / self);
    return NOTIMPL_VAL();
}
...
KRK_Method(float, __rfloordiv__) {
    ...
    if (unlikely(self == 0.0))
        ...
        return FLOATING_VAL(__builtin_floor(AS_FLOATING(argv[1]) / self));
    return NOTIMPL_VAL();
}

KRK_Method(float, __pos__) { return argv[0]; }

extern KrkValue krk_float_to_fraction(double d);
KRK_Method(float, as_integer_ratio) { return krk_float_to_fraction(self); }
#endif
...
KRK_Method(bool, __repr__) { return OBJECT_VAL((self ? S("True") : S("False"))); }
...
KRK_StaticMethod(NoneType, __new__) {
    if (argc > 1)
        return krk_runtimeError(vm.exceptions->argumentError, "%s takes no arguments",
                                "NoneType");
    return NONE_VAL();
}

KRK_Method(NoneType, __repr__) { return OBJECT_VAL(S("None")); }

KRK_Method(NoneType, __hash__) { return INTEGER_VAL((uint32_t)AS_INTEGER(argv[0])); }
...
KRK_Method(NotImplementedType, __repr__) { return OBJECT_VAL(S("NotImplemented")); }

KRK_Method(NotImplementedType, __hash__) { return INTEGER_VAL(0); }

KRK_Method(NotImplementedType, __eq__) {
    METHOD_TAKES_EXACTLY(1);
    if (IS_NOTIMPL(argv[1]))
        return BOOLEAN_VAL(1);
    return NOTIMPL_VAL();
}
...
_noexport void _createAndBind_numericClasses(void) {
    KrkClass *_int =
        ADD_BASE_CLASS(vm.baseClasses->intClass, "int", vm.baseClasses->objectClass);
    ...
    KrkClass *_float =
        ADD_BASE_CLASS(vm.baseClasses->floatClass, "float", vm.baseClasses->objectClass);
    ...
    KrkClass *_bool =
        ADD_BASE_CLASS(vm.baseClasses->boolClass, "bool", vm.baseClasses->intClass);
    ...
    KrkClass *_NoneType = ADD_BASE_CLASS(vm.baseClasses->noneTypeClass, "NoneType",
    ...
    KrkClass *_NotImplementedType = ADD_BASE_CLASS(
        vm.baseClasses->notImplClass, "NotImplementedType", vm.baseClasses->objectClass);
