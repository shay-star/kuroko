...
static inline uint32_t decode(uint32_t *state, uint32_t *codep, uint32_t byte) {
    ...
    static const int mask_bytes[32] = {0x7F, 0x7F, 0x7F, 0x7F, 0x7F, 0x7F, 0x7F, 0x7F,
                                       0x7F, 0x7F, 0x7F, 0x7F, 0x7F, 0x7F, 0x7F, 0x7F,
                                       0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                       0x1F, 0x1F, 0x1F, 0x1F, 0x0F, 0x0F, 0x07, 0x00};

    static const int next[5] = {0, 1, 0, 2, 3};

    if (*state == UTF8_ACCEPT) {
        if (byte >= 0x80 && byte <= 0xC1)
            goto _reject;
...
void *krk_unicodeString(KrkString *string) {
    if (string->codes)
        return string->codes;
    else if ((string->obj.flags & KRK_OBJ_FLAGS_STRING_MASK) == KRK_OBJ_FLAGS_STRING_UCS1)
        _readyUCS1(string);
    else if ((string->obj.flags & KRK_OBJ_FLAGS_STRING_MASK) == KRK_OBJ_FLAGS_STRING_UCS2)
        _readyUCS2(string);
    else if ((string->obj.flags & KRK_OBJ_FLAGS_STRING_MASK) == KRK_OBJ_FLAGS_STRING_UCS4)
        _readyUCS4(string);
    else
...
uint32_t krk_unicodeCodepoint(KrkString *string, size_t index) {
    ...
    case KRK_OBJ_FLAGS_STRING_UCS1:
        return ((uint8_t *)string->codes)[index];
    case KRK_OBJ_FLAGS_STRING_UCS2:
        return ((uint16_t *)string->codes)[index];
    case KRK_OBJ_FLAGS_STRING_UCS4:
        return ((uint32_t *)string->codes)[index];
    default:
        krk_runtimeError(vm.exceptions->valueError, "Internal string error.");
        return 0;
    }
...
static KrkString *allocateString(char *chars, size_t length, uint32_t hash) {
    ...
    string->codes = NULL;
    if (type == KRK_OBJ_FLAGS_STRING_ASCII)
        string->codes = string->chars;
...
KrkString *krk_copyString(const char *chars, size_t length) {
    ...
    _obtain_lock(_stringLock);
    KrkString *interned =
        krk_tableFindString(&vm.strings, chars ? chars : "", length, hash);
    if (interned) {
        _release_lock(_stringLock);
        return interned;
    }
    ...
    if (result->chars != heapChars)
        free(heapChars);
    _release_lock(_stringLock);
    return result;
}

KrkString *krk_takeStringVetted(char *chars, size_t length, size_t codesLength,
...
KrkInstance *krk_newInstance(KrkClass *_class) {
    KrkInstance *instance =
        (KrkInstance *)allocateObject(_class->allocSize, KRK_OBJ_INSTANCE);
