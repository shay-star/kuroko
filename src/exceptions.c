...
KRK_Method(SyntaxError, __str__) {
    ...
    if (!krk_tableGet(&self->fields, OBJECT_VAL(S("file")), &file) || !IS_STRING(file))
        goto _badSyntaxError;
    if (!krk_tableGet(&self->fields, OBJECT_VAL(S("line")), &line) || !IS_STRING(line))
        goto _badSyntaxError;
    if (!krk_tableGet(&self->fields, OBJECT_VAL(S("lineno")), &lineno) ||
        !IS_INTEGER(lineno))
        goto _badSyntaxError;
    if (!krk_tableGet(&self->fields, OBJECT_VAL(S("colno")), &colno) ||
        !IS_INTEGER(colno))
        goto _badSyntaxError;
    if (!krk_tableGet(&self->fields, OBJECT_VAL(S("arg")), &arg) || !IS_STRING(arg))
        goto _badSyntaxError;
    if (!krk_tableGet(&self->fields, OBJECT_VAL(S("func")), &func))
        goto _badSyntaxError;
    if (!krk_tableGet(&self->fields, OBJECT_VAL(S("width")), &width) ||
        !IS_INTEGER(width))
        goto _badSyntaxError;

    if (AS_INTEGER(colno) <= 0)
        colno = INTEGER_VAL(1);

    int definitelyNotFullWidth =
        !(AS_STRING(line)->obj.flags & KRK_OBJ_FLAGS_STRING_MASK);
    ...
    KrkValue formattedString =
        krk_string_format(9,
                          (KrkValue[]){krk_peek(3), file, lineno, krk_peek(0), line,
                                       krk_peek(2), krk_peek(4), krk_peek(1), arg},
                          0);
    krk_pop(); /* instr */
    krk_pop(); /* class */
    krk_pop(); /* spaces */
...
_noexport void _createAndBind_exceptions(void) {
    ...
    ADD_EXCEPTION_CLASS(vm.exceptions->baseException, BaseException,
    ...
    /* KeyboardInterrupt is currently the only thing that directly inherits from
     * BaseException. */
    ADD_EXCEPTION_CLASS(vm.exceptions->keyboardInterrupt, KeyboardInterrupt,
                        BaseException);
    ...
    ADD_EXCEPTION_CLASS(vm.exceptions->notImplementedError, NotImplementedError,
                        Exception);
    ...
    ADD_EXCEPTION_CLASS(vm.exceptions->syntaxError, SyntaxError,
                        vm.exceptions->Exception);
...
static void dumpInnerException(KrkValue exception, int depth) {
    ...
    if (IS_INSTANCE(exception)) {
        ...
        if (krk_tableGet(&AS_INSTANCE(exception)->fields, OBJECT_VAL(S("__cause__")),
                         &inner) &&
            !IS_NONE(inner)) {
            dumpInnerException(inner, depth + 1);
            fprintf(stderr, "\nThe above exception was the direct cause of the following "
                            "exception:\n\n");
        } else if (krk_tableGet(&AS_INSTANCE(exception)->fields,
                                OBJECT_VAL(S("__context__")), &inner) &&
                   !IS_NONE(inner)) {
            dumpInnerException(inner, depth + 1);
        ...
        if (krk_tableGet(&AS_INSTANCE(exception)->fields, OBJECT_VAL(S("traceback")),
                         &tracebackEntries) &&
            ...
            for (size_t i = 0; i < AS_LIST(tracebackEntries)->count; ++i) {
                ...
                /* Quietly skip invalid entries as we don't want to bother printing
                 * explanatory text for them */
                if (!IS_TUPLE(AS_LIST(tracebackEntries)->values[i]))
                    continue;
                KrkTuple *entry = AS_TUPLE(AS_LIST(tracebackEntries)->values[i]);
                if (entry->values.count != 2)
                    continue;
                if (!IS_CLOSURE(entry->values.values[0]))
                    continue;
                if (!IS_INTEGER(entry->values.values[1]))
                    continue;
                ...
                fprintf(
                    stderr, "  File \"%s\", line %d, in %s\n",
                    (function->chunk.filename ? function->chunk.filename->chars : "?"),
                    lineNo, (function->name ? function->name->chars : "(unnamed)"));
...
#    ifndef KRK_DISABLE_DEBUG
                                ...
                                if (krk_debug_expressionUnderline(function, &start,
                                                                  &midStart, &midEnd,
                                                                  &end, instruction)) {
                                    fprintf(stderr, "    ");
                                    for (; j < start; ++j)
...
static void attachTraceback(void) {
    if (IS_INSTANCE(krk_currentThread.currentException)) {
        ...
        if (krk_tableGet(&theException->fields, OBJECT_VAL(S("traceback")),
                         &tracebackList)) {
        ...
        if (krk_currentThread.frameCount) {
            ...
            if (krk_currentThread.stackTop > krk_currentThread.stack) {
                size_t stackOffset =
                    krk_currentThread.stackTop - krk_currentThread.stack - 1;
                while (
                    stackOffset > 0 &&
                ...
                while (frameOffset > 0 &&
                       krk_currentThread.frames[frameOffset].slots > stackOffset)
                    frameOffset--;
            }
            ...
            for (size_t i = frameOffset; i < krk_currentThread.frameCount; i++) {
                ...
                tbEntry->values.values[tbEntry->values.count++] =
                    OBJECT_VAL(frame->closure);
                tbEntry->values.values[tbEntry->values.count++] =
                    INTEGER_VAL(frame->ip - frame->closure->function->chunk.code - 1);
...
KrkValue krk_runtimeError(KrkClass *type, const char *fmt, ...) {
    ...
    krk_attachNamedValue(&exceptionObject->fields, "arg",
                         krk_valuesSame(msg, KWARGS_VAL(0)) ? finishStringBuilder(&sb)
                                                            : msg);
