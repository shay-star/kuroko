...
int krk_extractSlicer(const char *_method_name, KrkValue slicerVal,
...
KRK_Method(slice, __repr__) {
    ...
    if (((KrkObj *)self)->flags & KRK_OBJ_FLAGS_IN_REPR)
        return OBJECT_VAL(S("slice(...)"));
    ((KrkObj *)self)->flags |= KRK_OBJ_FLAGS_IN_REPR;
    struct StringBuilder sb = {0};
    pushStringBuilderStr(&sb, "slice(", 6);
    /* start */
    if (!krk_pushStringBuilderFormat(&sb, "%R", self->start))
        goto _error;
    pushStringBuilderStr(&sb, ", ", 2);
    if (!krk_pushStringBuilderFormat(&sb, "%R", self->end))
        goto _error;
    pushStringBuilderStr(&sb, ", ", 2);
    if (!krk_pushStringBuilderFormat(&sb, "%R", self->step))
        goto _error;
...
KRK_StaticMethod(ellipsis, __new__) {
    ...
    if (!krk_parseArgs("O!", (const char *[]){"cls"}, KRK_BASE_CLASS(type), &_class))
        return NONE_VAL();
    if (!krk_isSubClass(_class, KRK_BASE_CLASS(ellipsis))) {
        return krk_runtimeError(vm.exceptions->typeError, "%S is not a subclass of %S",
                                _class->name, KRK_BASE_CLASS(ellipsis)->name);
    }

    KrkValue out;
    if (!krk_tableGet_fast(&vm.builtins->fields, S("Ellipsis"), &out))
        return krk_runtimeError(vm.exceptions->typeError, "Ellipsis is missing");
    return out;
}

KRK_Method(ellipsis, __repr__) { return OBJECT_VAL(S("Ellipsis")); }

_noexport void _createAndBind_sliceClass(void) {
    KrkClass *slice =
        ADD_BASE_CLASS(KRK_BASE_CLASS(slice), "slice", KRK_BASE_CLASS(object));
    ...
    KrkClass *ellipsis =
        ADD_BASE_CLASS(KRK_BASE_CLASS(ellipsis), "ellipsis", KRK_BASE_CLASS(object));
    krk_attachNamedObject(&vm.builtins->fields, "Ellipsis",
                          (KrkObj *)krk_newInstance(ellipsis));
