...
KrkInstance *krk_buildGenerator(KrkClosure *closure, KrkValue *argsIn, size_t argCount) {
    ...
    struct generator *self =
        (struct generator *)krk_newInstance(KRK_BASE_CLASS(generator));
    self->args = args;
    self->argCount = argCount;
    self->closure = closure;
    self->ip = self->closure->function->chunk.code;
    self->result = NONE_VAL();
    self->type = closure->function->obj.flags & (KRK_OBJ_FLAGS_CODEOBJECT_IS_GENERATOR |
                                                 KRK_OBJ_FLAGS_CODEOBJECT_IS_COROUTINE);
    return (KrkInstance *)self;
}

FUNC_SIG(generator, __init__) {
    return krk_runtimeError(vm.exceptions->typeError, "cannot create '%s' instances",
                            "generator");
}
...
KRK_Method(generator, __repr__) {
    ...
    if (self->type == KRK_OBJ_FLAGS_CODEOBJECT_IS_COROUTINE) {
        /* Regular coroutine */
        typeStr = "coroutine";
    } else if (self->type == (KRK_OBJ_FLAGS_CODEOBJECT_IS_COROUTINE |
                              KRK_OBJ_FLAGS_CODEOBJECT_IS_GENERATOR)) {
        typeStr = "async_generator";
    }

    return krk_stringFromFormat("<%s object %S at %p>", typeStr,
                                self->closure->function->name, (void *)self);
}
...
KRK_Method(generator, __call__) {
    ...
    while (krk_currentThread.openUpvalues != NULL &&
           krk_currentThread.openUpvalues->location >= (int)frame->slots) {
    ...
    /* Save stack entries */
    memcpy(self->args, krk_currentThread.stackTop - self->argCount,
           sizeof(KrkValue) * self->argCount);
    self->ip = frame->ip;
...
int krk_getAwaitable(void) {
    if (IS_generator(krk_peek(0)) &&
        AS_generator(krk_peek(0))->type == KRK_OBJ_FLAGS_CODEOBJECT_IS_COROUTINE) {
        /* Good to go */
        return 1;
    }
    ...
    if (!IS_NONE(method)) {
        ...
        if (!_type || !_type->_iter) {
            krk_runtimeError(vm.exceptions->attributeError,
                             "__await__ returned non-iterator of type '%T'", krk_peek(0));
            return 0;
        }
    } else {
        krk_runtimeError(vm.exceptions->attributeError, "'%T' object is not awaitable",
                         krk_peek(0));
        return 0;
    }
...
_noexport void _createAndBind_generatorClass(void) {
    KrkClass *generator = ADD_BASE_CLASS(vm.baseClasses->generatorClass, "generator",
