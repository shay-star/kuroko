...
KRK_Method(set, __init__) {
    ...
    if (argc == 2) {
        if (krk_unpackIterable(argv[1], self, _set_init_callback))
            return NONE_VAL();
    }
    return NONE_VAL();
}
...
KRK_Method(set, __eq__) {
    ...
    for (unsigned int i = 0; i < self->entries.capacity; ++i) {
        if (IS_KWARGS(self->entries.entries[i].key))
            continue;
        if (!krk_tableGet(&them->entries, self->entries.entries[i].key, &v))
            return BOOLEAN_VAL(0);
    }
...
KRK_Method(set, __gt__) {
    ...
    for (unsigned int i = 0; i < them->entries.capacity; ++i) {
        if (IS_KWARGS(them->entries.entries[i].key))
            continue;
        if (!krk_tableGet(&self->entries, them->entries.entries[i].key, &v))
            return BOOLEAN_VAL(0);
    }
    return BOOLEAN_VAL(1);
}
...
KRK_Method(setiterator, __call__) {
    ...
    if (unlikely(!IS_set(self->set)))
        return argv[0];

    do {
        if (self->i >= AS_set(self->set)->entries.capacity)
            return argv[0];
...
_noexport void _createAndBind_setClass(void) {
    KrkClass *set = krk_makeClass(vm.builtins, &KRK_BASE_CLASS(set), "set",
    ...
    KRK_DOC(BIND_METHOD(set, remove), "@brief Remove an element from the set.\n"
                                      "@arguments value\n\n"
                                      "Removes @p value from the set, raising @ref "
                                      "KeyError if it is not a member of the set.");
    KRK_DOC(BIND_METHOD(set, discard), "@brief Remove an element from the set, quietly.\n"
                                       "@arguments value\n\n"
                                       "Removes @p value from the set, without raising "
                                       "an exception if it is not a member.");
    KRK_DOC(BIND_METHOD(set, clear), "@brief Empty the set.\n\n"
    ...
    KrkClass *setiterator = krk_makeClass(vm.builtins, &KRK_BASE_CLASS(setiterator),
                                          "setiterator", vm.baseClasses->objectClass);
