...
/**
 * Exposed method called to produce dictionaries from `{expr: expr, ...}` sequences in
 * managed code. Expects arguments as `key,value,key,value`...
 */
KrkValue krk_dict_of(int argc, const KrkValue argv[], int hasKw) {
    ...
    if (argc) {
        ...
        if ((1UL << powerOfTwoCapacity) != capacity)
            powerOfTwoCapacity++;
...
static int unpackKeyValuePair(void *self, const KrkValue *pairs, size_t count) {
    ...
    for (size_t i = 0; i < count; ++i) {
        if (krk_unpackIterable(pairs[i], &context, _keyvalue_pair_callback))
            return 1;
    }
...
KRK_Method(dict, __init__) {
    ...
    if (argc > 1) {
        if (krk_unpackIterable(argv[1], self, unpackKeyValuePair))
            return NONE_VAL();
    }
...
KRK_Method(dict, __eq__) {
    ...
    for (unsigned int i = 0; i < self->entries.capacity; ++i) {
        if (IS_KWARGS(self->entries.entries[i].key))
            continue;
        KrkValue val;
        if (!krk_tableGet(&them->entries, self->entries.entries[i].key, &val))
            return BOOLEAN_VAL(0);
        if (!krk_valuesSameOrEqual(self->entries.entries[i].value, val))
            return BOOLEAN_VAL(0);
    }
...
KRK_Method(dict, __repr__) {
    ...
    for (size_t i = 0; i < len; ++i) {
        ...
        c++;
        if (!krk_pushStringBuilderFormat(&sb, "%R", entry->key))
            goto _error;
        pushStringBuilderStr(&sb, ": ", 2);
        if (!krk_pushStringBuilderFormat(&sb, "%R", entry->value))
            goto _error;
    }
...
KRK_Method(dictitems, __call__) {
    do {
        if (self->i >= AS_DICT(self->dict)->used)
            return argv[0];
...
KRK_Method(dictvalues, __repr__) {
    METHOD_TAKES_NONE();
    if (((KrkObj *)self)->flags & KRK_OBJ_FLAGS_IN_REPR)
        return OBJECT_VAL(S("dictvalues([...])"));
    ((KrkObj *)self)->flags |= KRK_OBJ_FLAGS_IN_REPR;
    struct StringBuilder sb = {0};
...
_noexport void _createAndBind_dictClass(void) {
    KrkClass *dict =
        ADD_BASE_CLASS(vm.baseClasses->dictClass, "dict", vm.baseClasses->objectClass);
    ...
    krk_defineNative(&dict->methods, "__class_getitem__", krk_GenericAlias)->obj.flags |=
    ...
    KrkClass *dictitems = ADD_BASE_CLASS(vm.baseClasses->dictitemsClass, "dictitems",
    ...
    KrkClass *dictkeys = ADD_BASE_CLASS(vm.baseClasses->dictkeysClass, "dictkeys",
                                        vm.baseClasses->objectClass);
    ...
    KrkClass *dictvalues = ADD_BASE_CLASS(vm.baseClasses->dictvaluesClass, "dictvalues",
                                          vm.baseClasses->objectClass);
