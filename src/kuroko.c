...
KRK_Function(paste) {
    ...
    if (!krk_parseArgs("|p", (const char *[]){"enabled"}, &enabled))
        return NONE_VAL();
...
static void printResult(FILE *file, KrkValue result) {
    struct StringBuilder sb = {0};
    if (!krk_pushStringBuilderFormat(
            &sb, noColor ? " => %R\n" : " \033[1;90m=> %R\033[0m\n", result)) {
        krk_dumpTraceback();
    } else {
...
static KrkValue findFromProperty(KrkValue current, KrkToken next) {
    ...
    krk_push(member);
    KrkValue value =
        krk_valueGetAttribute_default(current, AS_CSTRING(member), NONE_VAL());
    krk_pop();
    return value;
}

static char *syn_krk_keywords[] = {
    "and",     "class",  "def",   "else",     "for",   "if",     "in",
    "import",  "del",    "let",   "not",      "or",    "return", "while",
    "try",     "except", "raise", "continue", "break", "as",     "from",
    "elif",    "lambda", "with",  "is",       "pass",  "assert", "yield",
    "finally", "async",  "await", "True",     "False", "None",   NULL};
...
static void tab_complete_func(rline_context_t *c) {
    ...
    if (c->offset) {
        ...
        } while (space[count - 1].type != TOKEN_EOF &&
                 space[count - 1].type != TOKEN_ERROR);
        ...
        if (space[count - base].type == TOKEN_DOT) {
            ...
            base--;
                   ...
                   space[count - base].type <= TOKEN_WITH) {
        ...
        while (n < count) {
            if (space[count - n - 1].type != TOKEN_DOT)
                break;
            n++;
            if (n == count)
                break;
            if (space[count - n - 1].type != TOKEN_IDENTIFIER)
                break;
            n++;
        }
        ...
        if (n <= count) {
            ...
            while (n > base) {
                ...
                if (IS_NONE(next)) {
                    ...
                    if (!krk_tableGet_fast(&vm.builtins->fields,
                                           krk_copyString(space[count - n].start,
                                                          space[count - n].literalWidth),
                                           &next) ||
                        IS_NONE(next)) {
                        goto _cleanup;
                    }
            ...
            if (isGlobal && n < count &&
                (space[count - n - 1].type == TOKEN_IMPORT ||
                 space[count - n - 1].type == TOKEN_FROM)) {
                ...
                for (size_t i = 0; i < vm.modules.capacity; ++i) {
                    ...
                    if (IS_KWARGS(entry->key))
                        continue;
                    krk_attachNamedValue(&modules->fields, AS_CSTRING(entry->key),
                                         NONE_VAL());
                }
            }

            /* Now figure out what we're completing - did we already have a partial symbol
             * name? */
            int length = (space[count - base].type == TOKEN_DOT)
                             ? 0
                             : (space[count - base].length);
            ...
            for (;;) {
                ...
                if (isGlobal && AS_OBJECT(root) == (KrkObj *)krk_currentThread.module) {
                    ...
                    KrkInstance *fakeKeywordsObject =
                        krk_newInstance(vm.baseClasses->objectClass);
                    root = OBJECT_VAL(fakeKeywordsObject);
                    krk_push(root);
                    for (char **keyword = syn_krk_keywords; *keyword; keyword++) {
                        krk_attachNamedValue(&fakeKeywordsObject->fields, *keyword,
                                             NONE_VAL());
                    }
                    continue;
                } else {
        ...
        _toomany:
            ...
            if (matchCount == 1) {
                ...
                if (j == length) {
                    ...
                    int maxWidth = 0;
                    for (int i = 0; i < matchCount; ++i) {
                        if ((int)strlen(matches[i]) > maxWidth)
                            maxWidth = strlen(matches[i]);
                    }
                    /* Now how many can we fit in a screen */
                    int colsPerLine =
                        rline_terminal_width / (maxWidth + 2); /* +2 for the spaces */
                    fprintf(stderr, "\n");
                    int column = 0;
                    for (int i = 0; i < matchCount; ++i) {
        ...
        if (!strcmp(buf, "c") || !strcmp(buf, "continue")) {
            ...
            if (!arg) {
                ...
                if (expression) {
                    ...
                    krk_push(OBJECT_VAL(krk_newClosure(
                        expression, OBJECT_VAL(krk_currentThread.module))));
                    krk_swap(1);
                    krk_pop();
            ...
            *lineno = '\0';
            ...
            int result = krk_debug_addBreakpointFileLine(
                krk_copyString(filename, strlen(filename)), lineInt, flags);
            ...
            if (!strcmp(arg, "breakpoints")) {
                ...
                int breakIndex = 0;
                while (1) {
                    int result = krk_debug_examineBreakpoint(breakIndex, &codeObject,
                                                             &offset, &flags, &enabled);
                    if (result == -1)
                        break;
                    if (result == -2)
                        continue;
                    ...
                    fprintf(stderr, "%-4d in %s+%d %s %s\n", breakIndex,
                            codeObject->name->chars, (int)offset,
...
static void bindSignalHandlers(void) {
    ...
    sigIntAction.sa_flags = 0; /* Do not restore the default, do not restart syscalls, do
                                  not pass go, do not collect $500 */
    sigaction(SIGINT,          /* ^C for keyboard interrupts */
              &sigIntAction, NULL);
    ...
    sigaction(SIGTRAP, &sigTrapAction, NULL);
#else
...
_finishArgs:
    ...
    if (argc == optind)
        krk_push(OBJECT_VAL(krk_copyString("", 0)));
    for (int arg = optind; arg < argc; ++arg) {
        krk_push(OBJECT_VAL(krk_copyString(argv[arg], strlen(argv[arg]))));
    }
    KrkValue argList = krk_callNativeOnStack(
        argc - optind + (optind == argc),
        &krk_currentThread.stackTop[-(argc - optind + (optind == argc))], 0, krk_list_of);
    krk_push(argList);
    krk_attachNamedValue(&vm.system->fields, "argv", argList);
    krk_pop();
    for (int arg = optind; arg < argc + (optind == argc); ++arg)
        krk_pop();
...
#    define BUNDLED(name)                                                                \
        do {                                                                             \
            ...
            krk_attachNamedObject(&AS_INSTANCE(moduleOut)->fields, "__name__",           \
                                  (KrkObj *)krk_copyString(#name, sizeof(#name) - 1));   \
            krk_attachNamedValue(&AS_INSTANCE(moduleOut)->fields, "__file__",            \
                                 NONE_VAL());                                            \
        } while (0)
    KRK_BUNDLE_LIBS
#    undef BUNDLED
    ...
    KRK_DOC(
        ...
        "If a prompt is provided, it will be printed without a line feed before "
        "requesting "
        "input. If @c rline is available, the prompt will be passed to the library as "
        "the "
        "left-hand prompt string. If not provided, @p promptwidth will default to the "
        "width "
        "of @p prompt in codepoints; if you are providing a prompt with escape "
        "characters or "
    ...
    if ((!moduleAsMain && !runCmd && optind == argc) || inspectAfter) {
        ...
        KRK_DOC(BIND_FUNC(vm.builtins, exit),
        ...
        if (vm.system) {
            ...
            fprintf(stdout, "Kuroko %s (%s) with %s\n", AS_CSTRING(version),
                    AS_CSTRING(builddate), AS_CSTRING(buildenv));
        }

        fprintf(stdout, "Type `help` for guidance, `paste()` to toggle automatic "
                        "indentation, `license` for copyright information.\n");
...
#endif
                if (krk_currentThread.flags & KRK_THREAD_SIGNALLED) {
                    /* We should actually be properly raising the interrupt and printing
                     * it with an empty traceback, but whatever. */
