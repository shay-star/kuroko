...
void *krk_reallocate(void *ptr, size_t old, size_t new) {
    ...
    if (new > old && ptr != krk_currentThread.stack &&&krk_currentThread == vm.threads &&
...
static void freeObject(KrkObj *object) {
    ...
    case KRK_OBJ_STRING: {
        ...
        if (string->codes && string->codes != string->chars)
            free(string->codes);
    ...
    case KRK_OBJ_CODEOBJECT: {
        ...
        KRK_FREE_ARRAY(KrkExpressionsMap, function->expressions,
                       function->expressionsCapacity);
        KRK_FREE_ARRAY(KrkOverlongJump, function->overlongJumps,
                       function->overlongJumpsCapacity);
...
void krk_markObject(KrkObj *object) {
    if (!object)
        return;
    if (object->flags & KRK_OBJ_FLAGS_IS_MARKED)
        return;
...
static void blackenObject(KrkObj *object) {
    ...
    case KRK_OBJ_INSTANCE: {
        ...
        if (((KrkInstance *)object)->_class->_ongcscan)
            ((KrkInstance *)object)->_class->_ongcscan((KrkInstance *)object);
...
static void tableRemoveWhite(KrkTable *table) {
    for (size_t i = 0; i < table->used; ++i) {
        KrkTableEntry *entry = &table->entries[i];
        if (IS_OBJECT(entry->key) &&
            !((AS_OBJECT(entry->key))->flags & KRK_OBJ_FLAGS_IS_MARKED)) {
...
static void markThreadRoots(KrkThreadState *thread) {
    ...
    if (thread->module)
        krk_markObject((KrkObj *)thread->module);
...
#    endif
    for (; count > 0 && *prefix; count--, prefix++) {
        ...
        if (s >= base) {
            size_t t = s / base;
            return snprintf(_out, 100, "%zu.%1zu %ciB", t, (s - t * base) / (base / 10),
                            *prefix);
        }
    }
...
size_t krk_collectGarbage(void) {
    ...
    if (vm.globalFlags & KRK_GLOBAL_REPORT_GC_COLLECTS) {
        ...
        fprintf(stderr,
                "[gc] %lld.%.9lds %s before; %s after; freed %s in %llu objects; next "
                "collection at %s\n",
                (long long)diff.tv_sec, diff.tv_nsec, smartBefore, smartAfter, smartFreed,
