...
KRK_Method(Thread, start) {
    ...
    if (!krk_parseArgs(".|I", (const char *[]){"maxrec"}, &maxrec))
        return NONE_VAL();
...
void krk_module_init_threading(void) {
    ...
    KRK_DOC(threadsModule, "@brief Methods and classes for creating platform threads.");

    KRK_DOC(BIND_FUNC(threadsModule, current_thread),
            "@brief Obtain a reference to the current thread.\n"
            "@arguments \n\n"
            "Returns the @ref Thread object associated with the calling thread, if one "
            "exists.");

    KrkClass *ThreadError = krk_makeClass(threadsModule, &KRK_EXC(ThreadError),
                                          "ThreadError", vm.exceptions->OSError);
    KRK_DOC(ThreadError,
            "Raised in various situations when an action on a thread is invalid.");
    krk_finalizeClass(ThreadError);

    KrkClass *Thread = krk_makeClass(threadsModule, &KRK_BASE_CLASS(Thread), "Thread",
                                     vm.baseClasses->objectClass);
    KRK_DOC(Thread, "Base class for building threaded execution contexts.\n\n"
                    "The @ref Thread class should be subclassed and the subclass should "
                    "implement a @c run method.");
    Thread->allocSize = sizeof(struct Thread);
    KRK_DOC(BIND_METHOD(Thread, start),
            "Start the thread. A thread may only be started once.");
    KRK_DOC(BIND_METHOD(Thread, join),
    ...
    KRK_DOC(BIND_PROP(Thread, tid),
            "The platform-specific thread identifier, if available. Usually an integer.");
    krk_finalizeClass(Thread);

    KrkClass *Lock = krk_makeClass(threadsModule, &KRK_BASE_CLASS(Lock), "Lock",
                                   vm.baseClasses->objectClass);
    KRK_DOC(Lock, "Represents an atomic mutex.\n\n"
                  "@ref Lock objects allow for exclusive access to a resource and can be "
                  "used in a @c with block.");
