...
KrkValue krk_dirObject(int argc, const KrkValue argv[], int hasKw) {
    if (argc != 1)
        return krk_runtimeError(vm.exceptions->argumentError,
                                "%s() takes %s %d argument%s (%d given)", "dir",
                                "exactly", 1, "", argc);
    ...
    if (IS_INSTANCE(argv[0])) {
        ...
        for (size_t i = 0; i < self->fields.capacity; ++i) {
            if (!IS_KWARGS(self->fields.entries[i].key)) {
                krk_writeValueArray(AS_LIST(myList), self->fields.entries[i].key);
            }
        }
        ...
        while (_class) {
            for (size_t i = 0; i < _class->methods.capacity; ++i) {
                if (!IS_KWARGS(_class->methods.entries[i].key)) {
                    krk_writeValueArray(AS_LIST(myList), _class->methods.entries[i].key);
                }
            }
            _class = _class->base;
        }
    ...
    while (type) {
        for (size_t i = 0; i < type->methods.capacity; ++i) {
            if (!IS_KWARGS(type->methods.entries[i].key)) {
                krk_writeValueArray(AS_LIST(myList), type->methods.entries[i].key);
            }
        }
        type = type->base;
    }
...
KRK_Method(object, __dir__) { return krk_dirObject(argc, argv, hasKw); }
...
KRK_Method(object, __class__) {
    ...
    if (argc > 1) {
        ...
        if (!IS_INSTANCE(argv[0]) || current->allocSize != sizeof(KrkInstance))
            return krk_runtimeError(vm.exceptions->typeError,
                                    "'%T' object does not have modifiable type",
                                    argv[0]); /* TODO class? */
        if (AS_CLASS(argv[1])->allocSize != sizeof(KrkInstance))
            return krk_runtimeError(vm.exceptions->typeError,
                                    "'%S' type is not assignable",
                                    AS_CLASS(argv[1])->name);
...
KRK_StaticMethod(object, __new__) {
    ...
    if (!krk_parseArgs("O!*~", (const char *[]){"cls"}, vm.baseClasses->typeClass,
                       &_class, &_argc, &_args)) {
        return NONE_VAL();
    }
    ...
    while (_cls) {
        if (_cls->_new && IS_NATIVE(OBJECT_VAL(_cls->_new)) &&
            _cls->_new != KRK_BASE_CLASS(object)->_new) {
            return krk_runtimeError(vm.exceptions->typeError,
                                    "object.__new__(%S) is not safe, use %S.__new__()",
                                    _class->name, _cls->name);
        }
        _cls = _cls->base;
    }

    if (_class->_init == vm.baseClasses->objectClass->_init &&
        (_argc || (hasKw && AS_DICT(argv[argc])->count))) {
        return krk_runtimeError(vm.exceptions->typeError, "%S() takes no arguments",
                                _class->name);
    }
...
KRK_Method(object, __init__) { return NONE_VAL(); }

KRK_StaticMethod(object, __init_subclass__) {
    if (!krk_parseArgs(".", (const char *[]){NULL}, NULL))
        return NONE_VAL();
    return NONE_VAL();
}
...
KRK_Method(object, __repr__) {
    ...
    int includeModule =
        !(IS_NONE(module) || (IS_STRING(module) && AS_STRING(module) == S("builtins")));

    struct StringBuilder sb = {0};

    if (!krk_pushStringBuilderFormat(&sb, "<%s%s%s object",
                                     includeModule ? AS_CSTRING(module) : "",
                                     includeModule ? "." : "", name->chars))
        goto _error;

    if (IS_OBJECT(self) &&
        !krk_pushStringBuilderFormat(&sb, " at %p", (void *)AS_OBJECT(self))) {
        goto _error;
    }
...
KRK_Method(object, __str__) {
    KrkClass *type = krk_getType(self);
    if (unlikely(!type->_reprer))
        return krk_runtimeError(vm.exceptions->typeError, "object is not representable");
    krk_push(self);
...
KRK_Method(object, __format__) {
    ...
    if (AS_STRING(argv[1])->length != 0)
        return krk_runtimeError(vm.exceptions->typeError, "Unsupported format string");
    KrkClass *type = krk_getType(argv[0]);
    if (!type->_tostr)
        return krk_runtimeError(vm.exceptions->typeError,
                                "'%T' can not be converted to str", argv[0]);
    krk_push(argv[0]);
...
KRK_Function(dir) {
    ...
    if (argc) {
        ...
        if (!krk_currentThread.frameCount) {
            return krk_runtimeError(vm.exceptions->nameError,
                                    "No active globals context");
        }
        ...
        KrkTable *globals =
            krk_currentThread.frames[krk_currentThread.frameCount - 1].globals;
...
#define KRK_STRING_FAST(string, offset)                                                  \
    (uint32_t)((string->obj.flags & KRK_OBJ_FLAGS_STRING_MASK) <=                        \
                       (KRK_OBJ_FLAGS_STRING_UCS1)                                       \
                   ? ((uint8_t *)string->codes)[offset]                                  \
                   : ((string->obj.flags & KRK_OBJ_FLAGS_STRING_MASK) ==                 \
                              (KRK_OBJ_FLAGS_STRING_UCS2)                                \
                          ? ((uint16_t *)string->codes)[offset]                          \
                          : ((uint32_t *)string->codes)[offset]))

int krk_unpackIterable(KrkValue iterable, void *context,
                       int callback(void *, const KrkValue *, size_t)) {
    if (IS_TUPLE(iterable)) {
        if (callback(context, AS_TUPLE(iterable)->values.values,
                     AS_TUPLE(iterable)->values.count))
            return 1;
    } else if (IS_list(iterable)) {
        if (callback(context, AS_LIST(iterable)->values, AS_LIST(iterable)->count))
            return 1;
    } else if (IS_dict(iterable)) {
        for (size_t i = 0; i < AS_DICT(iterable)->used; ++i) {
            if (!IS_KWARGS(AS_DICT(iterable)->entries[i].key)) {
                if (callback(context, &AS_DICT(iterable)->entries[i].key, 1))
                    return 1;
            }
        }
        ...
        if (unlikely(!type->_iter)) {
            krk_runtimeError(vm.exceptions->typeError, "'%T' object is not iterable",
                             iterable);
            return 1;
        }
...
KRK_Method(map, __call__) {
    ...
    if (!krk_tableGet(&self->fields, OBJECT_VAL(S("_function")), &function))
        return krk_runtimeError(vm.exceptions->valueError, "corrupt map object");
    if (!krk_tableGet(&self->fields, OBJECT_VAL(S("_iterables")), &iterators) ||
        !IS_TUPLE(iterators))
        return krk_runtimeError(vm.exceptions->valueError, "corrupt map object");
...
KRK_Method(zip, __init__) {
    if (hasKw && AS_DICT(argv[argc])->count)
        return krk_runtimeError(vm.exceptions->typeError,
                                "%s() takes no keyword arguments", "zip");
...
KRK_Method(zip, __call__) {
    ...
    if (!krk_tableGet(&self->fields, OBJECT_VAL(S("_iterables")), &iterators) ||
        !IS_TUPLE(iterators))
        return krk_runtimeError(vm.exceptions->valueError, "corrupt zip object");
...
KRK_Method(filter, __init__) {
    ...
    if (!type->_iter) {
        return krk_runtimeError(vm.exceptions->typeError, "'%T' object is not iterable",
                                argv[2]);
    }
    krk_push(argv[2]);
    KrkValue asIter = krk_callDirect(type->_iter, 1);
...
KRK_Method(filter, __call__) {
    ...
    if (!krk_tableGet(&self->fields, OBJECT_VAL(S("_function")), &function))
        return krk_runtimeError(vm.exceptions->valueError, "corrupt filter object");
    if (!krk_tableGet(&self->fields, OBJECT_VAL(S("_iterator")), &iterator))
        return krk_runtimeError(vm.exceptions->valueError, "corrupt filter object");
...
KRK_Method(enumerate, __init__) {
    ...
    if (!krk_parseArgs(".V|V", (const char *[]){"iterable", "start"}, &iterator, &start))
        return NONE_VAL();
...
KRK_Method(enumerate, __call__) {
    ...
    if (!krk_tableGet(&self->fields, OBJECT_VAL(S("_counter")), &counter))
        return krk_runtimeError(vm.exceptions->valueError, "corrupt enumerate object");
    if (!krk_tableGet(&self->fields, OBJECT_VAL(S("_iterator")), &iterator))
        return krk_runtimeError(vm.exceptions->valueError, "corrupt enumerate object");
...
KRK_Function(sum) {
    ...
    if (!krk_parseArgs("V|$V", (const char *[]){"iterable", "start"}, &iterable, &base))
        return NONE_VAL();
    struct SimpleContext context = {base};
    if (krk_unpackIterable(iterable, &context, _sum_callback))
        return NONE_VAL();
    return context.base;
}
...
static int _min_callback(void *context, const KrkValue *values, size_t count) {
    struct SimpleContext *_context = context;
    for (size_t i = 0; i < count; ++i) {
        if (IS_KWARGS(_context->base))
            _context->base = values[i];
        else {
            KrkValue check = krk_operator_lt(values[i], _context->base);
            if (!IS_BOOLEAN(check))
                return 1;
            else if (AS_BOOLEAN(check) == 1)
                _context->base = values[i];
        }
    }
    return 0;
...
KRK_Function(min) {
    ...
    if (argc > 1) {
        if (_min_callback(&context, argv, argc))
            return NONE_VAL();
    } else {
        if (krk_unpackIterable(argv[0], &context, _min_callback))
            return NONE_VAL();
    }
    if (IS_KWARGS(context.base))
        return krk_runtimeError(vm.exceptions->valueError, "empty argument to %s()",
                                "min");
    return context.base;
}
...
KRK_Function(max) {
    ...
    struct SimpleContext context = {KWARGS_VAL(0)};
    if (argc > 1) {
        if (_max_callback(&context, argv, argc))
            return NONE_VAL();
    } else {
        if (krk_unpackIterable(argv[0], &context, _max_callback))
            return NONE_VAL();
    }
    if (IS_KWARGS(context.base))
        return krk_runtimeError(vm.exceptions->valueError, "empty argument to %s()",
...
KRK_Function(print) {
    ...
    if (!krk_parseArgs("*z#z#Vp", (const char *[]){"sep", "end", "file", "flush"},
                       &remArgc, &remArgv, &sep, &sepLen, &end, &endLen, &file, &flush)) {
        return NONE_VAL();
    }
    ...
    for (int i = 0; i < remArgc; ++i) {
        ...
        /* Convert the argument to a printable form, first by trying __str__, then
         * __repr__ */
...
KRK_Function(globals) {
    ...
    krk_tableAddAll(krk_currentThread.frames[krk_currentThread.frameCount - 1].globals,
                    AS_DICT(dict));
    krk_pop();
...
KRK_Function(locals) {
    ...
    if (argc > 0 && IS_INTEGER(argv[0])) {
        if (AS_INTEGER(argv[0]) < 1) {
            return krk_runtimeError(vm.exceptions->indexError,
                                    "Frame index must be >= 1");
        }
    ...
    for (short int i = 0; i < func->potentialPositionals; ++i) {
        krk_tableSet(AS_DICT(dict), func->positionalArgNames.values[i],
    ...
    for (short int i = 0; i < func->keywordArgs; ++i) {
        krk_tableSet(AS_DICT(dict), func->keywordArgNames.values[i],
                     krk_currentThread.stack[frame->slots + slot]);
        slot++;
    }
    if (func->obj.flags & KRK_OBJ_FLAGS_CODEOBJECT_COLLECTS_KWS) {
        krk_tableSet(AS_DICT(dict), func->keywordArgNames.values[func->keywordArgs],
                     krk_currentThread.stack[frame->slots + slot]);
        slot++;
    }
    ...
    for (size_t i = 0; i < func->localNameCount; ++i) {
        if (func->localNames[i].birthday <= offset &&
            ...
            krk_tableSet(AS_DICT(dict), OBJECT_VAL(func->localNames[i].name),
...
KRK_Function(isinstance) {
    ...
    if (IS_CLASS(argv[1])) {
        ...
        for (size_t i = 0; i < AS_TUPLE(argv[1])->values.count; ++i) {
            if (IS_CLASS(AS_TUPLE(argv[1])->values.values[i]) &&
                krk_isInstanceOf(argv[0],
                                 AS_CLASS(AS_TUPLE(argv[1])->values.values[i]))) {
                return BOOLEAN_VAL(1);
            }
        }
        return BOOLEAN_VAL(0);
...
KRK_Function(issubclass) {
    ...
    if (IS_CLASS(argv[1])) {
        ...
        for (size_t i = 0; i < AS_TUPLE(argv[1])->values.count; ++i) {
            if (IS_CLASS(AS_TUPLE(argv[1])->values.values[i]) &&
                krk_isSubClass(cls, AS_CLASS(AS_TUPLE(argv[1])->values.values[i]))) {
                return BOOLEAN_VAL(1);
            }
        }
        return BOOLEAN_VAL(0);
...
KRK_Method(module, __repr__) {
    ...
    if (!krk_pushStringBuilderFormat(&sb, "<module '%S' ", AS_STRING(name)))
        goto _error;

    if (IS_STRING(file)) {
        if (!krk_pushStringBuilderFormat(&sb, "from %R>", file))
            goto _error;
    } else {
        if (!krk_pushStringBuilderFormat(&sb, "(built-in)>"))
            goto _error;
    }
...
KRK_Method(Helper, __repr__) {
    return OBJECT_VAL(
        S("Type help() for more help, or help(obj) to describe an object."));
}

KRK_Method(Helper, __call__) {
    METHOD_TAKES_AT_MOST(1);
    if (!krk_doRecursiveModuleLoad(S("help")))
        return NONE_VAL();
    KrkValue helpModule = krk_pop();
    KrkValue callable = NONE_VAL();

    if (argc == 2) {
        krk_tableGet(&AS_INSTANCE(helpModule)->fields, OBJECT_VAL(S("simple")),
                     &callable);
    } else {
        krk_tableGet(&AS_INSTANCE(helpModule)->fields, OBJECT_VAL(S("interactive")),
                     &callable);
    }
...
KRK_Method(LicenseReader, __repr__) {
    return OBJECT_VAL(S("Copyright 2020-2024 K. Lange <klange@toaruos.org>. Type "
...
static void _property_gcscan(KrkInstance *_self) {
    ...
    if (self->fget)
        krk_markObject(self->fget);
    if (self->fset)
        krk_markObject(self->fset);
}
...
KRK_Method(property, __init__) {
    ...
    if (IS_NATIVE(argv[1])) {
        krk_attachNamedValue(
            &self->fields, "__doc__",
            AS_NATIVE(argv[1])->doc
                ? OBJECT_VAL(krk_copyString(AS_NATIVE(argv[1])->doc,
                                            strlen(AS_NATIVE(argv[1])->doc)))
                : NONE_VAL());
    } else if (IS_CLOSURE(argv[1])) {
        krk_attachNamedValue(&self->fields, "__doc__",
                             AS_CLOSURE(argv[1])->function->docstring
    ...
    if (IS_NATIVE(argv[1])) {
        krk_attachNamedValue(
            &self->fields, "__name__",
            AS_NATIVE(argv[1])->name
                ? OBJECT_VAL(krk_copyString(AS_NATIVE(argv[1])->name,
                                            strlen(AS_NATIVE(argv[1])->name)))
                : NONE_VAL());
    } else if (IS_CLOSURE(argv[1])) {
        krk_attachNamedValue(&self->fields, "__name__",
                             AS_CLOSURE(argv[1])->function->name
...
KRK_Method(property, __get__) {
    ...
    if (!krk_tableGet(&self->fields, OBJECT_VAL(S("fget")), &fget))
        return krk_runtimeError(vm.exceptions->attributeError,
                                "'%T' object has no attribute '%s'", argv[0], "fget");
...
/**
 * Create a new property object that calls a C function; same semantics as defineNative,
 * but instead of applying the function directly it is applied as a property value, so it
 * should be used with the "fields" table rather than the methods table. This will
 * eventually replace the ":field" option for defineNative().
 */
KrkNative *krk_defineNativeProperty(KrkTable *table, const char *name,
                                    NativeFn function) {
...
KRK_Function(id) {
    FUNCTION_TAKES_EXACTLY(1);
    if (!IS_OBJECT(argv[0]))
        return krk_runtimeError(vm.exceptions->typeError, "'%T' has no identity",
                                argv[0]);
...
KRK_Function(hash) {
    FUNCTION_TAKES_EXACTLY(1);
    uint32_t hashed;
    if (krk_hashValue(argv[0], &hashed))
        return NONE_VAL();
    return INTEGER_VAL(hashed);
}
...
KRK_Function(format) {
    ...
    if (!type->_format) {
        return krk_runtimeError(vm.exceptions->typeError, "'%T' has no __format__ method",
                                argv[0]);
    }
...
KRK_Function(__build_class__) {
    ...
    if (!krk_parseArgs("VO!|O!$V~", (const char *[]){"func", "name", "base", "metaclass"},
                       &func, vm.baseClasses->strClass, &name, vm.baseClasses->typeClass,
                       &base, &metaclass)) {
        return NONE_VAL();
    }
    ...
    if (IS_CLASS(metaclass)) {
        ...
        if (krk_isSubClass(AS_CLASS(metaclass), basemeta)) {
            ...
            return krk_runtimeError(vm.exceptions->typeError,
                                    "metaclass conflict: %S is not a subclass of %S",
                                    AS_CLASS(metaclass)->name, basemeta->name);
        }
    }
    ...
    if (IS_CLOSURE(func)) {
        if (AS_CLOSURE(func)->upvalueCount &&
            AS_CLOSURE(func)->upvalues[0]->location == -1 &&
            IS_NONE(AS_CLOSURE(func)->upvalues[0]->closed)) {
...
KRK_StaticMethod(Cell, __new__) {
    ...
    if (!krk_parseArgs("O!|V:Cell", (const char *[]){"cls", "contents"},
                       KRK_BASE_CLASS(type), &_class, &contents)) {
        return NONE_VAL();
    }
    if (_class != KRK_BASE_CLASS(Cell)) {
        return krk_runtimeError(vm.exceptions->typeError,
                                "can not assemble new Cell from %R", OBJECT_VAL(_class));
    }
...
KRK_Method(Cell, __repr__) {
    ...
    if (!krk_pushStringBuilderFormat(&sb, "<cell at %p: %T object", (void *)self,
                                     contents))
        goto _error;
    if (IS_OBJECT(contents)) {
        if (!krk_pushStringBuilderFormat(&sb, " at %p>", (void *)AS_OBJECT(contents)))
            goto _error;
    } else {
        krk_pushStringBuilder(&sb, '>');
    }
...
_noexport void _createAndBind_builtins(void) {
    ...
    KRK_DOC(
        ...
        "@ref object___dir__ \"__dir__\". All object and primitive types eventually "
        "inherit from @c object.");
    ...
    krk_attachNamedObject(&vm.builtins->fields, "object",
                          (KrkObj *)vm.baseClasses->objectClass);
    krk_pop();
    krk_pop();
    ...
    KRK_DOC(
        ...
        "Classes and functions from the @c \\__builtins__ module are generally available "
        "from "
        "all global namespaces. Built-in names can still be shadowed by module-level "
        "globals "
        ...
        "Built-in names may be bound from several sources. Most come from the core "
        "interpreter "
        "directly, but some may come from loaded C extension modules or the interpreter "
        "binary. "
        "Kuroko source modules are also free to append new names to the built-in name "
        "space by "
    ...
    KRK_DOC(BIND_METHOD(property, __init__),
            ...
            "@arguments fget,[fset]\n\n"
            "When a property object is obtained from an instance of the class in which "
            "it is defined, "
            "the function or method assigned to @p fget is called with the instance as "
            "an argument. "
            "If @p fset is provided, it will be called with the instance and a value "
            "when the property "
            "object is assigned to through an instance. For legacy compatibility "
            "reasons, a property "
            "object's @p fget method may also accept an additional argument to act as a "
            "setter if "
            "@p fset is not provided, but this functionality may be removed in the "
            "future.\n\n"
            "The typical use for @c property is as a decorator on methods in a class. "
            "See also "
            "@ref property_setter \"property.setter\" for the newer Python-style "
            "approach to decorating a companion "
            "setter method.");
    ...
    KRK_DOC(BIND_METHOD(property, setter),
            ...
            "@arguments fset\n\n"
            "This should be used as a decorator from an existing property object as "
            "follows:\n\n"
            "```\n"
            "class Foo():\n"
            "    @property\n"
            ...
            "```\n"
            "Be sure to apply the decorator to a function or method with the same name, "
            "as this "
            "name will be used to assign the property to the class's attribute table; "
            "using a "
    ...
    KRK_DOC(Helper, "@brief Special object that prints a helpeful message.\n\n"
                    "Object that prints help summary when passed to @ref repr.");
    KRK_DOC(BIND_METHOD(Helper, __call__), "@brief Prints help text.\n"
                                           "@arguments obj=None\n\n"
                                           "Prints the help documentation attached to @p "
                                           "obj or starts the interactive help system by "
                                           "importing the @ref mod_help module.");
    BIND_METHOD(Helper, __repr__);
    krk_finalizeClass(Helper);
    krk_attachNamedObject(&vm.builtins->fields, "help",
                          (KrkObj *)krk_newInstance(Helper));

    KrkClass *LicenseReader =
        ADD_BASE_CLASS(KRK_BASE_CLASS(LicenseReader), "LicenseReader", object);
    KRK_DOC(LicenseReader, "Special object that prints Kuroko's copyright information "
                           "when passed to @ref repr");
    ...
    krk_attachNamedObject(&vm.builtins->fields, "license",
                          (KrkObj *)krk_newInstance(LicenseReader));
    ...
    KRK_DOC(zip, "@brief Returns an iterator that produces tuples of the nth element of "
                 "each passed iterable.\n"
                 "@arguments *iterables\n\n"
                 "Creates an iterator that returns a tuple of elements from each of @p "
                 "iterables, until one "
    ...
    KRK_DOC(filter, "Return an iterator that returns only the items from an iterable for "
    ...
    KRK_DOC(enumerate, "Return an iterator that produces a tuple with a count the "
                       "iterated values of the passed iteratable.");
    ...
    BUILTIN_FUNCTION("isinstance", FUNC_NAME(krk, isinstance),
                     ...
                     "@arguments inst, cls\n\n"
                     "Determine if an object @p inst is an instance of the given class "
                     "@p cls or one if its subclasses. "
    ...
    BUILTIN_FUNCTION("issubclass", FUNC_NAME(krk, issubclass),
                     ...
                     "Determine if the class @p cls is a subclass of the class @p "
                     "clsinfo. @p clsinfo may be a single "
    ...
    BUILTIN_FUNCTION(
        ...
        "Produces a dict mapping all of the names of the current globals namespace to "
        "their values. "
        "Updating this dict has no meaning, but modifying mutable values within it can "
    ...
    BUILTIN_FUNCTION(
        ...
        "Produces a dict mapping the names of the requested locals scope to their "
        "current stack values. "
        "If @p callDepth is provided, the locals of an outer call frame will be "
        "returned. If the requested "
    ...
    BUILTIN_FUNCTION("dir", FUNC_NAME(krk, dir),
                     ...
                     "@arguments [obj]\n\n"
                     "Uses various internal methods to collect a list of property names "
                     "of @p obj, returning "
                     "that list sorted lexicographically. If no argument is given, the "
                     "returned list will "
    ...
    BUILTIN_FUNCTION("len", FUNC_NAME(krk, len),
                     ...
                     "@arguments seq\n\n"
                     "Returns the length of the sequence object @p seq, which must "
                     "implement @c __len__.");
    BUILTIN_FUNCTION("repr", FUNC_NAME(krk, repr),
                     "@brief Produce a string representation of the given object.\n"
                     "@arguments val\n\n"
                     "Return a string representation of the given object through its @c "
                     "__repr__ method. "
                     "@c repr strings should convey all information needed to recreate "
                     "the object, if this is possible.");
    BUILTIN_FUNCTION(
        ...
        "The keyword argument @p end specifies the string to print after all of the "
    ...
    BUILTIN_FUNCTION("chr", FUNC_NAME(krk, chr),
                     ...
                     "@arguments codepoint\n\n"
                     "Creates a single-codepoint string with the character represented "
                     "by the integer codepoint @p codepoint.");
    BUILTIN_FUNCTION(
    ...
    BUILTIN_FUNCTION("any", FUNC_NAME(krk, any),
                     "@brief Returns True if at least one element in the given iterable "
                     "is truthy, False otherwise.\n"
                     "@arguments iterable");
    BUILTIN_FUNCTION("all", FUNC_NAME(krk, all),
                     "@brief Returns True if every element in the given iterable is "
    ...
    BUILTIN_FUNCTION(
        ...
        "descriptor's @c \\__set__ method will be called. If @p obj is a class, "
        "instance, "
    ...
    BUILTIN_FUNCTION("sum", FUNC_NAME(krk, sum),
                     ...
                     "Continuously adds all of the elements from @p iterable to @p start "
    ...
    BUILTIN_FUNCTION(
        ...
        "Internally, this is the pointer value for a heap object, but this is an "
        "implementation detail.");
    BUILTIN_FUNCTION("hash", FUNC_NAME(krk, hash),
                     "@brief Returns the hash of a value, used for table indexing.\n"
                     "@arguments val\n\n"
                     "If @p val is hashable, its hash value will be calculated if "
