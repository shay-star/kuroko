...
static __attribute__((used)) int _isdigit(int c) {
    if (c > 128)
        return 0;
    return isdigit(c);
}
static __attribute__((used)) int _isxdigit(int c) {
    if (c > 128)
        return 0;
    return isxdigit(c);
}
...
void rline_history_append_line(char *str) {
    if (rline_history_count) {
        char **s = &rline_history[(rline_history_count - 1 + rline_history_offset) %
...
static inline uint32_t decode(uint32_t *state, uint32_t *codep, uint32_t byte) {
    ...
    static int mask_bytes[32] = {0x7F, 0x7F, 0x7F, 0x7F, 0x7F, 0x7F, 0x7F, 0x7F,
                                 0x7F, 0x7F, 0x7F, 0x7F, 0x7F, 0x7F, 0x7F, 0x7F,
                                 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                 0x1F, 0x1F, 0x1F, 0x1F, 0x0F, 0x0F, 0x07, 0x00};
...
#define paint(length, flag)                                                              \
    do {                                                                                 \
        for (int i = 0; i < (length) && state->i < state->line->actual;                  \
             i++, state->i++) {                                                          \
            state->line->text[state->i].flags = (flag);                                  \
        }                                                                                \
    } while (0)
#define charat()                                                                         \
    (state->i < state->line->actual ? state->line->text[(state->i)].codepoint : -1)
#define nextchar()                                                                       \
    (state->i + 1 < state->line->actual ? state->line->text[(state->i + 1)].codepoint    \
                                        : -1)
#define lastchar() (state->i - 1 >= 0 ? state->line->text[(state->i - 1)].codepoint : -1)
#define skip() (state->i++)
#define charrel(x)                                                                       \
    (state->i + (x) < state->line->actual                                                \
         ? state->line->text[(state->i + (x))].codepoint                                 \
         : -1)
...
static int match_and_paint(struct syntax_state *state, const char *keyword, int flag,
                           int (*keyword_qualifier)(int c)) {
    if (keyword_qualifier(lastchar()))
        return 0;
    if (!keyword_qualifier(charat()))
        return 0;
    int i = state->i;
    int slen = 0;
    while (i < state->line->actual || *keyword == '\0') {
        if (*keyword == '\0' && (i >= state->line->actual ||
                                 !keyword_qualifier(state->line->text[i].codepoint))) {
            for (int j = 0; j < slen; ++j) {
                paint(1, flag);
            }
            return 1;
        }
        if (*keyword != state->line->text[i].codepoint)
            return 0;
...
/**
 * Find keywords from a list and paint them, assuming they aren't in the middle of other
 * words. Returns 1 if a keyword from the last was found, otherwise 0.
 */
static int find_keywords(struct syntax_state *state, char **keywords, int flag,
                         int (*keyword_qualifier)(int c)) {
    if (keyword_qualifier(lastchar()))
        return 0;
    if (!keyword_qualifier(charat()))
    ...
    for (char **keyword = keywords; *keyword; ++keyword) {
        int d = 0;
        while (state->i + d < state->line->actual &&
               state->line->text[state->i + d].codepoint == (*keyword)[d])
            d++;
        if ((*keyword)[d] == '\0' &&
            (state->i + d >= state->line->actual ||
             !keyword_qualifier(state->line->text[state->i + d].codepoint))) {
 ...
 */
static int simple_keyword_qualifier(int c) { return isalnum(c) || (c == '_'); }

static int common_comment_buzzwords(struct syntax_state *state) {
    if (match_and_paint(state, "TODO", FLAG_NOTICE, simple_keyword_qualifier)) {
        return 1;
    } else if (match_and_paint(state, "XXX", FLAG_NOTICE, simple_keyword_qualifier)) {
        return 1;
    } else if (match_and_paint(state, "FIXME", FLAG_ERROR, simple_keyword_qualifier)) {
        return 1;
    }
    return 0;
}
...
static int paint_comment(struct syntax_state *state) {
    while (charat() != -1) {
        if (common_comment_buzzwords(state))
            continue;
        else {
...
static int c_keyword_qualifier(int c) { return isalnum(c) || (c == '_'); }
...
static char *syn_krk_keywords[] = {
    ...
    "del",   "let",      "not",    "or",    "return",  "while", "try",    "except",
    "raise", "continue", "break",  "as",    "from",    "elif",  "lambda", "with",
    "is",    "pass",     "assert", "yield", "finally", "async", "await",  NULL};

static char *syn_krk_types[] = {
    /* built-in functions */
    "self",        "super", /* implicit in a class method */
    "len",         "str",        "int",     "float",    "dir",
    "repr",                               /* global functions from __builtins__ */
    "list",        "dict",       "range", /* builtin classes */
    "object",      "isinstance", "type",    "tuple",    "reversed",
    "print",       "set",        "any",     "all",      "bool",
    "ord",         "chr",        "hex",     "oct",      "filter",
    "sorted",      "bytes",      "getattr", "sum",      "min",
    "max",         "id",         "hash",    "map",      "bin",
    "enumerate",   "zip",        "setattr", "property", "staticmethod",
    "classmethod", "issubclass", "hasattr", "delattr",  "NotImplemented",
    "abs",         "slice",      "long",    NULL};

static char *syn_krk_special[] = {"True", "False", "None",
                                  /* Exception names */
                                  NULL};

static char *syn_krk_exception[] = {"Exception",
                                    "TypeError",
                                    "ArgumentError",
                                    "IndexError",
                                    "KeyError",
...
static void paint_krk_string_shared(struct syntax_state *state, int type, int isFormat,
                                    int isTriple) {
    if (charat() == '\\') {
        if (nextchar() == 'x') {
            paintNHex(state, 2);
        ...
        if (charat() == '}') {
            ...
            while (charat() != -1) {
                if (charat() == '{') {
                    ...
                    return;
                } else if (find_keywords(state, syn_krk_keywords, FLAG_ESCAPE,
                                         c_keyword_qualifier)) {
                    continue;
                } else if (lastchar() != '.' &&
                           find_keywords(state, syn_krk_types, FLAG_TYPE,
                                         c_keyword_qualifier)) {
                    continue;
                } else if (find_keywords(state, syn_krk_exception, FLAG_PRAGMA,
                                         c_keyword_qualifier)) {
                    continue;
                }
...
static int paint_krk_numeral(struct syntax_state *state) {
    if (charat() == '0' && (nextchar() == 'x' || nextchar() == 'X')) {
        ...
        while (isxdigit(charat()) || charat() == '_')
            paint(1, FLAG_NUMERAL);
    } else if (charat() == '0' && (nextchar() == 'o' || nextchar() == 'O')) {
        paint(2, FLAG_NUMERAL);
        while ((charat() >= '0' && charat() <= '7') || charat() == '_')
            paint(1, FLAG_NUMERAL);
    } else if (charat() == '0' && (nextchar() == 'b' || nextchar() == 'B')) {
        paint(2, FLAG_NUMERAL);
        while (charat() == '0' || charat() == '1' || charat() == '_')
            paint(1, FLAG_NUMERAL);
    } else {
        while (isdigit(charat()) || charat() == '_')
            paint(1, FLAG_NUMERAL);
...
static int syn_krk_calculate(struct syntax_state *state) {
    ...
    case 0:
        if (charat() == '#') {
            ...
            return 0;
        } else if (find_keywords(state, syn_krk_keywords, FLAG_KEYWORD,
                                 c_keyword_qualifier)) {
            return 0;
        } else if (lastchar() != '.' &&
                   find_keywords(state, syn_krk_types, FLAG_TYPE, c_keyword_qualifier)) {
            return 0;
        } else if (find_keywords(state, syn_krk_special, FLAG_NUMERAL,
                                 c_keyword_qualifier)) {
            return 0;
        } else if (find_keywords(state, syn_krk_exception, FLAG_PRAGMA,
                                 c_keyword_qualifier)) {
...
static char *syn_krk_dbg_commands[] = {
    "s", "skip",   "c",  "continue",  "q",     "quit",  "e",    "enable", "d", "disable",
    "r", "remove", "bt", "backtrace", "break", "abort", "help", NULL,
};
...
static int syn_krk_dbg_calculate(struct syntax_state *state) {
    if (state->state < 1) {
        if (state->i == 0) {
            ...
            } else if (match_and_paint(state, "info", FLAG_KEYWORD,
                                       c_keyword_qualifier) ||
                       match_and_paint(state, "i", FLAG_KEYWORD, c_keyword_qualifier)) {
                skip();
                find_keywords(state, syn_krk_dbg_info_types, FLAG_TYPE,
                              c_keyword_qualifier);
                return -1;
            } else if (find_keywords(state, syn_krk_dbg_commands, FLAG_KEYWORD,
                                     c_keyword_qualifier)) {
                return 0;
            }
        }
...
static int paint_esh_variable(struct syntax_state *state) {
    if (charat() == '{') {
        ...
        if (charat() == '?' || charat() == '$' || charat() == '#') {
            ...
            while (esh_variable_qualifier(charat()))
                paint(1, FLAG_TYPE);
        }
    }
    return 0;
...
static int esh_keyword_qualifier(int c) {
    return (isalnum(c) || c == '?' || c == '_' ||
            c == '-'); /* technically anything that isn't a space should qualify... */
}

static char *esh_keywords[] = {"cd",     "exit",   "export",  "help",   "history",
                               "if",     "empty?", "equals?", "return", "export-cmd",
                               "source", "exec",   "not",     "while",  "then",
                               "else",   "echo",   NULL};
...
static int syn_esh_calculate(struct syntax_state *state) {
    ...
    if (charat() == '#') {
        ...
        while (charat() == ' ')
            skip();
        while (esh_keyword_qualifier(charat()))
            paint(1, FLAG_TYPE);
        return 0;
    } else if (match_and_paint(state, "export-cmd", FLAG_KEYWORD,
                               esh_keyword_qualifier)) {
        while (charat() == ' ')
            skip();
        while (esh_keyword_qualifier(charat()))
            paint(1, FLAG_TYPE);
        ...
        return 0;
    } else if (find_keywords(state, shell_commands, FLAG_KEYWORD,
                             esh_keyword_qualifier)) {
        return 0;
...
static char *syn_py_keywords[] = {
    "class",    "def",      "return", "del",    "if",  "else",  "elif",   "for",
    "while",    "continue", "break",  "assert", "as",  "and",   "or",     "except",
    "finally",  "from",     "global", "import", "in",  "is",    "lambda", "with",
    "nonlocal", "not",      "pass",   "raise",  "try", "yield", NULL};

static char *syn_py_types[] = {
    /* built-in functions */
    "abs",        "all",     "any",       "ascii",      "bin",        "bool",
    "breakpoint", "bytes",   "bytearray", "callable",   "compile",    "complex",
    "delattr",    "chr",     "dict",      "dir",        "divmod",     "enumerate",
    ...
    "pow",        "print",   "property",  "range",      "repr",       "reverse",
    "round",      "set",     "setattr",   "slice",      "sorted",     "staticmethod",
    "str",        "sum",     "super",     "tuple",      "type",       "vars",
    "zip",        NULL};

static char *syn_py_special[] = {"True", "False", "None", NULL};
    ...
    case 0:
        if (charat() == '#') {
            paint_comment(state);
                   ...
                   match_and_paint(state, "import", FLAG_PRAGMA, c_keyword_qualifier)) {
            return 0;
        } else if (charat() == '@') {
            paint(1, FLAG_PRAGMA);
            while (c_keyword_qualifier(charat()))
        ...
        } else if (find_keywords(state, syn_py_keywords, FLAG_KEYWORD,
                                 c_keyword_qualifier)) {
            return 0;
        } else if (lastchar() != '.' &&
                   find_keywords(state, syn_py_types, FLAG_TYPE, c_keyword_qualifier)) {
            return 0;
        } else if (find_keywords(state, syn_py_special, FLAG_NUMERAL,
                                 c_keyword_qualifier)) {
            return 0;
        } else if (charat() == '\'') {
            if (nextchar() == '\'' && charrel(2) == '\'') {
    ...
    while (i < line->actual) {
        ...
        if (j >= offset) {
            ...
            if (c.codepoint == '\t') {
                ...
                set_colors(COLOR_ALT_FG,
                           COLOR_BG); /* Normal background so this is more subtle */
                printf("▏");
                set_colors(last_color ? last_color : COLOR_FG, COLOR_BG);
            } else {
...
static line_t *line_insert(line_t *line, char_t c, int offset) {
    ...
    if (offset < line->actual) {
        memmove(&line->text[offset + 1], &line->text[offset],
                sizeof(char_t) * (line->actual - offset));
    }
...
static void line_delete(line_t *line, int offset) {
    ...
    /* If this isn't the last character, we need to move all subsequent characters
     * backwards */
    if (offset < line->actual) {
        memmove(&line->text[offset - 1], &line->text[offset],
                sizeof(char_t) * (line->actual - offset));
...
static void find_matching_paren(int *out_col, int in_col) {
    ...
    while (col > -1 && col < the_line->actual) {
        ...
        if ((the_line->text[col].flags & 0x1F) == flags) {
            ...
            if (the_line->text[col].codepoint == start)
                count++;
...
static void word_right(void) {
    ...
    while (column < the_line->actual) {
        column++;
        if (column < the_line->actual && the_line->text[column].codepoint == ' ')
            break;
    }
    rline_place_cursor();
}
...
static void history_next(void) {
    if (rline_scroll >= 1) {
        unsigned char *buf;
        if (rline_scroll > 1)
            buf = (unsigned char *)rline_history_prev(rline_scroll - 1);
        else
            buf = (unsigned char *)temp_buffer;
        rline_scroll--;
...
static void set_buffered(void) { tcsetattr(STDOUT_FILENO, TCSADRAIN, &old); }
#else
...
static void dummy_redraw(rline_context_t *context) { /* Do nothing */ }
        ...
        if (!decode(&istate, &c, cin)) {
            if (timeout == 0) {
                ...
                case 21: /* ^U - Kill to beginning */
                    ...
                    break;
                case '\t':
                    if ((syntax && syntax->tabIndents) &&
                        (column == 0 || the_line->text[column - 1].codepoint == ' ')) {
...
void rline_insert(rline_context_t *context, const char *what) {
    ...
    /* Move */
    memmove(&context->buffer[context->offset + insertion_length],
            &context->buffer[context->offset], context->collected - context->offset);
